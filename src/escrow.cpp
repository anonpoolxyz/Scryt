// Copyright (c) 2015-2018 The ScrytExchange developers
...
bool CEscrow::UnserializeFromTx(const CTransaction &tx) {
	...
	int nOut;
	if(!GetScrytData(tx, vchData, vchHash, nOut))
	{
		SetNull();
		return false;
	}
...
void CEscrowDB::WriteEscrowIndex(const COffer& offer, const CEscrow& escrow, const std::vector<std::vector<unsigned char> > &vvchArgs) {
	if (IsArgSet("-zmqpubescrowrecord")) {
		...
		if (BuildEscrowIndexerJson(offer, escrow, oName)) {
			GetMainSignals().NotifyScrytUpdate(oName.write().c_str(), "escrowrecord");
		}
	}
}
void CEscrowDB::WriteEscrowFeedbackIndex(const COffer &offer, const CEscrow& escrow) {
	if (IsArgSet("-zmqpubescrowfeedback")) {
		...
		GetMainSignals().NotifyScrytUpdate(oName.write().c_str(), "escrowfeedback");
	}
}
void CEscrowDB::WriteEscrowBidIndex(const COffer& offer, const CEscrow& escrow, const string& status) {
	...
	if (IsArgSet("-zmqpubescrowbid")) {
		...
		GetMainSignals().NotifyScrytUpdate(oName.write().c_str(), "escrowbid");
	}
}
...
bool CheckEscrowInputs(const CTransaction &tx, int op, const vector<vector<unsigned char> > &vvchArgs, const std::vector<std::vector<unsigned char> > &vvchAliasArgs, bool fJustCheck, int nHeight, string &errorMessage, bool bSanityCheck) {
	...
	vector<unsigned char> vchHash;
	int nDataOut;
	if(!GetScrytData(tx, vchData, vchHash, nDataOut) || !theEscrow.UnserializeFromData(vchData, vchHash))
	{
		errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR ERRCODE: 4000 - " + _("Cannot unserialize data inside of this transaction relating to an escrow");
		return true;
	}
	...
	if(fJustCheck)
	{
		if(vvchArgs.size() != 1)
		{
			errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4001 - " + _("Escrow arguments incorrect size");
			return error(errorMessage.c_str());
		}
		if(vchHash != vvchArgs[0])
		{
			errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4002 - " + _("Hash provided doesn't match the calculated hash of the data");
			return true;
		}
	...
	if(fJustCheck)
	{
		...
		{
			errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4003 - " + _("Escrow redeem script too long");
			return error(errorMessage.c_str());
		}
		if(theEscrow.feedback.vchFeedback.size() > MAX_VALUE_LENGTH)
		{
			errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4004 - " + _("Feedback too long");
			return error(errorMessage.c_str());
		}
		if (theEscrow.nQty <= 0)
		{
			errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4005 - " + _("Quantity of order must be greator than 0");
			return error(errorMessage.c_str());
		}
		if(theEscrow.vchOffer.size() > MAX_GUID_LENGTH)
		{
			errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4006 - " + _("Escrow offer guid too long");
			return error(errorMessage.c_str());
		}
		switch (op) {
			case OP_ESCROW_ACKNOWLEDGE:
			...
			case OP_ESCROW_ACTIVATE:
				...
				{
					errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4007 - " + _("escrow hex guid too long");
					return error(errorMessage.c_str());
				}
				if(!IsValidPaymentOption(theEscrow.nPaymentOption))
				{
					errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4008 - " + _("Invalid payment option");
					return error(errorMessage.c_str());
				}
				if (!theEscrow.extTxId.IsNull() && theEscrow.nPaymentOption == PAYMENTOPTION_SCY)
				{
					errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4009 - " + _("External payment cannot be paid with SCY");
					return error(errorMessage.c_str());
				}
				if (theEscrow.extTxId.IsNull() && theEscrow.nPaymentOption != PAYMENTOPTION_SCY)
				{
					errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4010 - " + _("External payment missing transaction ID");
					return error(errorMessage.c_str());
				}
				if(!theEscrow.feedback.IsNull())
				{
					errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4011 - " + _("Cannot leave feedback in escrow activation");
					return error(errorMessage.c_str());
				}
				break;
			case OP_ESCROW_BID:
				if (!theEscrow.feedback.IsNull())
				{
					errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4012 - " + _("Cannot leave feedback in escrow bid");
					return error(errorMessage.c_str());
				}
				break;
			case OP_ESCROW_ADD_SHIPPING:
				if (!theEscrow.feedback.IsNull())
				{
					errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4013 - " + _("Cannot leave feedback in escrow bid");
					return error(errorMessage.c_str());
				}
				break;
			case OP_ESCROW_RELEASE:
				if(!theEscrow.feedback.IsNull())
				{
					errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4014 - " + _("Cannot leave feedback in escrow release");
					return error(errorMessage.c_str());
				}									
				if (theEscrow.role != EscrowRoles::BUYER && theEscrow.role != EscrowRoles::ARBITER)
				{
					errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4015 - " + _("Invalid role specified. Only arbiter or buyer can initiate an escrow release");
					return error(errorMessage.c_str());
				}
				break;
			case OP_ESCROW_RELEASE_COMPLETE:
				if (!theEscrow.feedback.IsNull())
				{
					errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4016 - " + _("Cannot leave feedback in escrow release");
					return error(errorMessage.c_str());
				}
				break;
			case OP_ESCROW_FEEDBACK:
				if(theEscrow.feedback.IsNull())
				{
					errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4017 - " + _("Feedback must leave a message");
					return error(errorMessage.c_str());
				}
			...
			case OP_ESCROW_REFUND:									
				...
				{
					errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4018 - " + _("Invalid role specified. Only arbiter or seller can initiate an escrow refund");
					return error(errorMessage.c_str());
				}
				...
				if(!theEscrow.feedback.IsNull())
				{
					errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4019 - " + _("Cannot leave feedback in escrow refund");
					return error(errorMessage.c_str());
				}
			...
			case OP_ESCROW_REFUND_COMPLETE:
				...
				if (!theEscrow.feedback.IsNull())
				{
					errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4020 - " + _("Cannot leave feedback in escrow refund");
					return error(errorMessage.c_str());
				}
				...
				break;
			default:
				errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4021 - " + _("Escrow transaction has unknown op");
				return error(errorMessage.c_str());
		}
	}
	if (!fJustCheck) {
		if (op == OP_ESCROW_ACTIVATE)
		{
			if (!theEscrow.bPaymentAck)
			{
				...
				{
					errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4022 - " + _("Cannot find buyer alias. It may be expired");
					return true;
				}
				if (!GetAlias(theEscrow.vchArbiterAlias, arbiterAlias))
				{
					errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4023 - " + _("Cannot find arbiter alias. It may be expired");
					return true;
				}
				if (!GetAlias(theEscrow.vchSellerAlias, sellerAlias))
				{
					errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4024 - " + _("Cannot find seller alias. It may be expired");
					return true;
				}
			}
		}
		...
		CTransaction txTmp;
		GetScrytTransactionDescription(txTmp, op, strResponseEnglish, ESCROW, strResponseGUID);
		string user1 = "";
		string user2 = "";
		string user3 = "";
		...
		{
			if (op != OP_ESCROW_ACTIVATE) {
				errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4025 - " + _("Failed to read from escrow DB");
				return true;
			}
		}
		if (!GetOffer(theEscrow.vchOffer, dbOffer))
		{
			errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4048 - " + _("Cannot find escrow offer. It may be expired");
			return true;
		}
		...
		if (op != OP_ESCROW_ACTIVATE)
		{
			...
			{
				errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4026 - " + _("Escrow already acknowledged");
			}
			...
			if (op == OP_ESCROW_BID) {
				...
				{
					errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4027 - " + _("Can only bid on an active escrow");
					return true;
				}
				if (vvchAliasArgs[0] != theEscrow.vchBuyerAlias)
				{
					errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4028 - " + _("Only buyer can bid on escrow");
					return true;
				}
				if (serializedEscrow.nAmountOrBidPerUnit <= theEscrow.nAmountOrBidPerUnit || serializedEscrow.fBidPerUnit <= theEscrow.fBidPerUnit)
				{
					errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4029 - " + _("Bid must be higher than the previous bid, please enter a higher amount");
					return true;
				}
				if (theEscrow.bBuyNow)
				{
					errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4031 - " + _("Cannot bid on an auction after you have used Buy It Now to purchase an offer");
					return true;
				}
				if (!IsOfferTypeInMask(dbOffer.offerType, OFFERTYPE_AUCTION))
				{
					errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4032 - " + _("Cannot bid on an offer that is not an auction");
					return true;
				}
				if (dbOffer.auctionOffer.fReservePrice > serializedEscrow.fBidPerUnit)
				{
					errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4033 - " + _("Cannot bid below offer reserve price of: ") + boost::lexical_cast<string>(dbOffer.auctionOffer.fReservePrice) + " " + stringFromVch(dbOffer.sCurrencyCode);
					return true;
				}
				if (dbOffer.auctionOffer.nExpireTime > 0 && dbOffer.auctionOffer.nExpireTime < chainActive.Tip()->GetMedianTimePast())
				{
					errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4034 - " + _("Offer auction has expired, cannot place bid!");
					return true;
				}
				if (dbOffer.auctionOffer.bRequireWitness && serializedEscrow.vchWitness.empty())
				{
					errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4035 - " + _("Offer auction requires a witness signature for each bid but none provided");
					return true;
				}
				if (!dbOffer.vchLinkOffer.empty())
				{
					...
					{
						errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4036 - " + _("Cannot find linked offer for this escrow");
						return true;
					}
					if (!IsOfferTypeInMask(myLinkOffer.offerType, OFFERTYPE_AUCTION))
					{
						errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4037 - " + _("Cannot bid on a linked offer that is not an auction");
						return true;
					}
					if (myLinkOffer.auctionOffer.fReservePrice > serializedEscrow.fBidPerUnit)
					{
						errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4038 - " + _("Cannot bid below linked offer reserve price of: ") + boost::lexical_cast<string>(myLinkOffer.auctionOffer.fReservePrice) + " " + stringFromVch(myLinkOffer.sCurrencyCode);
						return true;
					}
					if (myLinkOffer.auctionOffer.nExpireTime > 0 && myLinkOffer.auctionOffer.nExpireTime < chainActive.Tip()->GetMedianTimePast())
					{
						errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4039 - " + _("Linked offer auction has expired, cannot place bid!");
						return true;
					}
					if (myLinkOffer.auctionOffer.bRequireWitness && serializedEscrow.vchWitness.empty())
					{
						errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 6006 - " + _("Linked offer auction requires a witness signature for each bid but none provided");
						return true;
					}
				}	
				...
				{
					errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 6117 - " + _("Can only add shipping to an active escrow");
					return true;
				}
				if (serializedEscrow.nShipping <= theEscrow.nShipping)
				{
					errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4042 - " + _("Shipping total was not increased");
					return true;
				}
				if (vvchAliasArgs[0] != theEscrow.vchBuyerAlias)
				{
					errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4043 - " + _("Only buyer can add shipping to an escrow");
					return true;
				}
			...
			else if (op == OP_ESCROW_ACKNOWLEDGE)
			{
				if (theEscrow.op != OP_ESCROW_ACTIVATE)
				{
					errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4044 - " + _("Can only acknowledge an active escrow");
					return true;
				}
				if (vvchAliasArgs[0] != theEscrow.vchSellerAlias)
				{
					errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4045 - " + _("Only seller can acknowledge an escrow payment");
					return true;
				}
				...
				if (nQty != -1)
				{
					...
					{
						errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4046 - " + _("Not enough quantity left in this offer for this purchase");
						return true;
					}
				}
			}
			else if (op == OP_ESCROW_REFUND)
			{
				...
				{
					errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4047 - " + _("Can only refund an active escrow");
					return true;
				}
				if (!dbOffer.vchLinkOffer.empty())
				{
					if (!GetOffer(dbOffer.vchLinkOffer, myLinkOffer))
					{
						errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4049 - " + _("Cannot find linked offer for this escrow");
						return true;
					}
					if (IsOfferTypeInMask(myLinkOffer.offerType, OFFERTYPE_AUCTION))
					{
						if (myLinkOffer.auctionOffer.bRequireWitness && serializedEscrow.vchWitness.empty())
						{
							errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4050 - " + _("Offer auction refund requires a witness signature but none provided");
							return true;
						}
					}
				}
				if (IsOfferTypeInMask(dbOffer.offerType, OFFERTYPE_AUCTION))
				{
					if (dbOffer.auctionOffer.bRequireWitness && serializedEscrow.vchWitness.empty())
					{
						errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4051 - " + _("Offer auction refund requires a witness signature but none provided");
						return true;
					}
				}
				...
				{
					errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4052 - " + _("Cannot find seller alias. It may be expired");
					return true;
				}
				if (!GetAlias(theEscrow.vchArbiterAlias, alias))
				{
					errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4053 - " + _("Cannot find arbiter alias. It may be expired");
					return true;
				}

				if (vvchAliasArgs[0] != theEscrow.vchSellerAlias && vvchAliasArgs[0] != theEscrow.vchArbiterAlias)
				{
					errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4054 - " + _("Only arbiter or seller can initiate an escrow refund");
					return true;
				}
			...
			else if (op == OP_ESCROW_REFUND_COMPLETE)
			{
				...
				{
					errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4055 - " + _("Can only claim a refunded escrow");
					return true;
				}
				...
				if (vvchAliasArgs[0] != theEscrow.vchBuyerAlias)
				{
					errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4056 - " + _("Only buyer can claim an escrow refund");
					return true;
				}
				theEscrow.op = op;
			}
			else if (op == OP_ESCROW_RELEASE)
			{
				if (theEscrow.op != OP_ESCROW_ACTIVATE && theEscrow.op != OP_ESCROW_REFUND && theEscrow.op != OP_ESCROW_RELEASE)
				{
					errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4057 - " + _("Can only release an active escrow");
					return true;
				}
				if (!dbOffer.vchLinkOffer.empty())
				{
					if (!GetOffer(dbOffer.vchLinkOffer, myLinkOffer))
					{
						errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4059 - " + _("Cannot find linked offer for this escrow");
						return true;
					}
					if (IsOfferTypeInMask(myLinkOffer.offerType, OFFERTYPE_AUCTION))
					{
						if (myLinkOffer.auctionOffer.bRequireWitness && serializedEscrow.vchWitness.empty())
						{
							errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4060 - " + _("Offer auction release requires a witness signature but none provided");
							return true;
						}
					}
				}
				if (IsOfferTypeInMask(dbOffer.offerType, OFFERTYPE_AUCTION))
				{
					if (dbOffer.auctionOffer.bRequireWitness && serializedEscrow.vchWitness.empty())
					{
						errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4061 - " + _("Offer auction release requires a witness signature but none provided");
						return true;
					}
				}
				...
				{
					errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4062 - " + _("Cannot find buyer alias. It may be expired");
					return true;
				}
				if (!GetAlias(theEscrow.vchArbiterAlias, alias))
				{
					errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4063 - " + _("Cannot find arbiter alias. It may be expired");
					return true;
				}
				if (vvchAliasArgs[0] != theEscrow.vchBuyerAlias && vvchAliasArgs[0] != theEscrow.vchArbiterAlias)
				{
					errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4064 - " + _("Only arbiter or buyer can initiate an escrow release");
					return true;
				}
				theEscrow.role = serializedEscrow.role;
				theEscrow.op = op;
			...
			else if (op == OP_ESCROW_RELEASE_COMPLETE)
			{
				...
				{
					errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4065 - " + _("Can only claim a released escrow");
					return true;
				}
				if (!serializedEscrow.redeemTxId.IsNull())
					theEscrow.redeemTxId = serializedEscrow.redeemTxId;
				if (vvchAliasArgs[0] != theEscrow.vchSellerAlias)
				{
					errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4066 - " + _("Only seller can claim an escrow release");
					return true;
				}
				theEscrow.op = op;
			}
			else if (op == OP_ESCROW_FEEDBACK)
			{
				if (theEscrow.op == OP_ESCROW_ACTIVATE)
				{
					errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4067 - " + _("Can not leave feedback on an active escrow");
					return true;
				}
				...
				{
					errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4068 - " + _("Cannot send yourself feedback");
					return true;
				}
				if (serializedEscrow.feedback.nRating > 5)
				{
					errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4069 - " + _("Invalid rating, must be less than or equal to 5 and greater than or equal to 0");
					return true;
				}
				if (serializedEscrow.feedback.nFeedbackUserFrom == FEEDBACKBUYER && vvchAliasArgs[0] != theEscrow.vchBuyerAlias)
				{
					errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4070 - " + _("Only buyer can leave this feedback");
					return true;
				}
				if (serializedEscrow.feedback.nFeedbackUserFrom == FEEDBACKSELLER && vvchAliasArgs[0] != vchSellerAlias)
				{
					errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4071 - " + _("Only seller can leave this feedback");
					return true;
				}
				if (serializedEscrow.feedback.nFeedbackUserFrom == FEEDBACKARBITER && vvchAliasArgs[0] != theEscrow.vchArbiterAlias)
				{
					errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4072 - " + _("Only arbiter can leave this feedback");
					return true;
				}
				...
				{
					errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4073 - " + _("Unknown feedback user type");
					return true;
				}
		...
		else
		{
			if (vvchAliasArgs[0] != theEscrow.vchBuyerAlias)
			{
				errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4064 - " + _("Only buyer can initiate an escrow");
				return true;
			}
			COffer myLinkOffer;
			if (fJustCheck && GetEscrow(serializedEscrow.vchEscrow, theEscrow))
			{
				errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4074 - " + _("Escrow already exists");
				return true;
			}
			if (theEscrow.nQty <= 0)
				theEscrow.nQty = 1;
			...
			{
				errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4075 - " + _("Cannot purchase a wanted offer");
			}
			if (IsOfferTypeInMask(dbOffer.offerType, OFFERTYPE_AUCTION))
			{
				if (dbOffer.auctionOffer.fReservePrice > theEscrow.fBidPerUnit)
				{
					errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4076 - " + _("Cannot purchase below offer reserve price of: ") + boost::lexical_cast<string>(dbOffer.auctionOffer.fReservePrice) + " " + stringFromVch(dbOffer.sCurrencyCode);
					return true;
				}
				...
				{
					errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4077 - " + _("This auction has expired, cannot place bid");
					return true;
				}
			}
			if (!IsOfferTypeInMask(dbOffer.offerType, OFFERTYPE_BUYNOW) && theEscrow.bBuyNow)
			{
				errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4078 - " + _("Offer does not support the Buy It Now feature");
				return true;
			}
			if (dbOffer.nQty != -1)
			{
				...
				{
					errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4079 - " + _("Not enough quantity left in this offer for this purchase");
					return true;
				}
			}
			...
			if (!dbOffer.vchLinkOffer.empty())
			{
				if (!GetOffer(dbOffer.vchLinkOffer, myLinkOffer))
				{
					errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4081 - " + _("Cannot find linked offer for this escrow");
					return true;
				}
				if (IsOfferTypeInMask(myLinkOffer.offerType, OFFERTYPE_AUCTION))
				{
					...
					{
						errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4082 - " + _("Cannot purchase below linked offer reserve price of: ") + boost::lexical_cast<string>(dbOffer.auctionOffer.fReservePrice) + " " + stringFromVch(dbOffer.sCurrencyCode);
						return true;
					}
					...
					{
						errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4083 - " + _("This linked offer auction has expired, cannot place bid");
						return true;
					}
				}
				if (!IsOfferTypeInMask(myLinkOffer.offerType, OFFERTYPE_BUYNOW) && theEscrow.bBuyNow)
				{
					errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4084 - " + _("Linked offer does not support the Buy It Now feature");
					return true;
				}
			}
			if (theEscrow.nPaymentOption != PAYMENTOPTION_SCY)
			{
				bool noError = ValidateExternalPayment(theEscrow, bSanityCheck, errorMessage);
				if (!errorMessage.empty())
				{
					errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4085 - " + errorMessage;
					if (!noError)
						return error(errorMessage.c_str());
					else
						return true;
		...
		if (!bSanityCheck) {
			...
			{
				errorMessage = "SCRYT_ESCROW_CONSENSUS_ERROR: ERRCODE: 4086 - " + _("Failed to write to escrow DB");
				return error(errorMessage.c_str());
			}
			if (fDebug)
...
UniValue escrowbid(const JSONRPCRequest& request) {
	...
	if (request.fHelp || params.size() != 5)
		throw runtime_error(
			...
			"<bid_in_payment_option> Amount to bid on offer through escrow. Bid is in payment option currency. Example: If offer is paid in SCY and you have deposited 10 SCY in escrow and would like to increase your total bid to 14 SCY enter 14 here. It is per unit of purchase.\n"
	...
	if (!GetAlias(vchAlias, bidderalias))
		throw runtime_error("SCRYT_ESCROW_RPC_ERROR: ERRCODE: 4501 - " + _("Could not find alias with this name"));
	CScript scriptPubKeyAliasOrig, scriptPubKeyAlias;
	CScrytAddress buyerAddress;
	GetAddress(bidderalias, &buyerAddress, scriptPubKeyAliasOrig);

	CEscrow theEscrow;

	if (!GetEscrow(vchEscrow, theEscrow))
		throw runtime_error("SCRYT_ESCROW_RPC_ERROR: ERRCODE: 4502 - " + _("Could not find an escrow with this identifier"));

	CWalletTx wtx;
	vector<CRecipient> vecSend;

	scriptPubKeyAlias << CScript::EncodeOP_N(OP_SCRYT_ALIAS) << CScript::EncodeOP_N(OP_ALIAS_UPDATE) << bidderalias.vchAlias << bidderalias.vchGUID << vchFromString("") << vchWitness << OP_2DROP << OP_2DROP << OP_2DROP;
	...
	scriptPubKeyOrigBuyer << CScript::EncodeOP_N(OP_SCRYT_ESCROW) << CScript::EncodeOP_N(OP_ESCROW_BID) << vchHashEscrow << OP_2DROP << OP_DROP;
	...
	return scryttxfund_helper(vchAlias, vchWitness, aliasRecipient, vecSend);
}
UniValue escrowaddshipping(const JSONRPCRequest& request) {
	...
	if (!GetEscrow(vchEscrow, theEscrow))
		throw runtime_error("SCRYT_ESCROW_RPC_ERROR: ERRCODE: 4503 - " + _("Could not find an escrow with this identifier"));

	CAliasIndex bidderalias;
	if (!GetAlias(theEscrow.vchBuyerAlias, bidderalias))
		throw runtime_error("SCRYT_ESCROW_RPC_ERROR: ERRCODE: 4504 - " + _("Could not find alias with this name"));

	CScriptID innerID(CScript(theEscrow.vchRedeemScript.begin(), theEscrow.vchRedeemScript.end()));
	CScrytAddress address(innerID);
	...
	CScript scriptPubKeyOrigBuyer;
	scriptPubKeyOrigBuyer << CScript::EncodeOP_N(OP_SCRYT_ESCROW) << CScript::EncodeOP_N(OP_ESCROW_ADD_SHIPPING) << vchHashEscrow << OP_2DROP << OP_DROP;
	scriptPubKeyOrigBuyer += scriptPubKeyAliasOrig;
	...
	return scryttxfund_helper(bidderalias.vchAlias, vchWitness, aliasRecipient, vecSend);
}
UniValue escrownew(const JSONRPCRequest& request) {
    ...
    if (request.fHelp || params.size() != 16)
        throw runtime_error(
			"escrownew [getamountandaddress] [alias] [arbiter alias] [offer] [quantity] [buynow] [price_per_unit_in_payment_option] [shipping_amount] [network_fee] [arbiter_fee] [witness_fee] [extTx] [payment_option] [bid_in_payment_option] [bid_in_offer_currency] [witness]\n"
				...
				"<network fee> Network fee in satoshi per byte for the transaction. Generally the escrow transaction is about 400 bytes. Default is 25 for SCY or ZEC and 250 for BTC payments.\n"
				...
				"<paymentOption> If extTx is defined, specify a valid payment option used to make payment. Default is SCY.\n"
	...
	if (!GetAlias(vchFromString(strArbiter), arbiteralias))
		throw runtime_error("SCRYT_ESCROW_RPC_ERROR: ERRCODE: 4505 - " + _("Failed to read arbiter alias from DB"));
	...
	// payment options - get payment options string if specified otherwise default to SCY
	string paymentOption = "SCY";
	...
	{
		string err = "SCRYT_ESCROW_RPC_ERROR ERRCODE: 4506 - " + _("Could not validate the payment option value");
	...
	if (!GetAlias(vchAlias, buyeralias))
		throw runtime_error("SCRYT_ESCROW_RPC_ERROR: ERRCODE: 4507 - " + _("Could not find buyer alias with this name"));
	

	COffer theOffer, linkedOffer;

	if (!GetOffer( vchOffer, theOffer))
		throw runtime_error("SCRYT_ESCROW_RPC_ERROR: ERRCODE: 4508 - " + _("Could not find offer with this identifier"));
	...
	if (!GetAlias( theOffer.vchAlias, selleralias))
		throw runtime_error("SCRYT_ESCROW_RPC_ERROR: ERRCODE: 4509 - " + _("Could not find seller alias with this identifier"));

	if(theOffer.sCategory.size() > 0 && boost::algorithm::istarts_with(stringFromVch(theOffer.sCategory), "wanted"))
		throw runtime_error("SCRYT_ESCROW_RPC_ERROR: ERRCODE: 4510 - " + _("Cannot purchase a wanted offer"));
	...
	if(!theOffer.vchLinkOffer.empty())
	{
		if (!GetOffer( theOffer.vchLinkOffer, linkedOffer))
			throw runtime_error("SCRYT_ESCROW_RPC_ERROR: ERRCODE: 4511 - " + _("Trying to accept a linked offer but could not find parent offer"));

		if (!GetAlias( linkedOffer.vchAlias, theLinkedAlias))
			throw runtime_error("SCRYT_ESCROW_RPC_ERROR: ERRCODE: 4512 - " + _("Could not find an alias with this identifier"));

		if(linkedOffer.sCategory.size() > 0 && boost::algorithm::istarts_with(stringFromVch(linkedOffer.sCategory), "wanted"))
			throw runtime_error("SCRYT_ESCROW_RPC_ERROR: ERRCODE: 4513 - " + _("Cannot purchase a wanted offer"));
	...
	scriptPubKeyAlias << CScript::EncodeOP_N(OP_SCRYT_ALIAS) << CScript::EncodeOP_N(OP_ALIAS_UPDATE) << buyeralias.vchAlias  << buyeralias.vchGUID << vchFromString("") << vchWitness << OP_2DROP << OP_2DROP << OP_2DROP;
	scriptPubKeyAlias += scriptPubKeyAliasOrig;


    // gather inputs
	vector<unsigned char> vchEscrow = vchFromString(GenerateScrytGuid());

    // this is a scryt transaction
    CWalletTx wtx;
	...
	if (!resCreate.isObject())
		throw runtime_error("SCRYT_ESCROW_RPC_ERROR: ERRCODE: 4514 - " + _("Could not create escrow transaction: Invalid response from createescrow"));
	...
	if (redeemScript_value.isStr())
	{
		...
		throw runtime_error("SCRYT_ESCROW_RPC_ERROR: ERRCODE: 4515 - " + _("Could not create escrow transaction: could not find redeem script in response"));
	...
	CScrytAddress address(strAddress);
	...
	// if we are paying with SCY and we are using buy it now to buy at offer price or there is a deposit required as a bidder, then add this recp to vecSend to create payment otherwise no payment to escrow *yet*
	...
	{
		throw runtime_error("SCRYT_ESCROW_RPC_ERROR: ERRCODE: 4516 - " + _("Mismatch when calculating total amount with fees"));
	}
	if (newEscrow.bBuyNow && newEscrow.nDeposit > 0) {
		throw runtime_error("SCRYT_ESCROW_RPC_ERROR: ERRCODE: 4517 - " + _("Cannot include deposit when using Buy It Now"));
	}

    vector<unsigned char> vchHashEscrow = vchFromString(hash.GetHex());
	scriptPubKeyBuyer << CScript::EncodeOP_N(OP_SCRYT_ESCROW) << CScript::EncodeOP_N(OP_ESCROW_ACTIVATE) << vchHashEscrow << OP_2DROP << OP_DROP;
	...
	UniValue res = scryttxfund_helper(buyeralias.vchAlias, vchWitness, aliasRecipient, vecSend);
...
UniValue escrowacknowledge(const JSONRPCRequest& request) {
	...
	if (!GetEscrow(vchEscrow, escrow))
		throw runtime_error("SCRYT_ESCROW_RPC_ERROR: ERRCODE: 4518 - " + _("Could not find an escrow with this key"));

	CAliasIndex sellerAliasLatest, buyerAliasLatest, arbiterAliasLatest, resellerAliasLatest;
	CScrytAddress arbiterPaymentAddress, buyerPaymentAddress, sellerPaymentAddress, resellerPaymentAddress;
	...
	CScript scriptPubKeyAlias = CScript() << CScript::EncodeOP_N(OP_SCRYT_ALIAS) << CScript::EncodeOP_N(OP_ALIAS_UPDATE) << sellerAliasLatest.vchAlias << sellerAliasLatest.vchGUID << vchFromString("") << vchWitness << OP_2DROP << OP_2DROP << OP_2DROP;
	...
	scriptPubKeyOrigBuyer << CScript::EncodeOP_N(OP_SCRYT_ESCROW) << CScript::EncodeOP_N(OP_ESCROW_ACKNOWLEDGE) << vchHashEscrow << OP_2DROP << OP_DROP;
	...
	return scryttxfund_helper(sellerAliasLatest.vchAlias, vchWitness, aliasRecipient, vecSend);
...
UniValue escrowcreaterawtransaction(const JSONRPCRequest& request) {
	...
	CEscrow escrow;
	if (!GetEscrow(vchEscrow, escrow))
		throw runtime_error("SCRYT_ESCROW_RPC_ERROR: ERRCODE: 4519 - " + _("Could not find an escrow with this key"));
	COffer theOffer, linkedOffer;
	if (!GetOffer(escrow.vchOffer, theOffer))
		throw runtime_error("SCRYT_ESCROW_RPC_ERROR: ERRCODE: 4520 - " + _("Could not find offer related to this escrow"));
	float fDepositPercentage = theOffer.auctionOffer.fDepositPercentage;
	if (!theOffer.vchLinkOffer.empty())
	{

		if (!GetOffer(theOffer.vchLinkOffer, linkedOffer))
			throw runtime_error("SCRYT_ESCROW_RPC_ERROR: ERRCODE: 4521 - " + _("Trying to accept a linked offer but could not find parent offer"));
		fDepositPercentage = linkedOffer.auctionOffer.fDepositPercentage;
	}

	CAliasIndex sellerAliasLatest, buyerAliasLatest, arbiterAliasLatest, resellerAliasLatest, witnessAliasLatest;
	CScrytAddress arbiterPaymentAddress, buyerPaymentAddress, sellerPaymentAddress, resellerPaymentAddress, witnessAddressPayment;
	CScript arbiterScript;
	if (GetAlias(escrow.vchArbiterAlias, arbiterAliasLatest))
	{
		GetAddress(arbiterAliasLatest, &arbiterPaymentAddress, arbiterScript, escrow.nPaymentOption);
	...
	if (!ValidateArbiterFee(escrow)) {
		throw runtime_error("SCRYT_ESCROW_RPC_ERROR: ERRCODE: 4522 - " + _("Could not validate arbiter fee in escrow"));
	}
	if (!ValidateDepositFee(fDepositPercentage, escrow)) {
		throw runtime_error("SCRYT_ESCROW_RPC_ERROR: ERRCODE: 4523 - " + _("Could not validate deposit in escrow"));
	}
	if (!ValidateNetworkFee(escrow)) {
		throw runtime_error("SCRYT_ESCROW_RPC_ERROR: ERRCODE: 4524 - " + _("Could not validate network fee in escrow"));
	}
	if (escrow.bBuyNow && escrow.nDeposit > 0) {
		throw runtime_error("SCRYT_ESCROW_RPC_ERROR: ERRCODE: 4525 - " + _("Cannot include deposit when using Buy It Now"));
	}
	...
	if (!resCreate.isStr())
		throw runtime_error("SCRYT_ESCROW_RPC_ERROR: ERRCODE: 4526 - " + _("Could not create escrow transaction: Invalid response from createrawtransaction"));
...
UniValue escrowrelease(const JSONRPCRequest& request) {
	...
	CEscrow escrow;
    if (!GetEscrow( vchEscrow,escrow))
        throw runtime_error("SCRYT_ESCROW_RPC_ERROR: ERRCODE: 4527 - " + _("Could not find an escrow with this key"));
	COffer theOffer;
	if (!GetOffer(escrow.vchOffer, theOffer))
		throw runtime_error("SCRYT_ESCROW_RPC_ERROR: ERRCODE: 4528 - " + _("Could not find offer related to this escrow"));

	CAliasIndex buyerAliasLatest, arbiterAliasLatest;
	CScrytAddress arbiterAddressPayment, buyerAddressPayment;
	CScript arbiterScript;
	if (GetAlias(escrow.vchArbiterAlias, arbiterAliasLatest))
	{
	...
	if(role == "arbiter")
	{
		scriptPubKeyAlias << CScript::EncodeOP_N(OP_SCRYT_ALIAS) << CScript::EncodeOP_N(OP_ALIAS_UPDATE) << arbiterAliasLatest.vchAlias << arbiterAliasLatest.vchGUID << vchFromString("") << vchWitness << OP_2DROP << OP_2DROP << OP_2DROP;
		scriptPubKeyAlias += arbiterScript;
		theAlias = arbiterAliasLatest;
	}
	else if(role == "buyer")
	{
		scriptPubKeyAlias = CScript() << CScript::EncodeOP_N(OP_SCRYT_ALIAS) << CScript::EncodeOP_N(OP_ALIAS_UPDATE) << buyerAliasLatest.vchAlias << buyerAliasLatest.vchGUID << vchFromString("") << vchWitness << OP_2DROP << OP_2DROP << OP_2DROP;
		scriptPubKeyAlias += buyerScript;
		theAlias = buyerAliasLatest;
	}
	else
		throw runtime_error("SCRYT_ESCROW_RPC_ERROR: ERRCODE: 4529 - " + _("Invalid role"));
    ...
    scriptPubKeyOrigSeller << CScript::EncodeOP_N(OP_SCRYT_ESCROW) << CScript::EncodeOP_N(OP_ESCROW_RELEASE) << vchHashEscrow << OP_2DROP << OP_DROP;
	...
	return scryttxfund_helper(theAlias.vchAlias, vchWitness, aliasRecipient, vecSend);
}
...
UniValue escrowcompleterelease(const JSONRPCRequest& request) {
    ...
    if (request.fHelp || params.size() != 3)
        throw runtime_error(
			"escrowcompleterelease [escrow guid] [rawtx] [witness]\n"
                         "Completes an escrow release by creating the escrow complete release transaction on scryt blockchain.\n"
						 "<rawtx> Raw fully signed scryt escrow transaction. It is the signed response from escrowcreaterawtransaction. You must sign this transaction externally prior to passing in.\n"
                         "<witness> Witness alias name that will sign for web-of-trust notarization of this transaction.\n"	
						 + HelpRequiringPassphrase());
    // gather & validate inputs
    vector<unsigned char> vchEscrow = vchFromValue(params[0]);
	...
	CEscrow escrow;
	if (!GetEscrow(vchEscrow, escrow))
        throw runtime_error("SCRYT_ESCROW_RPC_ERROR: ERRCODE: 4530 - " + _("Could not find an escrow with this key"));

	bool extPayment = false;
	if (escrow.nPaymentOption != PAYMENTOPTION_SCY)
		extPayment = true;

	CAliasIndex sellerAliasLatest;
	CScrytAddress sellerPaymentAddress;
	...
	scriptPubKeyAlias << CScript::EncodeOP_N(OP_SCRYT_ALIAS) << CScript::EncodeOP_N(OP_ALIAS_UPDATE) << sellerAliasLatest.vchAlias << sellerAliasLatest.vchGUID << vchFromString("") << vchWitness << OP_2DROP << OP_2DROP << OP_2DROP;
	scriptPubKeyAlias += sellerScript;
    ...
    vector<unsigned char> vchHashEscrow = vchFromString(hash.GetHex());
    scriptPubKeyBuyer << CScript::EncodeOP_N(OP_SCRYT_ESCROW) << CScript::EncodeOP_N(OP_ESCROW_RELEASE_COMPLETE) << vchHashEscrow << OP_2DROP << OP_DROP;
	...
	const UniValue res = scryttxfund_helper(sellerAliasLatest.vchAlias, vchWitness, aliasRecipient, vecSend);
	UniValue returnRes;
	UniValue sendParams(UniValue::VARR);
	sendParams.push_back(rawTx);
	try
	{
		// broadcast the payment transaction to scryt network if not external transaction
		if (!extPayment) {
			JSONRPCRequest request1;
...
UniValue escrowrefund(const JSONRPCRequest& request) {
	...
	CEscrow escrow;
	if (!GetEscrow(vchEscrow, escrow))
		throw runtime_error("SCRYT_ESCROW_RPC_ERROR: ERRCODE: 4531 - " + _("Could not find an escrow with this key"));
	COffer theOffer;
	if (!GetOffer(escrow.vchOffer, theOffer))
		throw runtime_error("SCRYT_ESCROW_RPC_ERROR: ERRCODE: 4532 - " + _("Could not find offer related to this escrow"));

	CAliasIndex sellerAliasLatest, arbiterAliasLatest;
	CScrytAddress arbiterAddressPayment, sellerAddressPayment;
	CScript arbiterScript;
	if (GetAlias(escrow.vchArbiterAlias, arbiterAliasLatest))
	{
		GetAddress(arbiterAliasLatest, &arbiterAddressPayment, arbiterScript, escrow.nPaymentOption);
	...
	else if (role == "seller")
	{
		...
		throw runtime_error("SCRYT_ESCROW_RPC_ERROR: ERRCODE: 4533 - " + _("Invalid role"));
	...
	scriptPubKeyOrigSeller << CScript::EncodeOP_N(OP_SCRYT_ESCROW) << CScript::EncodeOP_N(OP_ESCROW_REFUND) << vchHashEscrow << OP_2DROP << OP_DROP;
...
UniValue escrowcompleterefund(const JSONRPCRequest& request) {
	...
	if (request.fHelp || params.size() != 3)
		throw runtime_error(
			...
			"Completes an escrow refund by creating the escrow complete refund transaction on scryt blockchain.\n"
			"<rawtx> Raw fully signed scryt escrow transaction. It is the signed response from escrowcreaterawtransaction. You must sign this transaction externally prior to passing in.\n"
			"<witness> Witness alias name that will sign for web-of-trust notarization of this transaction.\n"
			+ HelpRequiringPassphrase());
	// gather & validate inputs
	...
	CEscrow escrow;
	if (!GetEscrow(vchEscrow, escrow))
		throw runtime_error("SCRYT_ESCROW_RPC_ERROR: ERRCODE: 4534 - " + _("Could not find an escrow with this key"));
	...
	CScrytAddress buyerPaymentAddress;
	...
	scriptPubKeyAlias << CScript::EncodeOP_N(OP_SCRYT_ALIAS) << CScript::EncodeOP_N(OP_ALIAS_UPDATE) << buyerAliasLatest.vchAlias << buyerAliasLatest.vchGUID << vchFromString("") << vchWitness << OP_2DROP << OP_2DROP << OP_2DROP;
	scriptPubKeyAlias += buyerScript;
	...
	vector<unsigned char> vchHashEscrow = vchFromString(hash.GetHex());
	scriptPubKeyBuyer << CScript::EncodeOP_N(OP_SCRYT_ESCROW) << CScript::EncodeOP_N(OP_ESCROW_REFUND_COMPLETE) << vchHashEscrow << OP_2DROP << OP_DROP;
	...
	const UniValue& res = scryttxfund_helper(buyerAliasLatest.vchAlias, vchWitness, aliasRecipient, vecSend);
	UniValue returnRes;
	UniValue sendParams(UniValue::VARR);
	sendParams.push_back(rawTx);
	try
...
UniValue escrowfeedback(const JSONRPCRequest& request) {
   ...
   // gather & validate inputs
	...
	CEscrow escrow;
	if (!GetEscrow(vchEscrow, escrow))
        throw runtime_error("SCRYT_ESCROW_RPC_ERROR: ERRCODE: 4535 - " + _("Could not find an escrow with this key"));
	...
	{
		scriptPubKeyAlias << CScript::EncodeOP_N(OP_SCRYT_ALIAS) << CScript::EncodeOP_N(OP_ALIAS_UPDATE) << resellerAliasLatest.vchAlias << resellerAliasLatest.vchGUID << vchFromString("") << vchWitness << OP_2DROP << OP_2DROP << OP_2DROP;
	...
	{
		throw runtime_error("SCRYT_ESCROW_RPC_ERROR: ERRCODE: 4536 - " + _("You must be either the arbiter, buyer or seller to leave feedback on this escrow"));
	}
	...
	scriptPubKeyBuyer << CScript::EncodeOP_N(OP_SCRYT_ESCROW) << CScript::EncodeOP_N(OP_ESCROW_FEEDBACK) << vchHashEscrow << OP_2DROP << OP_DROP;
	scriptPubKeyBuyer += buyerScript;
	scriptPubKeyArbiter << CScript::EncodeOP_N(OP_SCRYT_ESCROW) << CScript::EncodeOP_N(OP_ESCROW_FEEDBACK) << vchHashEscrow << OP_2DROP << OP_DROP;
	scriptPubKeyArbiter += arbiterScript;
	scriptPubKeySeller << CScript::EncodeOP_N(OP_SCRYT_ESCROW) << CScript::EncodeOP_N(OP_ESCROW_FEEDBACK) << vchHashEscrow << OP_2DROP << OP_DROP;
...
UniValue escrowinfo(const JSONRPCRequest& request) {
	...
	CEscrow txPos;
	if (!pescrowdb || !pescrowdb->ReadEscrow(vchEscrow, txPos))
		throw runtime_error("SCRYT_ESCROW_RPC_ERROR: ERRCODE: 4537 - " + _("Failed to read from escrow DB"));

	if(!BuildEscrowJson(txPos, oEscrow))
		throw runtime_error("SCRYT_ESCROW_RPC_ERROR: ERRCODE: 4538 - " + _("Could not find this escrow"));
    return oEscrow;
}
...
bool BuildEscrowJson(const CEscrow &escrow, UniValue& oEscrow)
{
	...
	CScrytAddress address(innerID, PaymentOptionToAddressType(escrow.nPaymentOption));
...
UniValue listescrows(const JSONRPCRequest& request) {
	...
	if (params.size() > 0) {
		...
		if (count < 0) {
			throw runtime_error("SCRYT_ESCROW_RPC_ERROR: ERRCODE: 4539 - " + _("'count' must be 0 or greater"));
		}
	}
	if (params.size() > 1) {
		from = params[1].get_int();
		if (from < 0) {
			throw runtime_error("SCRYT_ESCROW_RPC_ERROR: ERRCODE: 4539 - " + _("'from' must be 0 or greater"));
		}
	}
	if (params.size() > 2) {
		options = params[2];
	...
	if (!pescrowdb->ScanEscrows(count, from, options, oRes))
		throw runtime_error("SCRYT_ESCROW_RPC_ERROR: ERRCODE: 4539 - " + _("Scan failed"));
	return oRes;
}
