...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = CScrytAddress(CScriptID(redeemScript)).ToString();
...
void CWallet::AvailableCoins(std::vector<COutput>& vCoins, bool fOnlyConfirmed, const CCoinControl *coinControl, bool fIncludeZeroValue, AvailableCoinsType nCoinType, bool fUseInstantSend, bool fIncludeScrytAliasBalances, bool fIncludeScrytAliasOutputs) const
{
    vCoins.clear();

    {
        ...
        for (std::map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->tx->vout.size(); i++) {
				// SCRYT
				if (!fIncludeScrytAliasBalances || !fIncludeScrytAliasOutputs) {
					if (coinControl && coinControl->HasSelected() && !coinControl->fAllowOtherInputs && !coinControl->IsSelected(COutPoint((*it).first, i)))
						continue;
					// SCRYT txs are unspendable by wallet unless using coincontrol(and the tx is selected)
					if (!coinControl || !coinControl->IsSelected(COutPoint((*it).first, i)))
					{
						...
						if (pcoin->tx->vout.size() >= i && ExtractDestination(pcoin->tx->vout[i].scriptPubKey, sysdestination))
						{
							...
							if (IsScrytScript(pcoin->tx->vout[i].scriptPubKey, op, vvchArgs) && !fIncludeScrytAliasOutputs)
								continue;
							CScrytAddress address = CScrytAddress(sysdestination);
							if (DoesAliasExist(address.ToString()) && !fIncludeScrytAliasBalances)
								continue;
						}
					}
				}
                ...
                if(nCoinType == ONLY_DENOMINATED) {
                    ...
                    int masternode_collateral = 500;
                    if (chainActive.Height() >= Params().GetConsensus().nCollateralChangeHeightv1)
                        masternode_collateral = 1000;
                    if (chainActive.Height() >= Params().GetConsensus().nCollateralChangeHeightv2)
                        masternode_collateral = 1500;
                    if (chainActive.Height() >= Params().GetConsensus().nCollateralChangeHeightv3)
                        masternode_collateral = 2000;
                    if (chainActive.Height() >= Params().GetConsensus().nCollateralChangeHeightv4)
                        masternode_collateral = 2500;
                    if (chainActive.Height() >= Params().GetConsensus().nCollateralChangeHeightv5)
                        masternode_collateral = 3000;
                    if (chainActive.Height() >= Params().GetConsensus().nCollateralChangeHeightv6)
                        masternode_collateral = 3500;
                    if (chainActive.Height() >= Params().GetConsensus().nCollateralChangeHeightv7)
                        masternode_collateral = 4000;
                    if (chainActive.Height() >= Params().GetConsensus().nCollateralChangeHeightv8)
                        masternode_collateral = 5000;
                    if (chainActive.Height() >= Params().GetConsensus().nCollateralChangeHeightv9)
                        masternode_collateral = 6000;
                    if (chainActive.Height() >= Params().GetConsensus().nCollateralChangeHeightv10)
                        masternode_collateral = 8000;
...
bool CWallet::SelectCoinsByDenominations(int nDenom, CAmount nValueMin, CAmount nValueMax, std::vector<CTxDSIn>& vecTxDSInRet, std::vector<COutput>& vCoinsRet, CAmount& nValueRet, int nPrivateSendRoundsMin, int nPrivateSendRoundsMax)
{
	...
	// SCRYT include scryt alias balances
...
bool CWallet::SelectCoinsGrouppedByAddresses(std::vector<CompactTallyItem>& vecTallyRet, bool fSkipDenominated, bool fAnonymizable, bool fSkipUnconfirmed) const
{
    ...
    if (LogAcceptCategory("selectcoins")) {
        ...
        for (const auto& item : vecTallyRet)
            strMessage += strprintf("  %s %f\n", CScrytAddress(item.txdest).ToString().c_str(), float(item.nAmount)/COIN);
...
bool CWallet::SelectCoinsDark(CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& vecTxInRet, CAmount& nValueRet, int nPrivateSendRoundsMin, int nPrivateSendRoundsMax) const
{
    ...
    for (const auto& out : vCoins)
    {
        ...
        if (nHeight >= Params().GetConsensus().nCollateralChangeHeightv1)
            masternode_collateral = 1000;
        if (nHeight >= Params().GetConsensus().nCollateralChangeHeightv2)
            masternode_collateral = 1500;
        if (nHeight >= Params().GetConsensus().nCollateralChangeHeightv3)
            masternode_collateral = 2000;
        if (nHeight >= Params().GetConsensus().nCollateralChangeHeightv4)
            masternode_collateral = 2500;
        if (nHeight >= Params().GetConsensus().nCollateralChangeHeightv5)
            masternode_collateral = 3000;
        if (nHeight >= Params().GetConsensus().nCollateralChangeHeightv6)
            masternode_collateral = 3500;
        if (nHeight >= Params().GetConsensus().nCollateralChangeHeightv7)
            masternode_collateral = 4000;
        if (nHeight >= Params().GetConsensus().nCollateralChangeHeightv8)
            masternode_collateral = 5000;
        if (nHeight >= Params().GetConsensus().nCollateralChangeHeightv9)
            masternode_collateral = 6000;
        if (nHeight >= Params().GetConsensus().nCollateralChangeHeightv10)
            masternode_collateral = 8000;
...
bool CWallet::GetMasternodeOutpointAndKeys(COutPoint& outpointRet, CPubKey& pubKeyRet, CKey& keyRet, const std::string& strTxHash, const std::string& strOutputIndex)
{
	...
	// SCRYT include sys alias balances
...
bool CWallet::GetOutpointAndKeysFromOutput(const COutput& out, COutPoint& outpointRet, CPubKey& pubKeyRet, CKey& keyRet)
{
    ...
    CScrytAddress address2(address1);
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                if (fUseInstantSend && nValueIn > sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE)*COIN) {
                    strFailReason += " " + strprintf(_("InstantSend doesn't support sending values that high yet. Transactions are currently limited to %1 SCY."), sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE));
                    return false;
                }
                ...
                if (nChange > 0)
                {
                    ...
                    if (nCoinType == ONLY_DENOMINATED) {
                        ...
                        // change transaction isn't always pay-to-scryt-address
                        CScript scriptChange;
...
bool CWallet::SetAddressBook(const CTxDestination& address, const std::string& strName, const std::string& strPurpose)
{
    ...
    if (!fFileBacked)
        return false;
    if (!strPurpose.empty() && !CWalletDB(strWalletFile).WritePurpose(CScrytAddress(address).ToString(), strPurpose))
        return false;
    return CWalletDB(strWalletFile).WriteName(CScrytAddress(address).ToString(), strName);
}
...
{
    {
        ...
        if(fFileBacked)
        {
            ...
            std::string strAddress = CScrytAddress(address).ToString();
    ...
    if (!fFileBacked)
        return false;
    CWalletDB(strWalletFile).ErasePurpose(CScrytAddress(address).ToString());
    return CWalletDB(strWalletFile).EraseName(CScrytAddress(address).ToString());
}
...
bool CWallet::AddDestData(const CTxDestination &dest, const std::string &key, const std::string &value)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).WriteDestData(CScrytAddress(dest).ToString(), key, value);
}
...
bool CWallet::EraseDestData(const CTxDestination &dest, const std::string &key)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).EraseDestData(CScrytAddress(dest).ToString(), key);
}
