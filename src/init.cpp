 ...
 * between scrytd, and scryt-qt and non-server tools.
*/
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), SCRYT_CONF_FILENAME));
    if (mode == HMM_SCRYTD)
    {
#if HAVE_DECL_DAEMON
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), SCRYT_PID_FILENAME));
#endif
    ...
    std::string debugCategories = "addrman, alert, bench, cmpctblock, coindb, db, http, leveldb, libevent, lock, mempool, mempoolrej, net, proxy, prune, rand, reindex, rpc, selectcoins, tor, zmq, "
                                  "scryt (or specifically: gobject, instantsend, keepass, masternode, mnpayments, mnsync, privatesend, spork)"; // Don't translate these and qt below
    if (mode == HMM_SCRYT_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Scryt specific functionality (Masternodes, PrivateSend, InstantSend, Governance) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-privatesendamount=<n>", strprintf(_("Keep N SCY anonymized (%u-%u, default: %u)"), MIN_PRIVATESEND_AMOUNT, MAX_PRIVATESEND_AMOUNT, DEFAULT_PRIVATESEND_AMOUNT));
...
std::string LicenseInfo()
{
	...
	return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
		"\n" +
		FormatParagraph(strprintf(_("Copyright (C) 2020-%i The ScrytExchange developers"), COPYRIGHT_YEAR)) + "\n" +
		"\n" +
		...
		"\n" +
		FormatParagraph(_("Scryt is open source software produced by a global network of developers. By downloading, distributing and using Scryt and the Scryt network you release the developers involved in the Scryt Project past, present, and future from any and all liability. You are responsible for your creations on the Scryt network. You agree that the developers of the Scryt Project carry no responsibility for the actions/data or entities of *any* definition created on the network by yourself or others on the network to which you may be exposed.")) + "\n" +
		"\n" +
		FormatParagraph(_("The developers of the Scryt Project do not have the power to modify data on the Scryt network, it is backed by an immutable blockchain, which you further acknowledge through use of Scryt, the Scryt network, and Scryt services. If you do not agree to these terms, please refrain from using Scryt and its related services.")) +
		"\n";
}
...
/** Sanity checks
 *  Ensure that Scryt Core is running in a usable environment with all
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Scryt Core version %s\n", FormatFullVersion());
}
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single Scryt Core process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(GetArg("-conf", SCRYT_CONF_FILENAME)).string());
    ...
    while (!fLoaded && !fRequestShutdown) {
        ...
        do {
            try {
				...
				// SCRYT Check for changed -addressindex state
    ...
    //lite mode disables all Scryt-specific functionality
    ...
    if((!fLiteMode && fTxIndex == false)
       && chainparams.NetworkIDString() != CBaseChainParams::REGTEST) { // TODO remove this when pruning is fixed. See https://github.com/scrytpay/scryt/pull/1817 and https://github.com/scrytpay/scryt/pull/1743
    ...
    if(fMasternodeMode) {
        ...
        if(!strMasterNodePrivKey.empty()) {
            ...
            LogPrintf("  pubKeyMasternode: %s\n", CScrytAddress(activeMasternode.pubKeyMasternode.GetID()).ToString());
        } else {
    ...
    // ********************************************************* Step 11c: update block tip in Scryt modules
    ...
    // ********************************************************* Step 11d: schedule Scryt-specific tasks
