...
bool ValidatePaymentOptionsMask(const uint64_t &paymentOptionsMask) {
	uint64_t maxVal = PAYMENTOPTION_SCY | PAYMENTOPTION_BTC | PAYMENTOPTION_ZEC;
	return !(paymentOptionsMask < 1 || paymentOptionsMask > maxVal);
}

bool IsValidPaymentOption(const uint64_t &paymentOptionsMask) {
	return (paymentOptionsMask == PAYMENTOPTION_SCY || paymentOptionsMask == PAYMENTOPTION_BTC || paymentOptionsMask == PAYMENTOPTION_ZEC);
}
...
bool ValidatePaymentOptionsString(const std::string &paymentOptionsString) {
	...
	for (size_t i = 0; i < strs.size(); i++) {
		if(strs[i].compare("BTC") != 0 && strs[i].compare("SCY") != 0 && strs[i].compare("ZEC") != 0) {
			retval = false;
			break;
		}
	}
...
uint64_t GetPaymentOptionsMaskFromString(const std::string &paymentOptionsString) {
	...
	boost::split(strs, paymentOptionsString, boost::is_any_of("+"));
	for (size_t i = 0; i < strs.size(); i++) {
		if(!strs[i].compare("SCY")) {
			retval |= PAYMENTOPTION_SCY;
		}
...
bool RevertOffer(const std::vector<unsigned char>& vchOffer, const int op, const uint256 &txHash, sorted_vector<std::vector<unsigned char> > &revertedOffers) {
	...
	if (!pofferdb->ReadLastOffer(vchOffer, dbOffer)) {
		...
		{
			errorMessage = "SCRYT_OFFER_CONSENSUS_ERROR: ERRCODE: 2022 - " + _("Failed to erase offer");
	...
	{
		errorMessage = "SCRYT_OFFER_CONSENSUS_ERROR: ERRCODE: 2022 - " + _("Failed to write to offer DB");
		return error(errorMessage.c_str());
	}
...
bool CheckOfferInputs(const CTransaction &tx, int op, const vector<vector<unsigned char> > &vvchArgs, const std::vector<unsigned char> &vvchAlias, bool fJustCheck, int nHeight, sorted_vector<std::vector<unsigned char> > &revertedOffers, string &errorMessage, bool bSanityCheck) {
	...
	int nDataOut;
	if(!GetScrytData(tx, vchData, vchHash, nDataOut) || !theOffer.UnserializeFromData(vchData, vchHash))
	{
		errorMessage = "SCRYT_OFFER_CONSENSUS_ERROR ERRCODE: 1000 - " + _("Cannot unserialize data inside of this transaction relating to an offer");
		return true;
	}
	...
	if(fJustCheck)
	{
		if(vvchArgs.size() != 1)
		{
			errorMessage = "SCRYT_OFFER_CONSENSUS_ERROR: ERRCODE: 1002 - " + _("Offer arguments incorrect size");
			return error(errorMessage.c_str());
		}
		
		if(vchHash != vvchArgs[0])
		{
			errorMessage = "SCRYT_OFFER_CONSENSUS_ERROR: ERRCODE: 1005 - " + _("Hash provided doesn't match the calculated hash of the data");
			return true;
		}
	}
	...
	if(fJustCheck)
	{
		...
		{
			errorMessage = "SCRYT_OFFER_CONSENSUS_ERROR: ERRCODE: 1007 - " + _("Offer description too long");
			return error(errorMessage.c_str());
		}
		if(theOffer.sTitle.size() > MAX_NAME_LENGTH)
		{
			errorMessage = "SCRYT_OFFER_CONSENSUS_ERROR: ERRCODE: 1008 - " + _("Offer title too long");
			return error(errorMessage.c_str());
		}
		if(theOffer.sCategory.size() > MAX_NAME_LENGTH)
		{
			errorMessage = "SCRYT_OFFER_CONSENSUS_ERROR: ERRCODE: 1009 - " + _("Offer category too long");
			return error(errorMessage.c_str());
		}
		if (theOffer.fPrice <= 0)
		{
			errorMessage = "SCRYT_OFFER_CONSENSUS_ERROR: ERRCODE: 1009 - " + _("Offer price must be a positive number");
			return error(errorMessage.c_str());
		}
		if(theOffer.vchLinkOffer.size() > MAX_GUID_LENGTH)
		{
			errorMessage = "SCRYT_OFFER_CONSENSUS_ERROR: ERRCODE: 1010 - " + _("Offer link guid hash too long");
			return error(errorMessage.c_str());
		}
		if(theOffer.sCurrencyCode.size() > MAX_GUID_LENGTH)
		{
			errorMessage = "SCRYT_OFFER_CONSENSUS_ERROR: ERRCODE: 1011 - " + _("Offer currency code too long");
			return error(errorMessage.c_str());
		}
		switch (op) {
		case OP_OFFER_ACTIVATE:
			...
			{
				errorMessage = "SCRYT_OFFER_CONSENSUS_ERROR: ERRCODE: 2004 - " + _("offer hex guid too long");
				return error(errorMessage.c_str());
			}
			if(!ValidatePaymentOptionsMask(theOffer.paymentOptions))
			{
				errorMessage = "SCRYT_OFFER_CONSENSUS_ERROR: ERRCODE: 1016 - " + _("Invalid payment option");
				return error(errorMessage.c_str());
			}
			if (!ValidateOfferTypeMask(theOffer.offerType))
			{
				errorMessage = "SCRYT_OFFER_CONSENSUS_ERROR: ERRCODE: 1016 - " + _("Invalid offer type");
				return error(errorMessage.c_str());
			}
			
			if(theOffer.nCommission > 100 || theOffer.nCommission < -90)
			{
				errorMessage = "SCRYT_OFFER_CONSENSUS_ERROR: ERRCODE: 1019 - " + _("Commission percentage must be between -90 and 100");
				return error(errorMessage.c_str());
			}		
			if(theOffer.vchLinkOffer.empty())
			{
				...
				{
					errorMessage = "SCRYT_OFFER_CONSENSUS_ERROR: ERRCODE: 1020 - " + _("Offer category cannot be empty");
					return error(errorMessage.c_str());
				}
				if(theOffer.sTitle.size() < 1)
				{
					errorMessage = "SCRYT_OFFER_CONSENSUS_ERROR: ERRCODE: 1021 - " + _("Offer title cannot be empty");
					return error(errorMessage.c_str());
				}
				if(theOffer.paymentOptions <= 0)
				{
					errorMessage = "SCRYT_OFFER_CONSENSUS_ERROR: ERRCODE: 1022 - " + _("Invalid payment option specified");
					return error(errorMessage.c_str());
				}
			}
			if(theOffer.nQty < -1)
			{
				errorMessage = "SCRYT_OFFER_CONSENSUS_ERROR: ERRCODE: 1023 - " + _("Quantity must be greater than or equal to -1");
				return error(errorMessage.c_str());
			}
			if(!theOffer.vchCert.empty() && theOffer.nQty != 1)
			{
				errorMessage = "SCRYT_OFFER_CONSENSUS_ERROR: ERRCODE: 1024 - " + _("Quantity must be 1 for a digital offer");
				return error(errorMessage.c_str());
			}
			...
			if (IsOfferTypeInMask(theOffer.offerType, OFFERTYPE_AUCTION))
			{
				...
				{
					errorMessage = "SCRYT_OFFER_CONSENSUS_ERROR: ERRCODE: 1024 - " + _("Reserve price must be greator or equal to 0");
					return error(errorMessage.c_str());
				}
				if (theOffer.auctionOffer.fDepositPercentage < 0)
				{
					errorMessage = "SCRYT_OFFER_CONSENSUS_ERROR: ERRCODE: 1024 - " + _("Auction deposit percentage must be greator or equal to 0");
					return error(errorMessage.c_str());
				}
				if (theOffer.auctionOffer.nExpireTime > 0 && theOffer.auctionOffer.nExpireTime < nTime)
				{
					errorMessage = "SCRYT_OFFER_CONSENSUS_ERROR: ERRCODE: 4042 - " + _("Invalid auction expiry");
					return error(errorMessage.c_str());
				}
			}
			break;
		case OP_OFFER_UPDATE:		
			...
			{
				errorMessage = "SCRYT_OFFER_CONSENSUS_ERROR: ERRCODE: 1027 - " + _("Invalid payment option");
				return error(errorMessage.c_str());
			}
			if (!ValidateOfferTypeMask(theOffer.offerType))
			{
				errorMessage = "SCRYT_OFFER_CONSENSUS_ERROR: ERRCODE: 1027 - " + _("Invalid offer type");
				return error(errorMessage.c_str());
			}

			if(theOffer.nQty < -1)
			{
				errorMessage = "SCRYT_OFFER_CONSENSUS_ERROR: ERRCODE: 1031 - " + _("Quantity must be greater than or equal to -1");
				return error(errorMessage.c_str());
			}
			if(!theOffer.vchCert.empty() && theOffer.nQty != 1)
			{
				errorMessage = "SCRYT_OFFER_CONSENSUS_ERROR: ERRCODE: 1032 - " + _("Quantity must be 1 for a digital offer");
				return error(errorMessage.c_str());
			}
			if(theOffer.nCommission > 100 || theOffer.nCommission < -90)
			{
				errorMessage = "SCRYT_OFFER_CONSENSUS_ERROR: ERRCODE: 1034 - " + _("Commission percentage must be between -90 and 100");
				return error(errorMessage.c_str());
			}
			break;
		default:
			errorMessage = "SCRYT_OFFER_CONSENSUS_ERROR: ERRCODE: 1047 - " + _("Offer transaction has unknown op");
			return error(errorMessage.c_str());
		}
	}
	if (!fJustCheck && !bSanityCheck) {
		...
		{
			errorMessage = "SCRYT_OFFER_CONSENSUS_ERROR: ERRCODE: 2028 - " + _("Failed to revert offer");
			return error(errorMessage.c_str());
		}
	}
	...
	CTransaction txTmp;
	GetScrytTransactionDescription(txTmp, op, strResponseEnglish, OFFER, strResponseGUID);
	COffer dbOffer;
	// load the offer data from the DB
	if (!GetOffer(theOffer.vchOffer, dbOffer))
	{
		if (op == OP_OFFER_UPDATE) {
			errorMessage = "SCRYT_OFFER_CONSENSUS_ERROR: ERRCODE: 1048 - " + _("Failed to read from offer DB");
			return true;
		}
	}
	if (op == OP_OFFER_UPDATE) {
		...
		{
			errorMessage = "SCRYT_OFFER_CONSENSUS_ERROR: ERRCODE: 2026 - " + _("Cannot edit this offer. Offer owner must sign off on this change.");
			return true;
		}
		...
		if (IsOfferTypeInMask(dbOffer.offerType, OFFERTYPE_AUCTION))
		{
			if (!IsOfferTypeInMask(theOffer.offerType, OFFERTYPE_AUCTION))
			{
				errorMessage = "SCRYT_OFFER_CONSENSUS_ERROR: ERRCODE: 1024 - " + _("Cannot change to this offer type until auction expires");
				return true;
			}
			if (theOffer.auctionOffer.fReservePrice < 0)
			{
			...
			{
				errorMessage = "SCRYT_OFFER_CONSENSUS_ERROR: ERRCODE: 1024 - " + _("Cannot modify auction parameters while it is active. Please wait until auction has expired before updating.");
				return true;
			}
		}
	...
	else if(op == OP_OFFER_ACTIVATE)
	{
		...
		{
			errorMessage = "SCRYT_OFFER_CONSENSUS_ERROR: ERRCODE: 2026 - " + _("Cannot create this offer. Offer owner must sign off on this change.");
			return true;
		}
		COfferLinkWhitelistEntry entry;
		if (GetOffer(theOffer.vchOffer, theOffer))
		{
			errorMessage = "SCRYT_OFFER_CONSENSUS_ERROR: ERRCODE: 1050 - " + _("Offer already exists");
			return true;
		}
		...
		if (!theOffer.vchLinkOffer.empty())
		{
			...
			{
				errorMessage = "SCRYT_OFFER_CONSENSUS_ERROR: ERRCODE: 1051 - " + _("Linked offer not found. It may be expired");
				return true;
			}
			if (!GetAlias(linkOffer.vchAlias, theAlias))
			{
				errorMessage = "SCRYT_OFFER_CONSENSUS_ERROR: ERRCODE: 1051 - " + _("Linked offer alias not found. It may be expired");
				return true;
			}
			if (theAlias.offerWhitelist.GetLinkEntryByHash(vvchAlias, entry))
			{
				...
				{
					throw runtime_error("SCRYT_OFFER_CONSENSUS_ERROR: ERRCODE: 1514 - " + _("This resold offer must be of higher price than the original offer including any discount"));
				}
			}
			// make sure alias exists in the root offer affiliate list
			else
			{
				throw runtime_error("SCRYT_OFFER_CONSENSUS_ERROR: ERRCODE: 1515 - " + _("Cannot find this alias in the parent offer affiliate list"));
			}

			if (!linkOffer.vchLinkOffer.empty())
			{
				throw runtime_error("SCRYT_OFFER_CONSENSUS_ERROR: ERRCODE: 1516 - " + _("Cannot link to an offer that is already linked to another offer"));
			}
			else if (linkOffer.sCategory.size() > 0 && boost::algorithm::istarts_with(stringFromVch(linkOffer.sCategory), "wanted"))
			{
				throw runtime_error("SCRYT_OFFER_CONSENSUS_ERROR: ERRCODE: 1517 - " + _("Cannot link to a wanted offer"));
			}
	...
	if(op == OP_OFFER_UPDATE)
	{
		...
		if(!theOffer.vchLinkOffer.empty())
		{
			if (!GetOffer(theOffer.vchLinkOffer, linkOffer))
			{
				errorMessage = "SCRYT_OFFER_CONSENSUS_ERROR: ERRCODE: 1096 - " + _("Linked offer not found. It may be expired");
				return true;
			}
			theOffer.nQty = linkOffer.nQty;
			theOffer.vchCert = linkOffer.vchCert;
	...
	if (!bSanityCheck) {
		...
		{
			errorMessage = "SCRYT_OFFER_CONSENSUS_ERROR: ERRCODE: 1096 - " + _("Failed to write to offer DB");
			return error(errorMessage.c_str());
		}
...
UniValue offernew(const JSONRPCRequest& request) {
	...
	if (request.fHelp || params.size() != 17)
		throw runtime_error(
			"offernew [alias] [category] [title] [quantity] [price] [description] [currency] [cert. guid] [payment options=SCY] [private=false] [units=1] [offertype=BUYNOW] [auction_expires=0] [auction_reserve=0] [auction_require_witness=false] [auction_deposit=0] [witness]\n"
						...
						"<paymentOptions> 'SCY' to accept SCY only, 'BTC' for BTC only, 'ZEC' for zcash only, or a |-delimited string to accept multiple currencies (e.g. 'BTC|SCY' to accept BTC or SCY). Leave empty for default. Defaults to 'SCY'.\n"		
	...
	if (!GetAlias(vchAlias, alias))
		throw runtime_error("SCRYT_OFFER_RPC_ERROR ERRCODE: 1500 - " + _("Could not find an alias with this name"));
	...
	{
		string err = "SCRYT_OFFER_RPC_ERROR ERRCODE: 1504 - " + _("Could not validate the payment options value");
	...
	{
		string err = "SCRYT_OFFER_RPC_ERROR ERRCODE: 1504 - " + _("Could not validate offer type");
		throw runtime_error(err.c_str());
	}
	...
	if(!vchCert.empty())
	{
		...
		{
			throw runtime_error("SCRYT_OFFER_RPC_ERROR: ERRCODE: 1506 - " + _("Creating an offer with a cert that does not exist"));
		}
		else if(!boost::algorithm::istarts_with(stringFromVch(vchCategory), "certificates"))
		{
			throw runtime_error("SCRYT_OFFER_RPC_ERROR: ERRCODE: 1507 - " + _("Offer selling a certificate must use a certificate category"));
		}
		else if(theCert.vchAlias != vchAlias)
		{
			throw runtime_error("SCRYT_OFFER_RPC_ERROR: ERRCODE: 1508 - " + _("Cannot create this offer because the certificate alias does not match the offer alias"));
		}
	}
	else if(boost::algorithm::istarts_with(stringFromVch(vchCategory), "certificates"))
	{
		throw runtime_error("SCRYT_OFFER_RPC_ERROR: ERRCODE: 1509 - " + _("Offer not selling a certificate cannot use a certificate category"));
	}
	...
	vector<unsigned char> vchOffer = vchFromString(GenerateScrytGuid());
	...
	scriptPubKey << CScript::EncodeOP_N(OP_SCRYT_OFFER) << CScript::EncodeOP_N(OP_OFFER_ACTIVATE) << vchHashOffer << OP_2DROP << OP_DROP;
	scriptPubKey += scriptPubKeyOrig;
	CScript scriptPubKeyAlias;
	scriptPubKeyAlias << CScript::EncodeOP_N(OP_SCRYT_ALIAS) << CScript::EncodeOP_N(OP_ALIAS_UPDATE) << alias.vchAlias << alias.vchGUID << vchFromString("") << vchWitness << OP_2DROP << OP_2DROP << OP_2DROP;
	...
	UniValue res = scryttxfund_helper(vchAlias, vchWitness, aliasRecipient, vecSend);
...
UniValue offerlink(const JSONRPCRequest& request) {
	...
	ToLowerCase(vchAlias);
	if (!GetAlias(vchAlias, alias))
		throw runtime_error("SCRYT_OFFER_RPC_ERROR ERRCODE: 1510 - " + _("Could not find an alias with this name"));
	...
	if (vchLinkOffer.empty() || !GetOffer( vchLinkOffer, linkOffer))
		throw runtime_error("SCRYT_OFFER_RPC_ERROR ERRCODE: 1513 - " + _("Could not find offer with this guid"));
...
UniValue offerupdate(const JSONRPCRequest& request) {
	...
	{
		throw runtime_error("SCRYT_OFFER_RPC_ERROR ERRCODE: 1532 - " + _("Could not validate payment options string"));
	}
	uint64_t paymentOptionsMask = GetPaymentOptionsMaskFromString(paymentOptions);
	...
	if (!GetOffer( vchOffer, theOffer))
		throw runtime_error("SCRYT_OFFER_RPC_ERROR ERRCODE: 1534 - " + _("Could not find offer with this guid"));
	...
	if (!fUnitTest) {
		...
		for (auto& arrivalTime : arrivalTimes) {
			...
			if ((nNow - (arrivalTime.second / 1000)) < ZDAG_MINIMUM_LATENCY_SECONDS) {
				throw runtime_error("SCRYT_OFFER_RPC_ERROR: ERRCODE: 2510 - " + _("Please wait a few more seconds and try again..."));
			}
		}
	}

	if (!GetAlias(theOffer.vchAlias, offerAlias))
		throw runtime_error("SCRYT_OFFER_RPC_ERROR ERRCODE: 1535 - " + _("Could not find an alias with this name"));
	ToLowerCase(vchAlias);
	if (!GetAlias(vchAlias, alias))
		throw runtime_error("SCRYT_OFFER_RPC_ERROR ERRCODE: 1536 - " + _("Could not find an alias with this name"));
	...
	if(!strCert.empty())
	{
		...
		{
			throw runtime_error("SCRYT_OFFER_RPC_ERROR: ERRCODE: 1538 - " + _("Updating an offer with a cert that does not exist"));
		}
		else if(theOffer.vchLinkOffer.empty() && theCert.vchAlias != theOffer.vchAlias)
		{
			throw runtime_error("SCRYT_OFFER_RPC_ERROR: ERRCODE: 1539 - " + _("Cannot update this offer because the certificate alias does not match the offer alias"));
		}
		if(!boost::algorithm::istarts_with(strCategory, "certificates"))
		{
			throw runtime_error("SCRYT_OFFER_RPC_ERROR: ERRCODE: 1540 - " + _("Offer selling a certificate must use a certificate category"));
		}
	}
	else if(boost::algorithm::istarts_with(strCategory, "certificates"))
	{
		throw runtime_error("SCRYT_OFFER_RPC_ERROR: ERRCODE: 1541 - " + _("Offer not selling a certificate cannot use a certificate category"));
	}
	if(!theOffer.vchLinkOffer.empty())
	{
		COffer linkOffer;
		if (!GetOffer( theOffer.vchLinkOffer, linkOffer))
		{
			throw runtime_error("SCRYT_OFFER_RPC_ERROR: ERRCODE: 1542 - " + _("Linked offer not found. It may be expired"));
		}
		if (!linkOffer.vchLinkOffer.empty())
		{
			throw runtime_error("SCRYT_OFFER_RPC_ERROR: ERRCODE: 1544 - " + _("Cannot link to an offer that is already linked to another offer"));
		}
		else if(linkOffer.sCategory.size() > 0 && boost::algorithm::istarts_with(stringFromVch(linkOffer.sCategory), "wanted"))
		{
			throw runtime_error("SCRYT_OFFER_RPC_ERROR: ERRCODE: 1545 - " + _("Cannot link to a wanted offer"));
		}
	}
	...
	{
		throw runtime_error("SCRYT_OFFER_RPC_ERROR: ERRCODE: 1546 - " + _("Cannot change category to wanted"));
	}
	CScrytAddress aliasAddress;
	GetAddress(alias, &aliasAddress, scriptPubKeyOrig);
    ...
    vector<unsigned char> vchHashOffer = vchFromString(hash.GetHex());
	scriptPubKey << CScript::EncodeOP_N(OP_SCRYT_OFFER) << CScript::EncodeOP_N(OP_OFFER_UPDATE) << vchHashOffer << OP_2DROP << OP_DROP;
	scriptPubKey += scriptPubKeyOrig;
	...
	vecSend.push_back(recipient);
	CScript scriptPubKeyAlias;
	scriptPubKeyAlias << CScript::EncodeOP_N(OP_SCRYT_ALIAS) << CScript::EncodeOP_N(OP_ALIAS_UPDATE) << offerAlias.vchAlias << offerAlias.vchGUID << vchFromString("") << vchWitness << OP_2DROP << OP_2DROP << OP_2DROP;
	scriptPubKeyAlias += scriptPubKeyOrig;
	CRecipient aliasRecipient;
	CreateAliasRecipient(scriptPubKeyAlias, aliasRecipient);
	...
	return scryttxfund_helper(offerAlias.vchAlias, vchWitness, aliasRecipient, vecSend);
}
...
void COfferDB::WriteOfferIndex(const COffer& offer, const int &op) {
	if (IsArgSet("-zmqpubofferrecord")) {
		...
		if (BuildOfferIndexerJson(offer, oName)) {
			GetMainSignals().NotifyScrytUpdate(oName.write().c_str(), "offerrecord");
		}
	}
...
void COfferDB::WriteOfferIndexHistory(const COffer& offer, const int &op) {
	if (IsArgSet("-zmqpubofferhistory")) {
		...
		if (BuildOfferIndexerHistoryJson(offer, oName)) {
			...
			GetMainSignals().NotifyScrytUpdate(oName.write().c_str(), "offerhistory");
		}
	}
}
UniValue offerinfo(const JSONRPCRequest& request) {
	...
	if (!pofferdb || !pofferdb->ReadOffer(vchOffer, txPos))
		throw runtime_error("SCRYT_OFFER_RPC_ERROR: ERRCODE: 5536 - " + _("Failed to read from offer DB"));

	if(!BuildOfferJson(txPos, oOffer))
		throw runtime_error("SCRYT_OFFER_RPC_ERROR ERRCODE: 5537 - " + _("Could not find this offer"));
...
std::string GetPaymentOptionsString(const uint64_t &paymentOptions)
{
	...
	if (IsPaymentOptionInMask(paymentOptions, PAYMENTOPTION_SCY)) {
		currencies.push_back(std::string("SCY"));
	}
...
{
	if (paymentOption == PAYMENTOPTION_SCY)
		return CChainParams::ADDRESS_SCY;
...
UniValue listoffers(const JSONRPCRequest& request) {
	...
	if (params.size() > 0) {
		...
		if (count < 0) {
			throw runtime_error("SCRYT_OFFER_RPC_ERROR: ERRCODE: 5538 - " + _("'count' must be 0 or greater"));
		}
	}
	if (params.size() > 1) {
		from = params[1].get_int();
		if (from < 0) {
			throw runtime_error("SCRYT_OFFER_RPC_ERROR: ERRCODE: 5538 - " + _("'from' must be 0 or greater"));
		}
	}
	if (params.size() > 2) {
		options = params[2];
	...
	if (!pofferdb->ScanOffers(count, from, options, oRes))
		throw runtime_error("SCRYT_OFFER_RPC_ERROR: ERRCODE: 5538 - " + _("Scan failed"));
	return oRes;
}
