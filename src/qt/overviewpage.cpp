...
public:
    TxViewDelegate(const PlatformStyle *_platformStyle, QObject *parent=nullptr):
        QAbstractItemDelegate(), unit(ScrytUnits::SCY),
                      ...
                      const QModelIndex &index ) const
    {
        ...
        painter->setPen(foreground);
        QString amountText = ScrytUnits::floorWithUnit(unit, amount, true, ScrytUnits::separatorAlways);
        if(!confirmed)
        {
...
void OverviewPage::setBalance(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance, const CAmount& anonymizedBalance, const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    ui->labelBalance->setText(ScrytUnits::floorHtmlWithUnit(nDisplayUnit, balance, false, ScrytUnits::separatorAlways));
    ui->labelUnconfirmed->setText(ScrytUnits::floorHtmlWithUnit(nDisplayUnit, unconfirmedBalance, false, ScrytUnits::separatorAlways));
    ui->labelImmature->setText(ScrytUnits::floorHtmlWithUnit(nDisplayUnit, immatureBalance, false, ScrytUnits::separatorAlways));
    ui->labelAnonymized->setText(ScrytUnits::floorHtmlWithUnit(nDisplayUnit, anonymizedBalance, false, ScrytUnits::separatorAlways));
    ui->labelTotal->setText(ScrytUnits::floorHtmlWithUnit(nDisplayUnit, balance + unconfirmedBalance + immatureBalance, false, ScrytUnits::separatorAlways));
    ui->labelWatchAvailable->setText(ScrytUnits::floorHtmlWithUnit(nDisplayUnit, watchOnlyBalance, false, ScrytUnits::separatorAlways));
    ui->labelWatchPending->setText(ScrytUnits::floorHtmlWithUnit(nDisplayUnit, watchUnconfBalance, false, ScrytUnits::separatorAlways));
    ui->labelWatchImmature->setText(ScrytUnits::floorHtmlWithUnit(nDisplayUnit, watchImmatureBalance, false, ScrytUnits::separatorAlways));
    ui->labelWatchTotal->setText(ScrytUnits::floorHtmlWithUnit(nDisplayUnit, watchOnlyBalance + watchUnconfBalance + watchImmatureBalance, false, ScrytUnits::separatorAlways));
...
void OverviewPage::setWalletModel(WalletModel *model)
{
    ...
    {
        // update the display unit, to not use the default ("SCY")
        updateDisplayUnit();
...
void OverviewPage::updatePrivateSendProgress()
{
    ...
    QString strPrivateSendAmount = ScrytUnits::formatHtmlWithUnit(nDisplayUnit, privateSendClient.nPrivateSendAmount * COIN, false, ScrytUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strPrivateSendAmount = strPrivateSendAmount.remove(strPrivateSendAmount.indexOf("."), ScrytUnits::decimals(nDisplayUnit) + 1);
    ...
    if(nMaxToAnonymize >= privateSendClient.nPrivateSendAmount * COIN) {
        ...
        QString strMaxToAnonymize = ScrytUnits::formatHtmlWithUnit(nDisplayUnit, nMaxToAnonymize, false, ScrytUnits::separatorAlways);
        ui->labelAmountRounds->setToolTip(tr("Not enough compatible inputs to anonymize <span style='color:red;'>%1</span>,<br>"
                                             "will anonymize <span style='color:red;'>%2</span> instead")
                                          .arg(strPrivateSendAmount)
                                          .arg(strMaxToAnonymize));
        strMaxToAnonymize = strMaxToAnonymize.remove(strMaxToAnonymize.indexOf("."), ScrytUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = "<span style='color:red;'>" +
                QString(ScrytUnits::factor(nDisplayUnit) == 1 ? "" : "~") + strMaxToAnonymize +
...
void OverviewPage::togglePrivateSend(){
    ...
    if(!privateSendClient.fEnablePrivateSend){
        ...
        if(currentBalance < nMinAmount){
            QString strMinAmount(ScrytUnits::formatWithUnit(nDisplayUnit, nMinAmount));
            QMessageBox::warning(this, tr("PrivateSend"),
