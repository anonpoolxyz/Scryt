...
const std::string ScrytGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString ScrytGUI::DEFAULT_WALLET = "~Default";

ScrytGUI::ScrytGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...
void ScrytGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Scryt address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and scryt: URIs)"));
    ...
    aboutAction->setStatusTip(tr("Show information about Scryt Core"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Scryt addresses to prove you own them"));
    verifyMessageAction = new QAction(QIcon(":/icons/" + theme + "/transaction_0"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Scryt addresses"));
    ...
    openAction->setStatusTip(tr("Open a scryt: URI or payment request"));

    showHelpMessageAction = new QAction(QApplication::style()->standardIcon(QStyle::SP_MessageBoxInformation), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Scryt command-line options").arg(tr(PACKAGE_NAME)));
...
void ScrytGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool ScrytGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool ScrytGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void ScrytGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void ScrytGUI::setWalletActionsEnabled(bool enabled)
{
...
void ScrytGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
...
void ScrytGUI::createIconMenu(QMenu *pmenu)
{
...
#ifndef Q_OS_MAC
void ScrytGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void ScrytGUI::showInfo()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_INFO);
    showDebugWindow();
}

void ScrytGUI::showConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void ScrytGUI::showGraph()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_GRAPH);
    showDebugWindow();
}

void ScrytGUI::showPeers()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_PEERS);
    showDebugWindow();
}

void ScrytGUI::showRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showDebugWindow();
}

void ScrytGUI::showConfEditor()
{
    GUIUtil::openConfigfile();
}

void ScrytGUI::showMNConfEditor()
{
    GUIUtil::openMNConfigfile();
}

void ScrytGUI::showBackups()
{
    GUIUtil::showBackups();
}

void ScrytGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

void ScrytGUI::showPrivateSendHelpClicked()
{
    if(!clientModel)
        return;
...
void ScrytGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void ScrytGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void ScrytGUI::gotoMasternodePage()
{
    QSettings settings;
    if (!fLiteMode && settings.value("fShowMasternodesTab").toBool() && masternodeAction) {
...
void ScrytGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void ScrytGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void ScrytGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void ScrytGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
#endif // ENABLE_WALLET

void ScrytGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Scryt network", "", count));
    } else {
...
void ScrytGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void ScrytGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void ScrytGUI::updateHeadersSyncProgressLabel()
{
...
void ScrytGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void ScrytGUI::setAdditionalDataSyncProgress(double nSyncProgress)
{
    if(!clientModel)
        return;
...
void ScrytGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Scryt Core"); // default title
...
void ScrytGUI::changeEvent(QEvent *e)
{
...
void ScrytGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void ScrytGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void ScrytGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(ScrytUnits::formatWithUnit(unit, amount, true)) +
                  tr("Type: %1\n").arg(type);
    if (!label.isEmpty())
        msg += tr("Label: %1\n").arg(label);
...
void ScrytGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void ScrytGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool ScrytGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool ScrytGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void ScrytGUI::setHDStatus(int hdEnabled)
{
    QString theme = GUIUtil::getThemeName();
...
void ScrytGUI::setEncryptionStatus(int status)
{
    QString theme = GUIUtil::getThemeName();
    switch(status)
    {
...
void ScrytGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void ScrytGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void ScrytGUI::detectShutdown()
{
...
void ScrytGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void ScrytGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void ScrytGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(ScrytGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void ScrytGUI::subscribeToCoreSignals()
{
...
void ScrytGUI::unsubscribeFromCoreSignals()
{
...
void ScrytGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void ScrytGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<ScrytUnits::Unit> units = ScrytUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    Q_FOREACH (const ScrytUnits::Unit unit, units)
    {
        max_width = qMax(max_width, fm.width(ScrytUnits::name(unit)));
    }
...
{
    menu = new QMenu(this);
    Q_FOREACH(ScrytUnits::Unit u, ScrytUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(ScrytUnits::name(u)), this);
...
{
    setText(ScrytUnits::name(newUnits));
}
