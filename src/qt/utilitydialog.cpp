    ...
    ui(new Ui::HelpMessageDialog)
{
    ...
    if (helpMode == about)
    {
        ...
        QString header = tr("Usage:") + "\n" +
            "  scryt-qt [" + tr("command-line options") + "]                     " + "\n";
        ...
        std::string strUsage = HelpMessage(HMM_SCRYT_QT);
        ...
        if (showDebug) {
            strUsage += HelpMessageOpt("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", ScrytGUI::DEFAULT_UIPLATFORM));
        }
...
All the Scryt in your wallet is comprised of different \"inputs\" which you can think of as separate, discrete coins.<br> \
...
These denominations are 0.1 SCY, 1 SCY, 10 SCY and 100 SCY -- sort of like the paper money you use every day.</li> \
...
For more info see <a href=\"https://scrytpay.atlassian.net/wiki/display/DOC/PrivateSend\">https://scrytpay.atlassian.net/wiki/display/DOC/PrivateSend</a> \
        "));
        ui->aboutMessage->setWordWrap(true);
        ui->helpMessage->setVisible(false);
        ui->aboutLogo->setVisible(false);
...
QWidget *ShutdownWindow::showShutdownWindow(ScrytGUI *window)
{
    if (!window)
        return nullptr;
