    ...
    platformStyle(_platformStyle)
{
    ...
    // see https://github.com/scryt/scryt/issues/5716
...
{
    GUIUtil::setClipboard(ScrytUnits::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    int nDisplayUnit = ScrytUnits::SCY;
    ...
    l2->setText(ScrytUnits::formatWithUnit(nDisplayUnit, nAmount));        // Amount
    l3->setText(ScrytUnits::formatWithUnit(nDisplayUnit, nPayFee));        // Fee
    l4->setText(ScrytUnits::formatWithUnit(nDisplayUnit, nAfterFee));      // After Fee
    l5->setText(((nBytes > 0) ? ASYMP_UTF8 : "") + QString::number(nBytes));        // Bytes
    l7->setText(fDust ? tr("yes") : tr("no"));                               // Dust
    l8->setText(ScrytUnits::formatWithUnit(nDisplayUnit, nChange));        // Change
    if (nPayFee > 0 && (coinControl->nMinimumTotalFee < nPayFee))
    {
...
void CoinControlDialog::updateView()
{
    ...
    BOOST_FOREACH(const PAIRTYPE(QString, std::vector<COutput>)& coins, mapCoins) {
        ...
        BOOST_FOREACH(const COutput& out, coins.second) {
            ...
            {
                sAddress = QString::fromStdString(CScrytAddress(outputAddress).ToString());

                // if listMode or change => show scryt address. In tree mode, address is not shown again for direct wallet address outputs
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, ScrytUnits::format(nDisplayUnit, out.tx->tx->vout[out.i].nValue));
            itemOutput->setToolTip(COLUMN_AMOUNT, ScrytUnits::format(nDisplayUnit, out.tx->tx->vout[out.i].nValue));
        ...
        if (treeMode)
        {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, ScrytUnits::format(nDisplayUnit, nSum));
            itemWalletAddress->setToolTip(COLUMN_AMOUNT, ScrytUnits::format(nDisplayUnit, nSum));
