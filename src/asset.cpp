// Copyright (c) 2017-2018 The ScrytExchange developers
...
void CAssetDB::WriteAssetIndex(const CAsset& asset, const int& op) {
	if (IsArgSet("-zmqpubassetrecord")) {
		...
		if (BuildAssetIndexerJson(asset, oName)) {
			GetMainSignals().NotifyScrytUpdate(oName.write().c_str(), "assetrecord");
		}
	}
...
void CAssetDB::WriteAssetIndexHistory(const CAsset& asset, const int &op) {
	if (IsArgSet("-zmqpubassethistory")) {
		...
		if (BuildAssetIndexerHistoryJson(asset, oName)) {
			...
			GetMainSignals().NotifyScrytUpdate(oName.write().c_str(), "assethistory");
		}
	}
}
...
bool CheckAssetInputs(const CTransaction &tx, const CCoinsViewCache &inputs, int op, const vector<vector<unsigned char> > &vvchArgs, const vector<unsigned char> &vchAlias,
	...
	vector<unsigned char> vchHash;
	int nDataOut;
	if(!GetScrytData(tx, vchData, vchHash, nDataOut) || (op != OP_ASSET_SEND &&!theAsset.UnserializeFromData(vchData, vchHash)) || (op == OP_ASSET_SEND && !theAssetAllocation.UnserializeFromData(vchData, vchHash)))
	{
		errorMessage = "SCRYT_ASSET_CONSENSUS_ERROR ERRCODE: 2000 - " + _("Cannot unserialize data inside of this transaction relating to an asset");
		return true;
	}
	...
	if(fJustCheck)
	{
		if(vvchArgs.size() != 1)
		{
			errorMessage = "SCRYT_ASSET_CONSENSUS_ERROR: ERRCODE: 2001 - " + _("Asset arguments incorrect size");
		...
		{
			errorMessage = "SCRYT_ASSET_CONSENSUS_ERROR: ERRCODE: 2002 - " + _("Hash provided doesn't match the calculated hash of the data");
			return true;
		}
	...
	if(fJustCheck)
	{
		if (op != OP_ASSET_SEND) {
			...
			{
				errorMessage = "SCRYT_ASSET_CONSENSUS_ERROR: ERRCODE: 2003 - " + _("Asset category too big");
				return error(errorMessage.c_str());
			}
			if (theAsset.vchPubData.size() > MAX_VALUE_LENGTH)
			{
				errorMessage = "SCRYT_ASSET_CONSENSUS_ERROR: ERRCODE: 2004 - " + _("Asset public data too big");
				return error(errorMessage.c_str());
			}
		}
		switch (op) {
		case OP_ASSET_ACTIVATE:
			...
			{
				errorMessage = "SCRYT_ASSET_CONSENSUS_ERROR: ERRCODE: 2005 - " + _("asset guid too long");
				return error(errorMessage.c_str());
			}
			if (theAsset.vchSymbol.size() < MIN_SYMBOL_LENGTH || theAsset.vchSymbol.size() > MAX_SYMBOL_LENGTH)
			{
				errorMessage = "SCRYT_ASSET_CONSENSUS_ERROR: ERRCODE: 2006 - " + _("asset symbol must be between 1 and 8 characters in length");
				return error(errorMessage.c_str());
			}
			if(!boost::algorithm::starts_with(stringFromVch(theAsset.sCategory), "assets"))
			{
				errorMessage = "SCRYT_ASSET_CONSENSUS_ERROR: ERRCODE: 2007 - " + _("Must use a asset category");
				return error(errorMessage.c_str());
			}
			if(theAsset.bUseInputRanges && theAsset.listAllocationInputs.empty())
			{
				errorMessage = "SCRYT_ASSET_CONSENSUS_ERROR: ERRCODE: 2008 - " + _("Must specify input range");
				return error(errorMessage.c_str());
			}
			if (!theAsset.bUseInputRanges && !theAsset.listAllocationInputs.empty())
			{
				errorMessage = "SCRYT_ASSET_CONSENSUS_ERROR: ERRCODE: 2009 - " + _("Cannot specify input range for this asset");
				return error(errorMessage.c_str());
			}

			if (theAsset.fInterestRate < 0 || theAsset.fInterestRate > 1)
			{
				errorMessage = "SCRYT_ASSET_CONSENSUS_ERROR: ERRCODE: 2010 - " + _("Interest must be between 0 and 1");
				return error(errorMessage.c_str());
			}
			if ((theAsset.fInterestRate != 0 || theAsset.bCanAdjustInterestRate) && theAsset.bUseInputRanges)
			{
				errorMessage = "SCRYT_ASSET_CONSENSUS_ERROR: ERRCODE: 2011 - " + _("Interest cannot be set on this type of asset");
				return error(errorMessage.c_str());
			}
			if (!AssetRange(theAsset.nBalance, theAsset.nPrecision, theAsset.bUseInputRanges))
			{
				errorMessage = "SCRYT_ASSET_CONSENSUS_ERROR: ERRCODE: 2012 - " + _("Initial balance out of money range");
				return true;
			}
			if (theAsset.nPrecision > 8)
			{
				errorMessage = "SCRYT_ASSET_CONSENSUS_ERROR: ERRCODE: 2013 - " + _("Precision must be between 0 and 8");
				return true;
			}
			if (theAsset.nMaxSupply != -1 && !AssetRange(theAsset.nMaxSupply, theAsset.nPrecision, theAsset.bUseInputRanges))
			{
				errorMessage = "SCRYT_ASSET_CONSENSUS_ERROR: ERRCODE: 2014 - " + _("Max supply out of money range");
				return true;
			}
			if (theAsset.nBalance > theAsset.nMaxSupply)
			{
				errorMessage = "SCRYT_ASSET_CONSENSUS_ERROR: ERRCODE: 2015 - " + _("Total supply cannot exceed maximum supply");
				return true;
			}
			if(nHeight < Params().GetConsensus().nShareFeeBlock && CScrytAddress(stringFromVch(theAsset.vchAliasOrAddress)).IsValid())
			{
				errorMessage = "SCRYT_ASSET_CONSENSUS_ERROR: ERRCODE: 2015 - " + _("Please wait until the fork to create this type of asset. It is on block: ") + boost::lexical_cast<string>(Params().GetConsensus().nShareFeeBlock);
				return true;
			}
			break;
		...
		case OP_ASSET_UPDATE:
			...
			{
				errorMessage = "SCRYT_ASSET_CONSENSUS_ERROR: ERRCODE: 2016 - " + _("Must use a asset category");
				return error(errorMessage.c_str());
			}
			if (theAsset.nBalance < 0)
			{
				errorMessage = "SCRYT_ASSET_CONSENSUS_ERROR: ERRCODE: 2017 - " + _("Balance must be greator than or equal to 0");
				return error(errorMessage.c_str());
			}
			if (theAsset.fInterestRate < 0 || theAsset.fInterestRate > 1)
			{
				errorMessage = "SCRYT_ASSET_CONSENSUS_ERROR: ERRCODE: 2018 - " + _("Interest must be between 0 and 1");
				return error(errorMessage.c_str());
			}
			break;
		...
		case OP_ASSET_TRANSFER:
			...
			{
				errorMessage = "SCRYT_ASSET_CONSENSUS_ERROR: ERRCODE: 2019 - " + _("Cannot transfer input allocations");
				return error(errorMessage.c_str());
			}
			break;
		case OP_ASSET_SEND:
			...
			{
				errorMessage = "SCRYT_ASSET_CONSENSUS_ERROR: ERRCODE: 2020 - " + _("Asset send must send an input or transfer balance");
				return error(errorMessage.c_str());
			}
			if (theAssetAllocation.listSendingAllocationInputs.size() > 250 || theAssetAllocation.listSendingAllocationAmounts.size() > 250)
			{
				errorMessage = "SCRYT_ASSET_CONSENSUS_ERROR: ERRCODE: 2021 - " + _("Too many receivers in one allocation send, maximum of 250 is allowed at once");
				return error(errorMessage.c_str());
			}
			if (theAssetAllocation.vchMemo.size() > MAX_MEMO_LENGTH)
			{
				errorMessage = "SCRYT_ASSET_CONSENSUS_ERROR: ERRCODE: 2022 - " + _("memo too long, must be 128 character or less");
				return error(errorMessage.c_str());
			}
			break;
		default:
			errorMessage = "SCRYT_ASSET_CONSENSUS_ERROR: ERRCODE: 2023 - " + _("Asset transaction has unknown op");
			return error(errorMessage.c_str());
		}
	}
	if (!fJustCheck) {
		...
		CTransaction txTmp;
		GetScrytTransactionDescription(txTmp, op, strResponseEnglish, ASSET, strResponseGUID);
		CAsset dbAsset;
		if (!GetAsset(op == OP_ASSET_SEND ? theAssetAllocation.vchAsset : theAsset.vchAsset, dbAsset))
		{
			if (op != OP_ASSET_ACTIVATE) {
				errorMessage = "SCRYT_ASSET_CONSENSUS_ERROR: ERRCODE: 2024 - " + _("Failed to read from asset DB");
				return true;
			}
		}
		...
		if (op == OP_ASSET_UPDATE) {
			if (vchAlias.empty()) {
				...
				{
					errorMessage = "SCRYT_ASSET_CONSENSUS_ERROR: ERRCODE: 1015 - " + _("Cannot update this asset. Asset owner must sign off on this change");
					return true;
				}
			}
			...
			if (!theAsset.listAllocationInputs.empty()) {
				...
				{
					errorMessage = "SCRYT_ASSET_CONSENSUS_ERROR: ERRCODE: 2026 - " + _("This asset does not use input ranges");
					return true;
				}
				...
				for (auto&input : theAsset.listAllocationInputs) {
					...
					{
						errorMessage = "SCRYT_ASSET_CONSENSUS_ERROR: ERRCODE: 2027 - " + _("Cannot edit this asset. New asset inputs must be added to the end of the supply: ") + boost::lexical_cast<std::string>(input.start) + " vs " + boost::lexical_cast<std::string>(dbAsset.nTotalSupply);
						return true;
					}
				}
				...
				{
					errorMessage = "SCRYT_ASSET_CONSENSUS_ERROR: ERRCODE: 2028 - " + _("Invalid input ranges");
					return true;
				}
			...
			{
				errorMessage = "SCRYT_ASSET_CONSENSUS_ERROR: ERRCODE: 2029 - " + _("Total supply out of money range");
				return true;
			}
			if (theAsset.nTotalSupply > dbAsset.nMaxSupply)
			{
				errorMessage = "SCRYT_ASSET_CONSENSUS_ERROR: ERRCODE: 2030 - " + _("Total supply cannot exceed maximum supply");
				return true;
			}
		...
		if (op == OP_ASSET_SEND) {
			...
			if (!vchAlias.empty() && CScrytAddress(user1).IsValid()) {
				errorMessage = "SCRYT_ASSET_CONSENSUS_ERROR: ERRCODE: 1016 - " + _("This asset cannot be sent because owner is an alias but the alias is also a valid scryt address");
				return true;
			}
			if (vchAlias.empty()) {
				if (dbAsset.vchAliasOrAddress != theAssetAllocation.vchAliasOrAddress || !FindAssetOwnerInTx(inputs, tx, user1))
				{
					errorMessage = "SCRYT_ASSET_CONSENSUS_ERROR: ERRCODE: 1015 - " + _("Cannot send this asset. Asset owner must sign off on this change");
					return true;
				}
			}
			else if (dbAsset.vchAliasOrAddress != vchAlias) {
			...
			if (!theAssetAllocation.listSendingAllocationAmounts.empty()) {
				if (dbAsset.bUseInputRanges) {
					errorMessage = "SCRYT_ASSET_CONSENSUS_ERROR: ERRCODE: 2031 - " + _("Invalid asset send, request to send amounts but asset uses input ranges");
					return true;
				}
				...
				for (auto& amountTuple : theAssetAllocation.listSendingAllocationAmounts) {
					...
					{
						errorMessage = "SCRYT_ASSET_CONSENSUS_ERROR: ERRCODE: 2032 - " + _("Receiving amount must be positive");
						return true;
					}
				}
				if (theAsset.nBalance < nTotal) {
					errorMessage = "SCRYT_ASSET_CONSENSUS_ERROR: ERRCODE: 2033 - " + _("Sender balance is insufficient");
					return true;
				}
				for (auto& amountTuple : theAssetAllocation.listSendingAllocationAmounts) {
					if (!bSanityCheck) {
						...
						{
							errorMessage = "SCRYT_ASSET_CONSENSUS_ERROR: ERRCODE: 2034 - " + _("Failed to write to asset allocation DB");
							continue;
						}
					}
				}
				...
				if (!dbAsset.bUseInputRanges) {
					errorMessage = "SCRYT_ASSET_CONSENSUS_ERROR: ERRCODE: 2035 - " + _("Invalid asset send, request to send input ranges but asset uses amounts");
					return true;
				}
				// check balance is sufficient on sender
				CAmount nTotal = 0;
				...
				for (auto& inputTuple : theAssetAllocation.listSendingAllocationInputs) {
					...
					if (rangeTotal == 0)
					{
						errorMessage = "SCRYT_ASSET_CONSENSUS_ERROR: ERRCODE: 2036 - " + _("Invalid input range");
						return true;
					}
				...
				if (theAsset.nBalance < nTotal) {
					errorMessage = "SCRYT_ASSET_CONSENSUS_ERROR: ERRCODE: 2037 - " + _("Sender balance is insufficient");
					return true;
				}
				for (unsigned int i = 0; i < theAssetAllocation.listSendingAllocationInputs.size(); i++) {
					...
					{
						errorMessage = "SCRYT_ASSET_CONSENSUS_ERROR: ERRCODE: 2038 - " + _("Input not found");
						return true;
					}
					if (!bSanityCheck) {						
						...
						{
							errorMessage = "SCRYT_ASSET_CONSENSUS_ERROR: ERRCODE: 2039 - " + _("Failed to write to asset allocation DB");
							return error(errorMessage.c_str());
						}
					}
				}
		...
		else if (op != OP_ASSET_ACTIVATE)
		{
			...
			if (op == OP_ASSET_UPDATE) {
				if (!theAsset.bCanAdjustInterestRate && theAsset.fInterestRate != dbAsset.fInterestRate) {
					errorMessage = "SCRYT_ASSET_CONSENSUS_ERROR: ERRCODE: 2040 - " + _("Cannot adjust interest rate for this asset");
					return true;
				}
			}
			if (op == OP_ASSET_TRANSFER)
		...
		if (op == OP_ASSET_ACTIVATE)
		{
			if (vchAlias.empty()) {
				if (!FindAssetOwnerInTx(inputs, tx, user1))
				{
					errorMessage = "SCRYT_ASSET_CONSENSUS_ERROR: ERRCODE: 1015 - " + _("Cannot create this asset. Asset owner must sign off on this change");
					return true;
				}
			}
			...
			{
				errorMessage = "SCRYT_ASSET_CONSENSUS_ERROR: ERRCODE: 2041 - " + _("Asset already exists");
				return true;
			}
		...
		if (!bSanityCheck) {
			...
			{
				errorMessage = "SCRYT_ASSET_CONSENSUS_ERROR: ERRCODE: 2042 - " + _("Failed to write to asset DB");
				return error(errorMessage.c_str());
			}
			// debug
			if (fDebug)
...
UniValue assetnew(const JSONRPCRequest& request) {
	...
	const CScrytAddress address(strAliasOrAddress);
	if (address.IsValid()) {
	...
	if (!strAddressFrom.empty()) {
		...
		if (!GetAlias(vchAliasOrAddress, fromAlias))
			throw runtime_error("SCRYT_ASSET_ALLOCATION_RPC_ERROR: ERRCODE: 1502 - " + _("Failed to read sender alias from DB"));
		CScrytAddress fromAddr;
	...
	newAsset.vchAsset = vchFromString(GenerateScrytGuid());
    ...
    scriptPubKey << CScript::EncodeOP_N(OP_SCRYT_ASSET) << CScript::EncodeOP_N(OP_ASSET_ACTIVATE) << vchHashAsset << OP_2DROP << OP_DROP;
	...
	if (strAddressFrom.empty()) {
		...
		scriptPubKeyAlias << CScript::EncodeOP_N(OP_SCRYT_ALIAS) << CScript::EncodeOP_N(OP_ALIAS_UPDATE) << fromAlias.vchAlias << fromAlias.vchGUID << vchFromString("") << vchWitness << OP_2DROP << OP_2DROP << OP_2DROP;
	...
	UniValue res = scryttxfund_helper(vchAliasOrAddress, vchWitness, aliasRecipient, vecSend);
...
UniValue assetupdate(const JSONRPCRequest& request) {
    ...
    if (!GetAsset( vchAsset, theAsset))
        throw runtime_error("SCRYT_ASSET_RPC_ERROR: ERRCODE: 2501 - " + _("Could not find a asset with this key"));
    ...
    vector<unsigned char> vchHashAsset = vchFromString(hash.GetHex());
    scriptPubKey << CScript::EncodeOP_N(OP_SCRYT_ASSET) << CScript::EncodeOP_N(OP_ASSET_UPDATE) << vchHashAsset << OP_2DROP << OP_DROP;
    scriptPubKey += scriptPubKeyFromOrig;
	...
	return scryttxfund_helper(vchFromString(strAliasOrAddress), vchWitness, aliasRecipient, vecSend);
}
...
UniValue assettransfer(const JSONRPCRequest& request) {
 ...
 if (request.fHelp || params.size() != 3)
	...
	CAsset theAsset;
    if (!GetAsset( vchAsset, theAsset))
        throw runtime_error("SCRYT_ASSET_RPC_ERROR: ERRCODE: 2505 - " + _("Could not find a asset with this key"));
	
	string strAddressFrom;
	const string& strAliasOrAddress = stringFromVch(theAsset.vchAliasOrAddress);
	const CScrytAddress addressFrom(strAliasOrAddress);
	if (addressFrom.IsValid()) {
		strAddressFrom = strAliasOrAddress;
	}


	const CScrytAddress addressTo(stringFromVch(vchAliasOrAddressTo));
	if (addressTo.IsValid()) {
		...
		if (!GetAlias(vchAliasOrAddressTo, toAlias))
			throw runtime_error("SCRYT_ASSET_ALLOCATION_RPC_ERROR: ERRCODE: 1502 - " + _("Failed to read receiving alias from DB"));
		CScrytAddress toAddr;
    ...
    vector<unsigned char> vchHashAsset = vchFromString(hash.GetHex());
    scriptPubKey << CScript::EncodeOP_N(OP_SCRYT_ASSET) << CScript::EncodeOP_N(OP_ASSET_TRANSFER) << vchHashAsset << OP_2DROP << OP_DROP;
...
UniValue assetsend(const JSONRPCRequest& request) {
	...
	CAsset theAsset;
	if (!GetAsset(vchAsset, theAsset))
		throw runtime_error("SCRYT_ASSET_RPC_ERROR: ERRCODE: 2507 - " + _("Could not find a asset with this key"));
	...
	for (unsigned int idx = 0; idx < receivers.size(); idx++) {
		...
		if (!CScrytAddress(stringFromVch(vchAliasOrAddressTo)).IsValid()) {
			ToLowerCase(vchAliasOrAddressTo);
			if (!GetAlias(vchAliasOrAddressTo, toAlias))
				throw runtime_error("SCRYT_ASSET_ALLOCATION_RPC_ERROR: ERRCODE: 1501 - " + _("Failed to read recipient alias from DB"));
		}
	...
	if (!fUnitTest) {
		...
		for (auto& arrivalTime : arrivalTimes) {
			...
			if ((nNow - (arrivalTime.second / 1000)) < ZDAG_MINIMUM_LATENCY_SECONDS) {
				throw runtime_error("SCRYT_ASSET_RPC_ERROR: ERRCODE: 2509 - " + _("Please wait a few more seconds and try again..."));
			}
		}
	}
	if (assetAllocationConflicts.find(assetAllocationTuple) != assetAllocationConflicts.end())
		throw runtime_error("SCRYT_ASSET_RPC_ERROR: ERRCODE: 2510 - " + _("This asset allocation is involved in a conflict which must be resolved with Proof-Of-Work. Please wait for a block confirmation and try again..."));
	...
	if(!theAssetAllocation.UnserializeFromData(data, vchHashAsset))
		throw runtime_error("SCRYT_ASSET_RPC_ERROR: ERRCODE: 2510 - " + _("Could not unserialize asset allocation data"));

	scriptPubKey << CScript::EncodeOP_N(OP_SCRYT_ASSET) << CScript::EncodeOP_N(OP_ASSET_SEND) << vchHashAsset << OP_2DROP << OP_DROP;
	scriptPubKey += scriptPubKeyFromOrig;
	// send the asset pay txn
	vector<CRecipient> vecSend;
	CRecipient recipient;
...
UniValue assetinfo(const JSONRPCRequest& request) {
	...
	CAsset txPos;
	if (!passetdb || !passetdb->ReadAsset(vchAsset, txPos))
		throw runtime_error("SCRYT_ASSET_RPC_ERROR: ERRCODE: 2511 - " + _("Failed to read from asset DB"));
...
UniValue listassets(const JSONRPCRequest& request) {
	...
	if (params.size() > 0) {
		...
		if (count < 0) {
			throw runtime_error("SCRYT_ASSET_RPC_ERROR: ERRCODE: 2512 - " + _("'count' must be 0 or greater"));
		}
	}
	if (params.size() > 1) {
		from = params[1].get_int();
		if (from < 0) {
			throw runtime_error("SCRYT_ASSET_RPC_ERROR: ERRCODE: 2512 - " + _("'from' must be 0 or greater"));
		}
	}
	if (params.size() > 2) {
		options = params[2];
	...
	if (!passetdb->ScanAssets(count, from, options, oRes))
		throw runtime_error("SCRYT_ASSET_RPC_ERROR: ERRCODE: 2512 - " + _("Scan failed"));
	return oRes;
}
