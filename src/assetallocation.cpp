...
void CAssetAllocationDB::WriteAssetAllocationIndex(const CAssetAllocation& assetallocation, const CAsset& asset, const CAmount& nSenderBalance, const CAmount& nAmount, const std::string& strSender, const std::string& strReceiver) {
	if (IsArgSet("-zmqpubassetallocation") || fAssetAllocationIndex) {
		...
		if (BuildAssetAllocationIndexerJson(assetallocation, asset, nSenderBalance, nAmount, strSender, strReceiver, isMine, oName)) {
			...
			GetMainSignals().NotifyScrytUpdate(strObj.c_str(), "assetallocation");
...
bool DecodeAssetAllocationScript(const CScript& script, int& op,
	...
	if (op != OP_SCRYT_ASSET_ALLOCATION)
		return false;
...
bool RevertAssetAllocation(const CAssetAllocationTuple &assetAllocationToRemove, const CAsset &asset, const uint256 &txHash, const int& nHeight, sorted_vector<CAssetAllocationTuple> &revertedAssetAllocations) {
	...
	{
		errorMessage = "SCRYT_ASSET_ALLOCATION_CONSENSUS_ERROR: ERRCODE: 1000 - " + _("Failed to write to asset allocation DB");
...
bool CheckAssetAllocationInputs(const CTransaction &tx, const CCoinsViewCache &inputs, int op, const vector<vector<unsigned char> > &vvchArgs, const vector<unsigned char> &vchAlias,
	...
	vector<unsigned char> vchHash;
	int nDataOut;
	if(!GetScrytData(tx, vchData, vchHash, nDataOut) || !theAssetAllocation.UnserializeFromData(vchData, vchHash))
	{
		errorMessage = "SCRYT_ASSET_ALLOCATION_CONSENSUS_ERROR ERRCODE: 1001 - " + _("Cannot unserialize data inside of this transaction relating to an assetallocation");
		return true;
	}
	...
	if(fJustCheck)
	{
		if(vvchArgs.size() != 1)
		{
			errorMessage = "SCRYT_ASSET_ALLOCATION_CONSENSUS_ERROR: ERRCODE: 1002 - " + _("Asset arguments incorrect size");
			return error(errorMessage.c_str());
		}		
		if(vchHash != vvchArgs[0])
		{
			errorMessage = "SCRYT_ASSET_ALLOCATION_CONSENSUS_ERROR: ERRCODE: 1003 - " + _("Hash provided doesn't match the calculated hash of the data");
			return true;
		}		
	}
	string retError = "";
	if(fJustCheck)
	{
		...
		case OP_ASSET_ALLOCATION_SEND:
			...
			{
				errorMessage = "SCRYT_ASSET_ALLOCATION_CONSENSUS_ERROR: ERRCODE: 1004 - " + _("Asset send must send an input or transfer balance");
				return error(errorMessage.c_str());
			}
			if (theAssetAllocation.listSendingAllocationInputs.size() > 250 || theAssetAllocation.listSendingAllocationAmounts.size() > 250)
			{
				errorMessage = "SCRYT_ASSET_ALLOCATION_CONSENSUS_ERROR: ERRCODE: 1005 - " + _("Too many receivers in one allocation send, maximum of 250 is allowed at once");
				return error(errorMessage.c_str());
			}
			if (theAssetAllocation.vchMemo.size() > MAX_MEMO_LENGTH)
			{
				errorMessage = "SCRYT_ASSET_ALLOCATION_CONSENSUS_ERROR: ERRCODE: 1006 - " + _("memo too long, must be 128 character or less");
				return error(errorMessage.c_str());
			}
			break;
		case OP_ASSET_COLLECT_INTEREST:
			if (!theAssetAllocation.listSendingAllocationInputs.empty() || !theAssetAllocation.listSendingAllocationAmounts.empty())
			{
				errorMessage = "SCRYT_ASSET_ALLOCATION_CONSENSUS_ERROR: ERRCODE: 1007 - " + _("Cannot send tokens in an interest collection transaction");
				return error(errorMessage.c_str());
			}
			if (theAssetAllocation.vchMemo.size() > 0)
			{
				errorMessage = "SCRYT_ASSET_ALLOCATION_CONSENSUS_ERROR: ERRCODE: 1008 - " + _("Cannot send memo when collecting interest");
				return error(errorMessage.c_str());
			}
			break;
		default:
			errorMessage = "SCRYT_ASSET_ALLOCATION_CONSENSUS_ERROR: ERRCODE: 1009 - " + _("Asset transaction has unknown op");
			return error(errorMessage.c_str());
		}
	}
	...
	CTransaction txTmp;
	GetScrytTransactionDescription(txTmp, op, strResponseEnglish, ASSETALLOCATION, strResponseGUID);
	CAssetAllocation dbAssetAllocation;
	CAsset dbAsset;
	bool bRevert = false;
	...
	if (op == OP_ASSET_COLLECT_INTEREST)
	{
		...
		{
			errorMessage = "SCRYT_ASSET_ALLOCATION_CONSENSUS_ERROR: ERRCODE: 1010 - " + _("Cannot find asset allocation to collect interest on");
			return true;
		}
		if (!GetAsset(dbAssetAllocation.vchAsset, dbAsset))
		{
			errorMessage = "SCRYT_ASSET_ALLOCATION_CONSENSUS_ERROR: ERRCODE: 1011 - " + _("Failed to read from asset DB");
			return true;
		}
		if (dbAsset.fInterestRate <= 0 || dbAssetAllocation.fInterestRate <= 0)
		{
			errorMessage = "SCRYT_ASSET_ALLOCATION_CONSENSUS_ERROR: ERRCODE: 1012 - " + _("Cannot collect interest on this asset, no interest rate has been defined");
			return true;
		}
		if (!bSanityCheck) {
			bRevert = !fJustCheck;
			if (bRevert) {
				...
				{
					errorMessage = "SCRYT_ASSET_ALLOCATION_CONSENSUS_ERROR: ERRCODE: 1014 - " + _("Failed to revert asset allocation");
					return error(errorMessage.c_str());
				}
			}
		}
		...
		{
			errorMessage = "SCRYT_ASSET_ALLOCATION_CONSENSUS_ERROR: ERRCODE: 1015 - " + _("Cannot find sender asset allocation.");
			return true;
		}
		...
		if (!fJustCheck) {
			...
			{
				errorMessage = "SCRYT_ASSET_ALLOCATION_CONSENSUS_ERROR: ERRCODE: 1013 - " + _("You cannot collect interest on this asset: ") + errorMessageCollection;
				return true;
			}
			if (!bSanityCheck && !passetdb->WriteAsset(dbAsset, OP_ASSET_UPDATE))
			{
				errorMessage = "SCRYT_ASSET_CONSENSUS_ERROR: ERRCODE: 1014 - " + _("Failed to write to asset DB");
				return error(errorMessage.c_str());
			}
		}
		if(bSanityCheck)
	...
	else if (op == OP_ASSET_ALLOCATION_SEND)
	{
		...
		if (!vchAlias.empty() && CScrytAddress(user1).IsValid()) {
			errorMessage = "SCRYT_ASSET_ALLOCATION_CONSENSUS_ERROR: ERRCODE: 1016 - " + _("This asset allocation cannot be spent because owner is an alias but the alias is also a valid scryt address");
			return true;
		}
		if (!GetAssetAllocation(assetAllocationTuple, dbAssetAllocation))
		{
		...
		if (vchAlias.empty()) {
			...
			{
				errorMessage = "SCRYT_ASSET_ALLOCATION_CONSENSUS_ERROR: ERRCODE: 1015 - " + _("Cannot send this asset. Asset allocation owner must sign off on this change");
				return true;
			}
		}
		else if (dbAssetAllocation.vchAliasOrAddress != vchAlias)
		{
			errorMessage = "SCRYT_ASSET_ALLOCATION_CONSENSUS_ERROR: ERRCODE: 1017 - " + _("Cannot send this asset. Asset allocation owner must sign off on this change");
			return true;
		}
		if (!GetAsset(assetAllocationTuple.vchAsset, dbAsset))
		{
			errorMessage = "SCRYT_ASSET_ALLOCATION_CONSENSUS_ERROR: ERRCODE: 1016 - " + _("Failed to read from asset DB");
			return true;
		}
		if (!bSanityCheck) {
			bRevert = !fJustCheck;
		...
		if (!theAssetAllocation.listSendingAllocationAmounts.empty()) {
			if (dbAsset.bUseInputRanges) {
				errorMessage = "SCRYT_ASSET_ALLOCATION_CONSENSUS_ERROR: ERRCODE: 1018 - " + _("Invalid asset send, request to send amounts but asset uses input ranges");
				return true;
			}
			...
			for (auto& amountTuple : theAssetAllocation.listSendingAllocationAmounts) {
				...
				if (bRevert) {
					...
					{
						errorMessage = "SCRYT_ASSET_ALLOCATION_CONSENSUS_ERROR: ERRCODE: 1019 - " + _("Failed to revert asset allocation");
						return error(errorMessage.c_str());
					}
				}
				...
				{
					errorMessage = "SCRYT_ASSET_ALLOCATION_CONSENSUS_ERROR: ERRCODE: 1020 - " + _("Receiving amount must be positive");
					return true;
				}
			}
			...
			if (nBalanceAfterSend < 0) {
				...
				if(bSanityCheck)
					errorMessage = "SCRYT_ASSET_ALLOCATION_CONSENSUS_ERROR: ERRCODE: 1021 - " + _("Sender balance is insufficient");
			...
			for (auto& amountTuple : theAssetAllocation.listSendingAllocationAmounts) {
				...
				if (amountTuple.first == theAssetAllocation.vchAliasOrAddress) {
					errorMessage = "SCRYT_ASSET_ALLOCATION_CONSENSUS_ERROR: ERRCODE: 1022 - " + _("Cannot send an asset allocation to yourself");
					return true;
				}
				if (!bSanityCheck) {
					const CAssetAllocationTuple receiverAllocationTuple(theAssetAllocation.vchAsset, amountTuple.first);
					...
					{
						errorMessage = "SCRYT_ASSET_ALLOCATION_CONSENSUS_ERROR: ERRCODE: 1023 - " + _("Failed to write to asset allocation DB");
						return error(errorMessage.c_str());
					}
				}
			}
			...
			if (!dbAsset.bUseInputRanges) {
				errorMessage = "SCRYT_ASSET_ALLOCATION_CONSENSUS_ERROR: ERRCODE: 1024 - " + _("Invalid asset send, request to send input ranges but asset uses amounts");
				return true;
			}
			// check balance is sufficient on sender
			CAmount nTotal = 0;
			...
			for (auto& inputTuple : theAssetAllocation.listSendingAllocationInputs) {
				...
				if (bRevert) {
					if (!RevertAssetAllocation(receiverAllocationTuple, dbAsset, tx.GetHash(), nHeight, revertedAssetAllocations))
					{
						errorMessage = "SCRYT_ASSET_ALLOCATION_CONSENSUS_ERROR: ERRCODE: 1025 - " + _("Failed to revert asset allocation");
						return error(errorMessage.c_str());
					}
				}
				...
				if(rangeTotal == 0)
				{
					errorMessage = "SCRYT_ASSET_ALLOCATION_CONSENSUS_ERROR: ERRCODE: 1026 - " + _("Invalid input ranges");
					return true;
				}
			...
			if (nBalanceAfterSend < 0) {
				bBalanceOverrun = true;
				if(bSanityCheck)
					errorMessage = "SCRYT_ASSET_ALLOCATION_CONSENSUS_ERROR: ERRCODE: 1027 - " + _("Sender balance is insufficient");
				if (fJustCheck && !bSanityCheck) {
					// add conflicting sender
					assetAllocationConflicts.insert(assetAllocationTuple);
				}
			...
			for (unsigned int i = 0; i < theAssetAllocation.listSendingAllocationInputs.size();i++) {
				...
				if (input.first == theAssetAllocation.vchAliasOrAddress) {
					errorMessage = "SCRYT_ASSET_ALLOCATION_CONSENSUS_ERROR: ERRCODE: 1028 - " + _("Cannot send an asset allocation to yourself");
					return true;
				}
				if (!bSanityCheck) {
					...
					{
						errorMessage = "SCRYT_ASSET_ALLOCATION_CONSENSUS_ERROR: ERRCODE: 1029 - " + _("Input not found");
						return true;
					}
					...
					{
						errorMessage = "SCRYT_ASSET_ALLOCATION_CONSENSUS_ERROR: ERRCODE: 1030 - " + _("Failed to write to asset allocation DB");
						return error(errorMessage.c_str());
					}
				}
			}
	...
	if (!bSanityCheck && ((op == OP_ASSET_COLLECT_INTEREST && !fJustCheck) || (op != OP_ASSET_COLLECT_INTEREST))) {
		...
		{
			errorMessage = "SCRYT_ASSET_ALLOCATION_CONSENSUS_ERROR: ERRCODE: 1031 - " + _("Failed to write to asset allocation DB");
			return error(errorMessage.c_str());
		}
		// debug
		if (fDebug)
...
UniValue tpstestinfo(const JSONRPCRequest& request) {
	...
	if(!fTPSTest)
		throw runtime_error("SCRYT_ASSET_ALLOCATION_RPC_ERROR: ERRCODE: 1501 - " + _("This function requires tpstest configuration to be set upon startup. Please shutdown and enable it by adding it to your scryt.conf file and then call 'tpstestsetenabled true'."));
...
UniValue tpstestsetenabled(const JSONRPCRequest& request) {
	...
	if(!fTPSTest)
		throw runtime_error("SCRYT_ASSET_ALLOCATION_RPC_ERROR: ERRCODE: 1501 - " + _("This function requires tpstest configuration to be set upon startup. Please shutdown and enable it by adding it to your scryt.conf file and then try again."));
...
UniValue tpstestadd(const JSONRPCRequest& request) {
	...
	if (fTPSTestEnabled) {
		...
		if (bFirstTime) {
			...
			if (!isThreadPosted)
				throw runtime_error("SCRYT_ASSET_ALLOCATION_RPC_ERROR: ERRCODE: 1501 - " + _("thread pool queue is full"));
		}
	}
	UniValue result(UniValue::VOBJ);
	result.push_back(Pair("status", "success"));
...
UniValue assetallocationsend(const JSONRPCRequest& request) {
	...
	if (!GetAssetAllocation(assetAllocationTuple, theAssetAllocation))
		throw runtime_error("SCRYT_ASSET_ALLOCATION_RPC_ERROR: ERRCODE: 1500 - " + _("Could not find a asset allocation with this key"));

	CAsset theAsset;
	if (!GetAsset(vchAsset, theAsset))
		throw runtime_error("SCRYT_ASSET_ALLOCATION_RPC_ERROR: ERRCODE: 1501 - " + _("Could not find a asset with this key"));
	...
	for (auto& arrivalTime : arrivalTimes) {
		...
		if ((nNow - arrivalTime.second) < minLatency) {
			throw runtime_error("SCRYT_ASSET_ALLOCATION_RPC_ERROR: ERRCODE: 1503 - " + _("Please wait a few more seconds and try again..."));
		}
	}
	
	if (assetAllocationConflicts.find(assetAllocationTuple) != assetAllocationConflicts.end())
		throw runtime_error("SCRYT_ASSET_ALLOCATION_RPC_ERROR: ERRCODE: 1504 - " + _("This asset allocation is involved in a conflict which must be resolved with Proof-Of-Work. Please wait for a block confirmation and try again..."));
	...
	if (!theAssetAllocation.UnserializeFromData(data, vchHashAsset))
		throw runtime_error("SCRYT_ASSET_ALLOCATION_RPC_ERROR: ERRCODE: 1505 - " + _("Could not unserialize asset allocation data"));
	scriptPubKey << CScript::EncodeOP_N(OP_SCRYT_ASSET_ALLOCATION) << CScript::EncodeOP_N(OP_ASSET_ALLOCATION_SEND) << vchHashAsset << OP_2DROP << OP_DROP;
	...
	return scryttxfund_helper(vchAliasOrAddressFrom, vchWitness, aliasRecipient, vecSend);
}
UniValue assetallocationcollectinterest(const JSONRPCRequest& request) {
	...
	const CAssetAllocationTuple assetAllocationTuple(vchAsset, vchAliasOrAddressFrom);
	if (!GetAssetAllocation(assetAllocationTuple, theAssetAllocation))
		throw runtime_error("SCRYT_ASSET_ALLOCATION_RPC_ERROR: ERRCODE: 1506 - " + _("Could not find a asset allocation with this key"));
	CScript scriptPubKeyFromOrig;
	CAliasIndex fromAlias;
	if (!strAddressFrom.empty()) {
	...
	vector<unsigned char> vchHashAsset = vchFromString(hash.GetHex());
	scriptPubKey << CScript::EncodeOP_N(OP_SCRYT_ASSET_ALLOCATION) << CScript::EncodeOP_N(OP_ASSET_COLLECT_INTEREST) << vchHashAsset << OP_2DROP << OP_DROP;
	scriptPubKey += scriptPubKeyFromOrig;
	// send the asset pay txn
	vector<CRecipient> vecSend;
	CRecipient recipient;
...
UniValue assetallocationinfo(const JSONRPCRequest& request) {
	...
	LOCK(cs_assetallocation);
	if (!passetallocationdb || !passetallocationdb->ReadAssetAllocation(assetAllocationTuple, txPos))
		throw runtime_error("SCRYT_ASSET_ALLOCATION_RPC_ERROR: ERRCODE: 1507 - " + _("Failed to read from assetallocation DB"));

	CAsset theAsset;
	if (!GetAsset(vchAsset, theAsset))
		throw runtime_error("SCRYT_ASSET_ALLOCATION_RPC_ERROR: ERRCODE: 1508 - " + _("Could not find a asset with this key"));
...
bool BuildAssetAllocationIndexerJson(const CAssetAllocation& assetallocation, const CAsset& asset, const CAmount& nSenderBalance, const CAmount& nAmount, const string& strSender, const string& strReceiver, bool &isMine, UniValue& oAssetAllocation)
{
	...
	if (fAssetAllocationIndex) {
		...
		if (!strSenderTmp.empty() || !strReceiverTmp.empty()) {
			...
			isminefilter mine = IsMine(*pwalletMain, CScrytAddress(strSenderTmp).Get());
			if ((mine & filter)) {
				strCat = "send";
				nAmountDisplay *= -1;
			}
			else if(!strReceiverTmp.empty()){
				mine = IsMine(*pwalletMain, CScrytAddress(strReceiverTmp).Get());
				if ((mine & filter))
					strCat = "receive";
			}
		}
...
UniValue listassetallocationtransactions(const JSONRPCRequest& request) {
	...
	if (params.size() > 2)
		options = params[2];
	if (!fAssetAllocationIndex) {
		throw runtime_error("SCRYT_ASSET_ALLOCATION_RPC_ERROR: ERRCODE: 1509 - " + _("Asset allocation index not enabled, you must enable -assetallocationindex as a startup parameter or through scryt.conf file to use this function.")); 
	}
	LOCK(cs_assetallocationindex);
	UniValue oRes(UniValue::VARR);
	if (!passetallocationtransactionsdb->ScanAssetAllocationIndex(count, from, options, oRes))
		throw runtime_error("SCRYT_ASSET_ALLOCATION_RPC_ERROR: ERRCODE: 1509 - " + _("Scan failed"));
	return oRes;
}
UniValue listassetallocations(const JSONRPCRequest& request) {
	...
	if (params.size() > 0) {
		...
		if (count < 0) {
			throw runtime_error("SCRYT_ASSET_ALLOCATION_RPC_ERROR: ERRCODE: 1510 - " + _("'count' must be 0 or greater"));
		}
	}
	if (params.size() > 1) {
		from = params[1].get_int();
		if (from < 0) {
			throw runtime_error("SCRYT_ASSET_ALLOCATION_RPC_ERROR: ERRCODE: 1510 - " + _("'from' must be 0 or greater"));
		}
	}
	if (params.size() > 2) {
		options = params[2];
	...
	if (!passetallocationdb->ScanAssetAllocations(count, from, options, oRes))
		throw runtime_error("SCRYT_ASSET_ALLOCATION_RPC_ERROR: ERRCODE: 1510 - " + _("Scan failed"));
	return oRes;
}
