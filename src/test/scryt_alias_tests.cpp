// Copyright (c) 2016-2018 The ScrytExchange developers
...
BOOST_FIXTURE_TEST_SUITE (scryt_alias_tests, BasicScrytTestingSetup)
...
BOOST_AUTO_TEST_CASE (generate_big_aliasdata)
{
	...
	BOOST_CHECK_NO_THROW(r = CallRPC("node1", "scryttxfund " + varray[0].get_str()));
	varray = r.get_array();
	BOOST_CHECK_NO_THROW(r = CallRPC("node1", "signrawtransaction " + varray[0].get_str()));
	BOOST_CHECK_NO_THROW(CallRPC("node1", "scrytsendrawtransaction " + find_value(r.get_obj(), "hex").get_str()));
	GenerateBlocks(5);
	// activation should fail
	BOOST_CHECK_NO_THROW(r = CallRPC("node1", "aliasnew jag2 " + baddata + " 3 0 " + newaddress + " '' '' ''"));
	varray = r.get_array();
	BOOST_CHECK_THROW(r = CallRPC("node1", "scryttxfund " + varray[0].get_str()), runtime_error);
	...
	BOOST_CHECK_NO_THROW(r = CallRPC("node1", "scryttxfund " + varray1[0].get_str()));
	...
	BOOST_CHECK_NO_THROW(r = CallRPC("node1", "scryttxfund " + varray3[0].get_str()));
	...
	BOOST_CHECK_NO_THROW(r = CallRPC("node1", "scryttxfund " + varray5[0].get_str()));
...
BOOST_AUTO_TEST_CASE (generate_aliaswitness)
{
	...
	BOOST_CHECK_NO_THROW(CallRPC("node2", "scrytsendrawtransaction " + find_value(r.get_obj(), "hex").get_str()));
...
BOOST_AUTO_TEST_CASE (generate_aliaspay)
{
	...
	for (unsigned int i = 0; i < MAX_ALIAS_UPDATES_PER_BLOCK; i++)
	{
		...
		BOOST_CHECK_NO_THROW(CallRPC("node3", "scrytsendrawtransaction " + find_value(r.get_obj(), "hex").get_str()));
	}
...
BOOST_AUTO_TEST_CASE (generate_aliasprunewithoffer)
{
	...
	string offerguid = OfferNew("node1", "aliasprunewithoffer", "category", "title", "1", "0.05", "description", "SCY");
...
BOOST_AUTO_TEST_CASE (generate_aliasprunewithcertoffer)
{
	...
	string certofferguid = OfferNew("node1", "aliasprunewithcertoffer", "certificates", "title", "1", "0.05", "description", "SCY", certguid);	
	string offerguid = OfferNew("node1", "aliasprunewithcertoffer", "category", "title", "1", "0.05", "description", "SCY");
...
BOOST_AUTO_TEST_CASE (generate_aliasexpired)
{
	...
	CScrytAddress aliasAddress(pubKey.GetID());
	...
	BOOST_CHECK_THROW(CallRPC("node2", "offernew aliasexpirednode2 category title 1 0.05 description USD '' SCY false 1 BUYNOW 0 0 false 0 ''"), runtime_error);
	// should fail: new escrow with expired arbiter alias

	BOOST_CHECK_THROW(CallRPC("node2", "escrownew false aliasexpire2node2 aliasexpirednode2 " + offerguid + " 1 true 0 0 25 0.005 0 '' SCY 0 0 ''"), runtime_error);
	// should fail: new escrow with expired alias
	BOOST_CHECK_THROW(CallRPC("node2", "escrownew false aliasexpirednode2 aliasexpire " + offerguid + " 1 true 0 0 25 0.005 0 '' SCY 0 0 ''"), runtime_error);
	...
	BOOST_CHECK_NO_THROW(CallRPC("node1", "offerupdate aliasexpire0 " + offerguid + " category title 100 0.05 description USD false '' 0 SCY BUYNOW 0 0 false 0 ''"));
	GenerateBlocks(5, "node1");
