// Copyright (c) 2016-2017 The ScrytExchange developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include "test/test_scryt_services.h"
#include "utiltime.h"
...
BOOST_GLOBAL_FIXTURE( ScrytTestingSetup );

BOOST_FIXTURE_TEST_SUITE (scryt_offer_tests, BasicScrytTestingSetup)
...
BOOST_AUTO_TEST_CASE (generate_offernew)
{
	...
	BOOST_CHECK_THROW(r = CallRPC("node1", "offernew fooalias category title 100 0.05 description USD '' SCY false 1 BUYNOW 0 0 false 0 ''"), runtime_error);

	// should fail: generate an offer with negative quantity
	BOOST_CHECK_THROW(r = CallRPC("node1", "offernew selleralias1 category title -2 0.05 description USD '' SCY false 1 BUYNOW 0 0 false 0 ''"), runtime_error);
	...
	BOOST_CHECK_THROW(r = CallRPC("node1", "offernew selleralias1 " + s257bytes + " title 100 0.05 description USD '' SCY false 1 BUYNOW 0 0 false 0 ''"), runtime_error);	

	// should fail: generate an offer too-large title
	BOOST_CHECK_THROW(r = CallRPC("node1", "offernew selleralias1 category " + s257bytes + " 100 0.05 description USD '' SCY false 1 BUYNOW 0 0 false 0 ''"), runtime_error);	
	...
	BOOST_CHECK_THROW(r = CallRPC("node1", "offernew selleralias1 category title 100 0.05 " + s1025bytes + " USD '' SCY false 1 BUYNOW 0 0 false 0 ''"), runtime_error);

	// should pass: generate an offer with random currency
	BOOST_CHECK_NO_THROW(r = CallRPC("node1", "offernew selleralias1 category title 100 0.05 description ZZZ '' SCY false 1 BUYNOW 0 0 false 0 ''"));
...
BOOST_AUTO_TEST_CASE (generate_certoffer)
{
	...
	BOOST_CHECK_THROW(r = CallRPC("node1", "offernew node1alias category title 1 0.05 description USD " + certguid1 + " SCY false 1 BUYNOW 0 0 false 0 ''"), runtime_error);

	// should fail: generate a cert offer using a quantity greater than 1
	BOOST_CHECK_THROW(r = CallRPC("node1", "offernew node1alias certificates title 2 0.05 description USD " + certguid1 + " SCY false 1 BUYNOW 0 0 false 0 ''"), runtime_error);

	// should fail: generate a cert offer using a zero quantity
	BOOST_CHECK_THROW(r = CallRPC("node1", "offernew  node1alias certificates title 0 0.05 description USD " + certguid1 + " SCY false 1 BUYNOW 0 0 false 0 ''"), runtime_error);

	// should fail: generate a cert offer using an unlimited quantity
	BOOST_CHECK_THROW(r = CallRPC("node1", "offernew node1alias certificates title -1 0.05 description USD " + certguid1 + " SCY false 1 BUYNOW 0 0 false 0 ''"), runtime_error);

	// should fail: generate a cert offer using a cert guid you don't own
	BOOST_CHECK_THROW(r = CallRPC("node1", "offernew node1alias certificates title 1 0.05 description USD " + certguid2 + " SCY false 1 BUYNOW 0 0 false 0 ''"), runtime_error);

	// should fail: update non cert offer to cert category
	BOOST_CHECK_THROW(r = CallRPC("node1", "offerupdate node1alias " + offerguidnoncert + " certificates title 1 0.15 description USD false '' 0 SCY BUYNOW 0 0 false 0 ''"), runtime_error);

	// should fail: update non cert offer to cert sub category
	BOOST_CHECK_THROW(r = CallRPC("node1", "offerupdate node1alias " + offerguidnoncert + " certificates>music title 1 0.15 description USD false '' 0 SCY BUYNOW 0 0 false 0 ''"), runtime_error);
	...
	BOOST_CHECK_THROW(r = CallRPC("node1", "offerupdate node1alias " + offerguidnoncert + " category title 1 0.15 description USD false " + certguid1 + " 0 SCY BUYNOW 0 0 false 0 ''"), runtime_error);
	...
	// generate a cert offer if accepting BTC OR SCY
	OfferNew("node1", "node1alias", "certificates", "title", "1", "0.05", "description", "USD", certguid1, "SCY+BTC");

	// should fail: generate a cert offer using different alias for cert and offer
	BOOST_CHECK_THROW(r = CallRPC("node1", "offernew node1alias certificates title 1 0.05 description USD " + certguid1a + " SCY false 1 BUYNOW 0 0 false 0 ''"), runtime_error);
...
BOOST_AUTO_TEST_CASE(generate_offerwhitelists)
{
	...
	// generate a good offer
	string offerguid = OfferNew("node1", "sellerwhitelistalias", "category", "title", "100", "9.00", "description", "SCY");
	// add to whitelist
...
BOOST_AUTO_TEST_CASE (generate_offerupdate)
{
	...
	BOOST_CHECK_NO_THROW(r = CallRPC("node2", "offerupdate selleralias2 " + offerguid + " category title 90 0.15 description USD false '' 0 SCY BUYNOW 0 0 false 0 ''"));
	...
	// should fail: generate an offer with unknown alias
	BOOST_CHECK_THROW(r = CallRPC("node1", "offerupdate fooalias " + offerguid + " category title 90 0.15 description USD false '' 0 SCY BUYNOW 0 0 false 0 ''"), runtime_error);
	...
	BOOST_CHECK_THROW(r = CallRPC("node1", "offerupdate selleralias2 " + offerguid + " " + s257bytes + " title 90 0.15 description USD false '' 0 SCY BUYNOW 0 0 false 0 ''"), runtime_error);	

	// should fail: generate an offer too-large title
	BOOST_CHECK_THROW(r = CallRPC("node1", "offerupdate selleralias2 " + offerguid + " category " + s257bytes + " 90 0.15 description USD false '' 0 SCY BUYNOW 0 0 false 0 ''"), runtime_error);	
	...
	BOOST_CHECK_THROW(r = CallRPC("node1", "offerupdate selleralias2 " + offerguid + " category title 90 0.15 " + s1025ytes + " USD false '' 0 SCY BUYNOW 0 0 false 0 ''"), runtime_error);
...
BOOST_AUTO_TEST_CASE (generate_offerupdate_editcurrency)
{
	...
	OfferUpdate("node1", "selleraliascurrency", offerguid, "category", "titlenew", "90", "1.00", "descriptionnew1", "SCY");
	...
	BOOST_CHECK_NO_THROW(r = CallRPC("node1", "offerupdate selleraliascurrency " + offerguid + " category title 93 0.2 desc EUR false '' 0 SCY BUYNOW 0 0 false 0 ''"));
	UniValue arr = r.get_array();
	BOOST_CHECK_NO_THROW(r = CallRPC("node1", "signrawtransaction " + arr[0].get_str()));
	BOOST_CHECK_NO_THROW(r = CallRPC("node1", "scrytsendrawtransaction " + find_value(r.get_obj(), "hex").get_str()));

	BOOST_CHECK_NO_THROW(r = CallRPC("node2", "escrownew false buyeraliascurrency arbiteraliascurrency1 " + offerguid + " 10 true 1 0 25 0.005 0 '' SCY 0 0 ''"));
	UniValue arr1 = r.get_array();
	escrowguid = arr1[1].get_str();
	BOOST_CHECK_NO_THROW(r = CallRPC("node2", "signrawtransaction " + arr1[0].get_str()));
	BOOST_CHECK_NO_THROW(r = CallRPC("node2", "scrytsendrawtransaction " + find_value(r.get_obj(), "hex").get_str()));
...
BOOST_AUTO_TEST_CASE (generate_offeraccept)
{
	...
	BOOST_CHECK_THROW(r = CallRPC("node2", "escrownew false buyeralias3 arbiteralias3 " + offerguid + " -1 true 0 0 25 0.005 0 '' SCY 0 0 ''"), runtime_error);

	// perform an accept on zero quantity
	BOOST_CHECK_THROW(r = CallRPC("node2", "escrownew false buyeralias3 arbiteralias3 " + offerguid + " 0 true 0 0 25 0.005 0 '' SCY 0 0 ''"), runtime_error);

	// perform an accept on more items than available
	BOOST_CHECK_THROW(r = CallRPC("node2", "escrownew false buyeralias3 arbiteralias3 " + offerguid + " 101 true 0 0 25 0.005 0 '' SCY 0 0 ''"), runtime_error);
...
BOOST_AUTO_TEST_CASE (generate_offerexpired)
{
	...
	BOOST_CHECK_THROW(r = CallRPC("node2", "escrownew false buyeralias4 arbiteralias4 " + offerguid + " 1 true 0 0 25 0.005 0 '' SCY 0 0 ''"), runtime_error);

	// should fail: offer update on an expired offer
	BOOST_CHECK_THROW(r = CallRPC("node1", "offerupdate selleralias4 " + offerguid + " category title 90 0.15 description USD false '' 0 SCY BUYNOW 0 0 false 0 ''"), runtime_error);
...
BOOST_AUTO_TEST_CASE (generate_certofferexpired)
{
	...
	BOOST_CHECK_NO_THROW(r = CallRPC("node1", "offernew node1alias2 certificates title 1 0.05 description USD " + certguid + " SCY false 1 BUYNOW 0 0 false 0 ''"));
	UniValue arr = r.get_array();
	BOOST_CHECK_NO_THROW(r = CallRPC("node1", "signrawtransaction " + arr[0].get_str()));
	BOOST_CHECK_NO_THROW(r = CallRPC("node1", "scrytsendrawtransaction " + find_value(r.get_obj(), "hex").get_str()));
	...
	BOOST_CHECK_THROW(r = CallRPC("node2", "escrownew false node2alias2 node3alias2 " + offerguid + " 1 true 0 0 25 0.005 0 '' SCY 0 0 ''"), runtime_error);
	// should fail: generate a cert offer using an expired cert
	BOOST_CHECK_THROW(r = CallRPC("node1", "offernew node1alias2 certificates title 1 0.05 description USD " + certguid1 + " SCY false 1 BUYNOW 0 0 false 0 ''"), runtime_error);
	/// should fail: generate a cert offer using an expired cert
	BOOST_CHECK_THROW(r = CallRPC("node1", "offernew node1alias2 certificates title 1 0.05 description USD " + certguid + " SCY false 1 BUYNOW 0 0 false 0 ''"), runtime_error);
	GenerateBlocks(5);
	GenerateBlocks(5, "node2");
}
BOOST_AUTO_TEST_CASE (generate_offerpruning)
{
	...
	BOOST_CHECK_THROW(CallRPC("node1", "offerupdate pruneoffer " + guid1 + " category title 1 0.05 description USD false '' 0 SCY BUYNOW 0 0 false 0 ''"), runtime_error);
	// can still search
	BOOST_CHECK_NO_THROW(CallRPC("node1", "offerinfo " + guid1));
	BOOST_CHECK_NO_THROW(CallRPC("node2", "offerinfo " + guid1));
