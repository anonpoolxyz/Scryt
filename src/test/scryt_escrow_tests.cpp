...
BOOST_FIXTURE_TEST_SUITE(scryt_escrow_tests, BasicScrytTestingSetup)
BOOST_AUTO_TEST_CASE(generate_auction_regular)
{
	...
	string offerguid = OfferNew("node2", "sellerauction", "category", "title", "100", "0.05", "description", "USD", "''" /*certguid*/, "SCY" /*paymentoptions*/, "BUYNOW+AUCTION", expiry);
	...
	BOOST_CHECK_THROW(r = CallRPC("node2", "offerupdate sellerauction " + offerguid + " category title 90 0.15 description USD false '' 0 SCY BUYNOW 0 0 true 0 ''"), runtime_error);
...
BOOST_AUTO_TEST_CASE(generate_auction_reserve)
{
	...
	string expiry = boost::lexical_cast<string>(mediantime);
	string offerguid = OfferNew("node2", "sellerauction1", "category", "title", "100", "0.05", "description", "USD", "''" /*certguid*/, "SCY" /*paymentoptions*/, "BUYNOW+AUCTION", expiry, "0.011");
...
BOOST_AUTO_TEST_CASE(generate_escrowrelease_invalid)
{
	...
	string offerguid = OfferNew("node2", "selleraliasrefund3", "category", "title", "100", "1.45", "description", "SCY");
...
BOOST_AUTO_TEST_CASE(generate_escrowfeedback)
{
	...
	BOOST_CHECK_NO_THROW(r = CallRPC("node1", "scrytsendrawtransaction " + finalhexstr));
	...
	BOOST_CHECK_NO_THROW(r = CallRPC("node2", "scrytsendrawtransaction " + finalhexstr));
	...
	finalhexstr = find_value(r.get_obj(), "hex").get_str();
	BOOST_CHECK_NO_THROW(CallRPC("node3", "scrytsendrawtransaction " + finalhexstr));
