...
#ifndef SCRYT_TEST_TEST_SCRYT_SERVICES_H
#define SCRYT_TEST_TEST_SCRYT_SERVICES_H
...
/** Testing scryt services setup that configures a complete environment with 3 nodes.
 */
...
const string OfferNew(const string& node, const string& aliasname, const string& category, const string& title, const string& qty, const string& price, const string& description, const string& currency, const string& certguid="''", const string& paymentoptions="SCY",  const string& offerType = "BUYNOW", const string& auction_expires = "0", const string& auction_reserve = "0", const string& auction_require_witness = "false", const string &auction_deposit = "0", const string& witness="''");
...
// SCRYT testing setup
struct ScrytTestingSetup {
    ScrytTestingSetup();
    ~ScrytTestingSetup();
};
struct BasicScrytTestingSetup {
    BasicScrytTestingSetup();
    ~BasicScrytTestingSetup();
};
struct ScrytMainNetSetup {
	ScrytMainNetSetup();
	~ScrytMainNetSetup();
};
#endif
