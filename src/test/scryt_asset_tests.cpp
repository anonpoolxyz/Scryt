...
BOOST_FIXTURE_TEST_SUITE (scryt_asset_tests, BasicScrytTestingSetup)
...
BOOST_AUTO_TEST_CASE(generate_asset_throughput)
{
	...
	for (int i = 0; i < numberOfTransactionToSend; i++) {
		...
		BOOST_CHECK_NO_THROW(r = CallRPC("node1", "scrytsendrawtransaction " + hex_str));
...
BOOST_AUTO_TEST_CASE(generate_asset_maxsenders)
{
	...
	for (int i = 0; i < 250; i++) {
		...
		BOOST_CHECK_NO_THROW(r = CallRPC("node1", "scryttxfund " + varray[0].get_str() + " " + "\"{\\\"addresses\\\":[\\\"" + strAddress + "\\\"]}\""));
		varray = r.get_array();
		...
		BOOST_CHECK_NO_THROW(r = CallRPC("node1", "scryttxfund " + varray1[0].get_str() + " " + "\"{\\\"addresses\\\":[\\\"" + strAddress + "\\\"]}\""));
		varray1 = r.get_array();
