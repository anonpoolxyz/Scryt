...
// Copyright (c) 2020-2020 The ScrytExchange developers
// Distributed under the MIT software license, see the accompanying
// Copyright (c) 2020-2020 \_\_I TYLER I_/_/#5223 
...
#if defined(NDEBUG)
# error "Scryt Core cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Scryt Signed Message:\n";
...
// SCRYT
bool CheckScrytInputs(const CTransaction& tx, CValidationState& state, const CCoinsViewCache &inputs, bool fJustCheck, int nHeight, const CBlock& block, bool bSanity)
{
	...
	if (block.vtx.empty() && tx.nVersion == SCRYT_TX_VERSION) {
		...
		if (good)
		{
			...
			return state.DoS(100, false, REJECT_INVALID, "scryt-inputs-error");
		...
		if (CreateGraphFromVTX(nHeight, sortedBlock.vtx, graph, vertices, mapTxIndex)) {
			...
			if (!sortedBlock.vtx.empty()) {
				if (!DAGTopologicalSort(sortedBlock.vtx, conflictedIndexes, graph, mapTxIndex)) {
					if (fDebug)
						LogPrintf("CheckScrytInputs: Toposort failed");
					return true;
				}
			}
		}
		...
		nFlushIndexBlocks++;
		if ((nFlushIndexBlocks % 200) == 0 && !FlushScrytDBs())
			return state.DoS(0, false, REJECT_INVALID, "Failed to flush scryt databases");
	}
	return true;
}
                              ...
                              std::vector<COutPoint>& coins_to_uncache, bool fDryRun, bool bMultiThreaded)
{
		...
		BOOST_FOREACH(const CTxIn &txin, tx.vin)
		{
			...
			if (itConflicting != pool.mapNextTx.end())
			{
				...
				if (!setConflicts.count(ptxConflicting->GetHash()))
				{
					...
					// SCRYT txs are not replacable
					else if (ptxConflicting->nVersion == SCRYT_TX_VERSION) {
						return state.DoS(0, error("AcceptToMemoryPool : Scryt Transaction %s conflicts with Transaction request %s",
							hash.ToString(), ptxConflicting->GetHash().ToString()),
...

	{
		...
		if (!bMultiThreaded) {
			...
			if (!control.Wait())
				return false;
			if (!CheckScrytInputs(tx, state, view, true, chainActive.Height(), CBlock())) {
				return false;
			}
		}
		...
		if (bMultiThreaded && threadpool != NULL)
		{
			...
			static int totalScrytCheckCount = 0;
			static int totalScrytCheckMicros = 0;
			...
			std::packaged_task<void()> task([&pool, ptx, hash, coins_to_uncache, hashCacheEntry, vChecks]() {
				...
				int thisScrytCheckCount = 0;
				int thisCheckMicros = 0;
				int thisScrytCheckMicros = 0;
				...
				if (isCheckPassing)
				{
					int64_t scrytCheckTime;
					if (fLogThreadpool) {
						scrytCheckTime = GetTimeMicros();
						thisScrytCheckCount += 1;
					}
					if (!CheckScrytInputs(txIn, validationState, coinsViewCache, true, chainActive.Height(), CBlock()))
					{
						nLastMultithreadMempoolFailure = GetTime();
						LOCK2(cs_main, mempool.cs);
						LogPrint("mempool", "%s: %s\n", "CheckScrytInputs Error", hash.ToString());
						BOOST_FOREACH(const COutPoint& hashTx, coins_to_uncache)
							pcoinsTip->Uncache(hashTx);
						pool.removeRecursive(txIn, MemPoolRemovalReason::UNKNOWN);
						pool.ClearPrioritisation(hash);
					...
					if (fLogThreadpool) {
						// how long did we run CheckScrytInputs()'s
						thisScrytCheckMicros = GetTimeMicros() - scrytCheckTime;
					}
				}
				if (fLogThreadpool) {
					// metrics
					...
					totalScrytCheckCount += thisScrytCheckCount;
					totalScrytCheckMicros += thisScrytCheckMicros;
			...
			if (fLogThreadpool && totalCheckCount > 0 && totalScrytCheckCount > 0 && totalExecutionCount > 0 && (!fUnitTest || (fUnitTest && totalExecutionCount % 100 == 0))) {
				int avgCheckMicros = totalCheckMicros / totalCheckCount;
				int avgScrytCheckMicros = totalScrytCheckMicros / totalScrytCheckCount;
				...
				const std::string &messageInternals = "THREADPOOL::%s:Signature check internals - check(%d): total %lld, avg %lld microseconds, scryt(%d): total %lld, avg %lld microseconds\n";
				...
				LogPrint("threadpool", messageInternals, hash.ToString(), totalCheckCount, totalCheckMicros, avgCheckMicros, totalScrytCheckCount, totalScrytCheckMicros, avgScrytCheckMicros);
...
{
	// SCRYT if its been less 60 seconds since the last MT mempool verification failure then fallback to single threaded
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams, CAmount &nTotalRewardWithMasternodes, bool fSuperblockPartOnly, bool fMasternodePartOnly, unsigned int nStartHeight)
{
    ...
    else if (nHeight >= 2 && nHeight <= 1000)
    {
        nSubsidy = 1 * COIN;
    }
    else if (nHeight >= 1001 && nHeight <= 10000)
    {
        nSubsidy = 2 * COIN;
    }
    ...
    else if (nHeight >= 40001 && nHeight <= 60000)
    {
        nSubsidy = 6 * COIN;
    }
    else if (nHeight >= 60001 && nHeight <= 80000)
    {
        nSubsidy = 8 * COIN;
    }
    ...
    else if (nHeight >= 150001 && nHeight <= 200000)
    {
        nSubsidy = 14 * COIN;
    }
        ...
        if (fAddressIndex) {
            ...
            for (unsigned int k = tx.vout.size(); k-- > 0;) {
                ...
                if (out.scriptPubKey.IsPayToScriptHash()) {
					...
					if (RemoveScrytScript(out.scriptPubKey, scriptPubKeyOut))
            ...
            for (unsigned int j = tx.vin.size(); j-- > 0;) {
                ...
                if (fAddressIndex) {
                    ...
                    if (prevout.scriptPubKey.IsPayToScriptHash()) {
						...
						CScript scriptOut;
						CScript scriptPubKeyOut;
						if (RemoveScrytScript(prevout.scriptPubKey, scriptPubKeyOut))
							scriptOut = scriptPubKeyOut;
						else
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck = false)
{
	...
	if (!CheckScrytInputs(*block.vtx[0], state, viewOld, fJustCheck, pindex->nHeight, block))
		return error("ConnectBlock(): CheckScrytInputs on block %s failed\n",
			block.GetHash().ToString());
    int64_t nTime3 = GetTimeMicros(); nTimeConnect += nTime3 - nTime2;
    LogPrint("bench", "      - Connect %u transactions: %.2fms (%.3fms/tx, %.3fms/txin) [%.2fs]\n", (unsigned)block.vtx.size(), 0.001 * (nTime3 - nTime2), 0.001 * (nTime3 - nTime2) / block.vtx.size(), nInputs <= 1 ? 0 : 0.001 * (nTime3 - nTime2) / (nInputs-1), nTimeConnect * 0.000001);

    // SCRYT : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
	...
	if (!IsBlockPayeeValid(*block.vtx[0], pindex->nHeight, blockReward, nFees, nTotalRewardWithMasternodes)) {
		...
		return state.DoS(0, error("ConnectBlock(SCY): couldn't find masternode or superblock payments"),
			REJECT_INVALID, "bad-cb-payee");
	}

    std::string strError = "";
    if (!IsBlockValueValid(block, pindex->nHeight, nTotalRewardWithMasternodes, nFees, strError)) {
        return state.DoS(0, error("ConnectBlock(SCY): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }
    // END SCRYT
...
bool static FlushStateToDisk(CValidationState &state, FlushStateMode mode, int nManualPruneHeight) {
    ...
    if (fDoFullFlush) {
		...
		// SCRYT
		if (!FlushScrytDBs())
			return AbortNode(state, "Failed to flush scryt databases");
        nLastFlush = nNow;
    }
...
void static UpdateTip(CBlockIndex *pindexNew, const CChainParams& chainParams) {
    ...
    if (!IsInitialBlockDownload())
    {
        ...
        for (int bit = 0; bit < VERSIONBITS_NUM_BITS; bit++) {
			// SCRYT, skip auxpow bit 8 or dummy bit 28
			if (bit == 8 || bit == 28)
				continue;
...
{
	// SCRYT check auxpow first then fallback to normal check
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // SCY : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        LogPrintf("CheckBlock(SCY): spork is off, skipping transaction locking checks\n");
    }
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // SCRYT check for version 2, 3 and 4 upgrades
