...
// Scryt This is not a CORS check. Only check if OPTIONS are
...
static bool checkPreflight(HTTPRequest* req)
{
    ...
    // Scryt Keep origin so as not to have to set Access-Control-Allow-Origin
    // to *
    ...
    // unbounded.
    // Scryt We are only satifying the OPTIONS request in this method in order to
    ...
    if (req->GetRequestMethod() == HTTPRequest::OPTIONS) {
        ...
        // Scryt as we are only handling OPTIONS there is
