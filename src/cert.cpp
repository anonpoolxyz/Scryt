...
void CCertDB::WriteCertIndex(const CCert& cert, const int& op) {
	if (IsArgSet("-zmqpubcertrecord")) {
		...
		if (BuildCertIndexerJson(cert, oName)) {
			GetMainSignals().NotifyScrytUpdate(oName.write().c_str(), "certrecord");
		}
	}
...
void CCertDB::WriteCertIndexHistory(const CCert& cert, const int &op) {
	if (IsArgSet("-zmqpubcerthistory")) {
		...
		if (BuildCertIndexerHistoryJson(cert, oName)) {
			...
			GetMainSignals().NotifyScrytUpdate(oName.write().c_str(), "certhistory");
		}
	}
}
...
bool RevertCert(const std::vector<unsigned char>& vchCert, const int op, const uint256 &txHash, sorted_vector<std::vector<unsigned char> > &revertedCerts) {
	...
	if (!pcertdb->ReadLastCert(vchCert, dbCert)) {
		...
		{
			errorMessage = "SCRYT_CERTIFICATE_CONSENSUS_ERROR: ERRCODE: 3000 - " + _("Failed to erase cert");
	...
	{
		errorMessage = "SCRYT_CERTIFICATE_CONSENSUS_ERROR: ERRCODE: 3001 - " + _("Failed to write to cert DB");
		return error(errorMessage.c_str());
	}
...
bool CheckCertInputs(const CTransaction &tx, int op, const vector<vector<unsigned char> > &vvchArgs, const std::vector<unsigned char> &vvchAlias,
	...
	vector<unsigned char> vchHash;
	int nDataOut;
	if(!GetScrytData(tx, vchData, vchHash, nDataOut) || !theCert.UnserializeFromData(vchData, vchHash))
	{
		errorMessage = "SCRYT_CERTIFICATE_CONSENSUS_ERROR ERRCODE: 3002 - " + _("Cannot unserialize data inside of this transaction relating to a certificate");
		return true;
	}
	...
	if(fJustCheck)
	{
		if(vvchArgs.size() != 1)
		{
			errorMessage = "SCRYT_CERTIFICATE_CONSENSUS_ERROR: ERRCODE: 3003 - " + _("Certificate arguments incorrect size");
			return error(errorMessage.c_str());
		}			
		if(vchHash != vvchArgs[0])
		{
			errorMessage = "SCRYT_CERTIFICATE_CONSENSUS_ERROR: ERRCODE: 2003 - " + _("Hash provided doesn't match the calculated hash of the data");
			return true;
		}	
	}
	...
	if(fJustCheck)
	{
		...
		{
			errorMessage = "SCRYT_CERTIFICATE_CONSENSUS_ERROR: ERRCODE: 3004 - " + _("Certificate category too big");
			return error(errorMessage.c_str());
		}
		if(theCert.vchPubData.size() > MAX_VALUE_LENGTH)
		{
			errorMessage = "SCRYT_CERTIFICATE_CONSENSUS_ERROR: ERRCODE: 3005 - " + _("Certificate public data too big");
			return error(errorMessage.c_str());
		}
		switch (op) {
		case OP_CERT_ACTIVATE:
			...
			{
				errorMessage = "SCRYT_CERTIFICATE_CONSENSUS_ERROR: ERRCODE: 3006 - " + _("Certificate hex guid too long");
				return error(errorMessage.c_str());
			}
			if((theCert.vchTitle.size() > MAX_NAME_LENGTH || theCert.vchTitle.empty()))
			{
				errorMessage = "SCRYT_CERTIFICATE_CONSENSUS_ERROR: ERRCODE: 3007 - " + _("Certificate title too big or is empty");
				return error(errorMessage.c_str());
			}
			if(!boost::algorithm::starts_with(stringFromVch(theCert.sCategory), "certificates"))
			{
				errorMessage = "SCRYT_CERTIFICATE_CONSENSUS_ERROR: ERRCODE: 3008 - " + _("Must use a certificate category");
				return true;
			}
			break;
		...
		case OP_CERT_UPDATE:
			...
			{
				errorMessage = "SCRYT_CERTIFICATE_CONSENSUS_ERROR: ERRCODE: 3009 - " + _("Certificate title too big");
				return error(errorMessage.c_str());
			}
			if(theCert.sCategory.size() > 0 && !boost::algorithm::istarts_with(stringFromVch(theCert.sCategory), "certificates"))
			{
				errorMessage = "SCRYT_CERTIFICATE_CONSENSUS_ERROR: ERRCODE: 3010 - " + _("Must use a certificate category");
				return true;
			}
			break;
		...
		case OP_CERT_TRANSFER:
			if(theCert.sCategory.size() > 0 && !boost::algorithm::istarts_with(stringFromVch(theCert.sCategory), "certificates"))
			{
				errorMessage = "SCRYT_CERTIFICATE_CONSENSUS_ERROR: ERRCODE: 3011 - " + _("Must use a certificate category");
				return true;
			}
			break;

		default:
			errorMessage = "SCRYT_CERTIFICATE_CONSENSUS_ERROR: ERRCODE: 3012 - " + _("Certificate transaction has unknown op");
			return error(errorMessage.c_str());
		}
	}
	if (!fJustCheck && !bSanityCheck) {
		...
		{
			errorMessage = "SCRYT_CERTIFICATE_CONSENSUS_ERROR: ERRCODE: 3013 - " + _("Failed to revert cert");
			return error(errorMessage.c_str());
		}
	}
	...
	CTransaction txTmp;
	GetScrytTransactionDescription(txTmp, op, strResponseEnglish, CERT, strResponseGUID);
	// if not an certnew, load the cert data from the DB
	CCert dbCert;
	if (!GetCert(theCert.vchCert, dbCert))
	{
		if (op != OP_CERT_ACTIVATE) {
			errorMessage = "SCRYT_CERTIFICATE_CONSENSUS_ERROR: ERRCODE: 3014 - " + _("Failed to read from certificate DB");
			return true;
		}
	}
	if(op != OP_CERT_ACTIVATE) 
	{
		...
		{
			errorMessage = "SCRYT_CERTIFICATE_CONSENSUS_ERROR: ERRCODE: 3015 - " + _("Cannot update this certificate. Certificate owner must sign off on this change.");
			return true;
		}
		...
		if (!GetFirstCert(dbCert.vchCert, firstCert)) {
			errorMessage = "SCRYT_CERTIFICATE_CONSENSUS_ERROR: ERRCODE: 3016 - " + _("Cannot read first cert from cert DB");
			return true;
		}
		if(op == OP_CERT_TRANSFER)
		{
			...
			{
				errorMessage = "SCRYT_CERTIFICATE_CONSENSUS_ERROR: ERRCODE: 3017 - " + _("Cannot find alias you are transferring to. It may be expired");	
				return true;
			}
			if(!(alias.nAcceptTransferFlags & ACCEPT_TRANSFER_CERTIFICATES))
			{
				errorMessage = "SCRYT_CERTIFICATE_CONSENSUS_ERROR: ERRCODE: 3018 - " + _("The alias you are transferring to does not accept certificate transfers");
				return true;
			}
			...
			{
				errorMessage = "SCRYT_CERTIFICATE_CONSENSUS_ERROR: ERRCODE: 3019 - " + _("Cannot transfer this certificate. Insufficient privileges.");
				return true;
			}
		}
		else if(op == OP_CERT_UPDATE)
		{
			...
			{
				errorMessage = "SCRYT_CERTIFICATE_CONSENSUS_ERROR: ERRCODE: 3020 - " + _("Cannot edit this certificate. It is view-only.");
				return true;
			}
		}
		if(theCert.nAccessFlags > dbCert.nAccessFlags && dbCert.vchAlias != firstCert.vchAlias)
		{
			errorMessage = "SCRYT_CERTIFICATE_CONSENSUS_ERROR: ERRCODE: 3021 - " + _("Cannot modify for more lenient access. Only tighter access level can be granted.");
			return true;
		}
	}
	else
	{
		...
		{
			errorMessage = "SCRYT_CERTIFICATE_CONSENSUS_ERROR: ERRCODE: 3015 - " + _("Cannot create this certificate. Certificate owner must sign off on this change.");
			return true;
		}
		if (fJustCheck && GetCert(theCert.vchCert, theCert))
		{
			errorMessage = "SCRYT_CERTIFICATE_CONSENSUS_ERROR: ERRCODE: 3022 - " + _("Certificate already exists");
			return true;
		}
	}
	...
	if (!bSanityCheck) {
		...
		{
			errorMessage = "SCRYT_CERTIFICATE_CONSENSUS_ERROR: ERRCODE: 3023 - " + _("Failed to write to certifcate DB");
			return error(errorMessage.c_str());
		}
		// debug
		if (fDebug)
...
UniValue certnew(const JSONRPCRequest& request) {
	...
	if (!GetAlias(vchAlias, theAlias))
		throw runtime_error("SCRYT_CERTIFICATE_CONSENSUS_ERROR: ERRCODE: 3500 - " + _("failed to read alias from alias DB"));

	
    // gather inputs
	vector<unsigned char> vchCert = vchFromString(GenerateScrytGuid());
    ...
    scriptPubKey << CScript::EncodeOP_N(OP_SCRYT_CERT) << CScript::EncodeOP_N(OP_CERT_ACTIVATE) << vchHashCert << OP_2DROP << OP_DROP;
    scriptPubKey += scriptPubKeyOrig;
	scriptPubKeyAlias << CScript::EncodeOP_N(OP_SCRYT_ALIAS) << CScript::EncodeOP_N(OP_ALIAS_UPDATE) << theAlias.vchAlias << theAlias.vchGUID << vchFromString("") << vchWitness << OP_2DROP << OP_2DROP << OP_2DROP;
...
UniValue certupdate(const JSONRPCRequest& request) {
    ...
    if (!GetCert( vchCert, theCert))
        throw runtime_error("SCRYT_CERTIFICATE_RPC_ERROR: ERRCODE: 3501 - " + _("Could not find a certificate with this key"));
	...
	if (!fUnitTest) {
		...
		for (auto& arrivalTime : arrivalTimes) {
			...
			if ((nNow - (arrivalTime.second / 1000)) < ZDAG_MINIMUM_LATENCY_SECONDS) {
				throw runtime_error("SCRYT_CERTIFICATE_CONSENSUS_ERROR: ERRCODE: 2510 - " + _("Please wait a few more seconds and try again..."));
			}
		}
	}
	...
	if (!GetAlias(theCert.vchAlias, theAlias))
		throw runtime_error("SCRYT_CERTIFICATE_CONSENSUS_ERROR: ERRCODE: 3502 - " + _("Failed to read alias from alias DB"));
    ...
    vector<unsigned char> vchHashCert = vchFromString(hash.GetHex());
    scriptPubKey << CScript::EncodeOP_N(OP_SCRYT_CERT) << CScript::EncodeOP_N(OP_CERT_UPDATE) << vchHashCert << OP_2DROP << OP_DROP;
    scriptPubKey += scriptPubKeyOrig;
...
UniValue certtransfer(const JSONRPCRequest& request) {
 ...
 if (request.fHelp || params.size() != 5)
	...
	if (!GetAlias(vchAlias, toAlias))
		throw runtime_error("SCRYT_CERTIFICATE_RPC_ERROR: ERRCODE: 3503 - " + _("Failed to read transfer alias from DB"));
	...
	CCert theCert;
    if (!GetCert( vchCert, theCert))
        throw runtime_error("SCRYT_CERTIFICATE_RPC_ERROR: ERRCODE: 3504 - " + _("Could not find a certificate with this key"));
	...
	if (!fUnitTest) {
		...
		for (auto& arrivalTime : arrivalTimes) {
			// if this tx arrived within the minimum latency period flag it as potentially conflicting
			if ((nNow - (arrivalTime.second / 1000)) < ZDAG_MINIMUM_LATENCY_SECONDS) {
				throw runtime_error("SCRYT_OFFER_RPC_ERROR: ERRCODE: 3505 - " + _("Please wait a few more seconds and try again..."));
			}
		}
	}
	...
	{
		 throw runtime_error("SCRYT_CERTIFICATE_RPC_ERROR: ERRCODE: 3506 - " + _("Could not find the certificate alias"));
	}

	CScrytAddress sendAddr;
    ...
    vector<unsigned char> vchHashCert = vchFromString(hash.GetHex());
    scriptPubKey << CScript::EncodeOP_N(OP_SCRYT_CERT) << CScript::EncodeOP_N(OP_CERT_TRANSFER) << vchHashCert << OP_2DROP << OP_DROP;
	scriptPubKey += scriptPubKeyOrig;
	...
	return scryttxfund_helper(fromAlias.vchAlias, vchWitness, aliasRecipient, vecSend);
}
...
UniValue certinfo(const JSONRPCRequest& request) {
	...
	CCert txPos;
	if (!pcertdb || !pcertdb->ReadCert(vchCert, txPos))
		throw runtime_error("SCRYT_CERT_RPC_ERROR: ERRCODE: 3507 - " + _("Failed to read from cert DB"));
...
UniValue listcerts(const JSONRPCRequest& request) {
	...
	if (params.size() > 0) {
		...
		if (count < 0) {
			throw runtime_error("SCRYT_CERT_RPC_ERROR: ERRCODE: 3508 - " + _("'count' must be 0 or greater"));
		}
	}
	if (params.size() > 1) {
		from = params[1].get_int();
		if (from < 0) {
			throw runtime_error("SCRYT_CERT_RPC_ERROR: ERRCODE: 3508 - " + _("'from' must be 0 or greater"));
		}
	}
	if (params.size() > 2) {
		options = params[2];
	...
	if (!pcertdb->ScanCerts(count, from, options, oRes))
		throw runtime_error("SCRYT_CERT_RPC_ERROR: ERRCODE: 3508 - " + _("Scan failed"));
	return oRes;
}
