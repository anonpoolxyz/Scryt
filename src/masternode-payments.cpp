...
*
*   Why is this needed?
*   - In Scryt some blocks are superblocks, which output much higher amounts of coins
...
{
    if(fLiteMode) return; // disable all Scryt specific functionality
    ...
    if (strCommand == NetMsgType::MASTERNODEPAYMENTSYNC) { //Masternode Payments Request Sync
		...
		// SCRYT update last vote after sig check
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew, const int64_t &nHeight, const CAmount& fee, CAmount& nTotalRewardWithMasternodes) const
{
    ...
    LogPrintf("CMasternodeBlockPayees::IsTransactionValid -- ERROR: Missing required payment, possible payees: '%s', amount: %f SCY\n", strPayeesPossible, (float)nMasternodePayment/COIN);
    return false;
}
