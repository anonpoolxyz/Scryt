...
#ifndef SCRYT_VALIDATION_H
#define SCRYT_VALIDATION_H
...
static const int64_t DEFAULT_MAX_TIP_AGE = 2.5 * 6 * 60 * 60; // ~144 blocks behind -> 2 x fork detection time, was 24 * 60 * 60 in scryt
...
bool CheckScrytInputs(const CTransaction& tx, CValidationState &state, const CCoinsViewCache &inputs, bool fJustCheck, int nHeight, const CBlock& block, bool bSanity = false);
...
// SCRYT add it here so chain.cpp can access
