...
bool FindScrytScriptOp(const CScript& script, int& op) {
	...
	return op == OP_SCRYT_ALIAS || op == OP_SCRYT_ASSET || op == OP_SCRYT_ASSET_ALLOCATION || op == OP_SCRYT_CERT || op == OP_SCRYT_ESCROW || op == OP_SCRYT_OFFER;
}
bool GetTimeToPrune(const CScript& scriptPubKey, uint64_t &nTime)
{
	vector<unsigned char> vchData;
	vector<unsigned char> vchHash;
	if(!GetScrytData(scriptPubKey, vchData, vchHash))
		return false;
	if(!chainActive.Tip())
		return false;
	CAliasIndex alias;
...
bool IsScrytScript(const CScript& scriptPubKey, int &op, vector<vector<unsigned char> > &vvchArgs)
{
...
bool RemoveScrytScript(const CScript& scriptPubKeyIn, CScript& scriptPubKeyOut)
{
...
{   
	if (IsPaymentOptionInMask(paymentOptionMask, PAYMENTOPTION_BTC))
		return 250;
	else  if (IsPaymentOptionInMask(paymentOptionMask, PAYMENTOPTION_SCY))
		return 25;
...
int GetScrytDataOutput(const CTransaction& tx) {
   for(unsigned int i = 0; i<tx.vout.size();i++) {
	   if(IsScrytDataOutput(tx.vout[i]))
		   return i;
	}
   return -1;
}
bool IsScrytDataOutput(const CTxOut& out) {
...
bool CheckAliasInputs(const CCoinsViewCache &inputs, const CTransaction &tx, int op, const vector<vector<unsigned char> > &vvchArgs, bool fJustCheck, int nHeight, string &errorMessage, bool bSanityCheck) {
	...
	for (unsigned int i = 0; i < tx.vout.size(); i++) {
		int pop;
		if (!FindScrytScriptOp(tx.vout[i].scriptPubKey, pop))
			continue;
		if (pop != OP_SCRYT_ALIAS) {
			aliasData = false;
		}
	}
	// if it has alias data, get it and unserialize the alias from data output
	if (aliasData) {
		bool bData = GetScrytData(tx, vchData, vchHash, nDataOut);
		if (bData)
	...
	if(fJustCheck)
	{
		...
		if(vvchArgs.size() != 4)
		{
			errorMessage = "SCRYT_ALIAS_CONSENSUS_ERROR: ERRCODE: 5000 - " + _("Alias arguments incorrect size");
		...
		if(!theAlias.IsNull())
		{
			...
			{
				errorMessage = "SCRYT_ALIAS_CONSENSUS_ERROR: ERRCODE: 5001 - " + _("Hash provided doesn't match the calculated hash of the data");
				return true;
			}
		}					
	...
	{
		errorMessage = "SCRYT_ALIAS_CONSENSUS_ERROR: ERRCODE: 5002 - " + _("Too many outputs for this Scryt transaction");
		return error(errorMessage.c_str());
	}
	Coin prevCoins;
	if(fJustCheck || op != OP_ALIAS_ACTIVATE)
	{
		...
		if(vvchArgs.size() >= 4 && !vvchArgs[3].empty())
		{
			...
			if(!bWitnessSigFound)
			{
				errorMessage = "SCRYT_ALIAS_CONSENSUS_ERROR: ERRCODE: 5003 - " + _("Witness signature not found");
				return error(errorMessage.c_str());
			}
		}
	}
	...
	if(fJustCheck)
	{
		...
		{
			errorMessage = "SCRYT_ALIAS_CONSENSUS_ERROR: ERRCODE: 5004 - " + _("Alias name does not follow the domain name specification");
			return error(errorMessage.c_str());
		}
		if(theAlias.vchPublicValue.size() > MAX_VALUE_LENGTH)
		{
			errorMessage = "SCRYT_ALIAS_CONSENSUS_ERROR: ERRCODE: 5005 - " + _("Alias public value too big");
			return error(errorMessage.c_str());
		}
		if(theAlias.vchEncryptionPrivateKey.size() > MAX_ENCRYPTED_GUID_LENGTH)
		{
			errorMessage = "SCRYT_ALIAS_CONSENSUS_ERROR: ERRCODE: 5006 - " + _("Encryption private key too long");
			return error(errorMessage.c_str());
		}
		if(theAlias.vchEncryptionPublicKey.size() > MAX_ENCRYPTED_GUID_LENGTH)
		{
			errorMessage = "SCRYT_ALIAS_CONSENSUS_ERROR: ERRCODE: 5007 - " + _("Encryption public key too long");
			return error(errorMessage.c_str());
		}
		switch (op) {
			case OP_ALIAS_ACTIVATE:
				...
				{
					errorMessage = "SCRYT_ALIAS_CONSENSUS_ERROR: ERRCODE: 5008 - " + _("Alias input to this transaction not found");
					return error(errorMessage.c_str());
				}
				// Check new/activate hash
				if (vvchPrevArgs.size() <= 0 || vvchPrevArgs[0] != vchHash)
				{
					errorMessage = "SCRYT_ALIAS_CONSENSUS_ERROR: ERRCODE: 5009 - " + _("Alias new and activate hash mismatch");
					return error(errorMessage.c_str());
				}
				if (vvchArgs.size() <= 1 || theAlias.vchGUID != vvchArgs[1])
				{
					errorMessage = "SCRYT_ALIAS_CONSENSUS_ERROR: ERRCODE: 5010 - " + _("Alias input guid mismatch");
					return error(errorMessage.c_str());
				}
				if(theAlias.vchAddress.empty())
				{
					errorMessage = "SCRYT_ALIAS_CONSENSUS_ERROR: ERRCODE: 5011 - " + _("Alias address cannot be empty");
					return error(errorMessage.c_str());
				}
				break;
			case OP_ALIAS_UPDATE:
				if (!IsAliasOp(prevOp))
				{
					errorMessage = "SCRYT_ALIAS_CONSENSUS_ERROR: ERRCODE: 5012 - " + _("Alias input to this transaction not found");
					return error(errorMessage.c_str());
				}
				if (!theAlias.IsNull())
				{
					...
					{
						errorMessage = "SCRYT_ALIAS_CONSENSUS_ERROR: ERRCODE: 5013 - " + _("Guid in data output doesn't match guid in transaction");
						return error(errorMessage.c_str());
					}
					if (vvchArgs.size() <= 1 || theAlias.vchGUID != vvchArgs[1])
					{
						errorMessage = "SCRYT_ALIAS_CONSENSUS_ERROR: ERRCODE: 5014 - " + _("Alias input guid mismatch");
						return error(errorMessage.c_str());
					}
				}
				break;
		default:
				errorMessage = "SCRYT_ALIAS_CONSENSUS_ERROR: ERRCODE: 5015 - " + _("Alias transaction has unknown op");
				return error(errorMessage.c_str());
		}
	...
	if (!GetAlias(vchAlias, dbAlias))
	{
		...
		{
			errorMessage = "SCRYT_ALIAS_CONSENSUS_ERROR: ERRCODE: 5016 - " + _("Failed to read from alias DB");
			return true;
		}
	}
	if (fJustCheck) {
		if (op == OP_ALIAS_UPDATE) {
			...
			{
				errorMessage = "SCRYT_ALIAS_CONSENSUS_ERROR: ERRCODE: 5018 - " + _("Cannot extract destination of alias input");
				return error(errorMessage.c_str());
			}
			else
			{
				CScrytAddress prevaddy(aliasDest);
				if (EncodeBase58(dbAlias.vchAddress) != prevaddy.ToString())
				{
					errorMessage = "SCRYT_ALIAS_CONSENSUS_ERROR: ERRCODE: 5019 - " + _("You are not the owner of this alias");
					return error(errorMessage.c_str());
		...
		if (!theAlias.IsNull() && theAlias.offerWhitelist.entries.empty() && theAlias.nExpireTime > 0)
		{
			...
			{
				errorMessage = "SCRYT_ALIAS_CONSENSUS_ERROR: ERRCODE: 5017 - " + _("Transaction does not pay enough fee: ") + ValueFromAmount(tx.vout[nDataOut].nValue).write() + "/" + ValueFromAmount(fee - 10000).write() + "/" + boost::lexical_cast<string>(fYears) + " years.";
				return true;
			}
		}
		...
		CTransaction txTmp;
		GetScrytTransactionDescription(txTmp, op, strResponseEnglish, ALIAS, strResponseGUID);
		...
		if (op == OP_ALIAS_UPDATE)
		{
			...
			{
				errorMessage = "SCRYT_ALIAS_CONSENSUS_ERROR: ERRCODE: 5020 - " + _("Cannot edit this alias, guid mismatch");
				if (!theAliasNull)
					theAlias = dbAlias;
			...
			if (!theAliasNull) {
				...
				else
				{
					...
					if (dbAlias.vchAddress != theAlias.vchAddress)
					{
						...
						if (paliasdb->ExistsAddress(theAlias.vchAddress))
						{
							...
							if (paliasdb->ReadAddress(theAlias.vchAddress, vchMyAlias) && !vchMyAlias.empty() && vchMyAlias != dbAlias.vchAlias)
							{
								...
								if (GetAlias(vchMyAlias, dbReadAlias)) {
									errorMessage = "SCRYT_ALIAS_CONSENSUS_ERROR: ERRCODE: 5021 - " + _("An alias already exists with that address, try another public key");
									theAlias = dbAlias;
								}
							}
						}
						...
						{
							errorMessage = "SCRYT_ALIAS_CONSENSUS_ERROR: ERRCODE: 5022 - " + _("Cannot edit this alias. Insufficient privileges.");
							theAlias = dbAlias;
						}
					...
					else
					{
						...
						{
							errorMessage = "SCRYT_ALIAS_CONSENSUS_ERROR: ERRCODE: 5023 - " + _("Cannot edit this alias. It is view-only.");
							theAlias = dbAlias;
						}
					}
					if (theAlias.nAccessFlags > dbAlias.nAccessFlags)
					{
						errorMessage = "SCRYT_ALIAS_CONSENSUS_ERROR: ERRCODE: 5024 - " + _("Cannot modify for more lenient access. Only tighter access level can be granted.");
						theAlias = dbAlias;
					}
				}
				...
				if (whiteList.entries.size() >= 1)
				{
					...
					{
						errorMessage = "SCRYT_ALIAS_CONSENSUS_ERROR: ERRCODE: 5025 -" + _("Too many affiliates for this whitelist, maximum 20 entries allowed");
					...
					else if (whiteList.entries.size() == 1 && whiteList.entries.begin()->second.nDiscountPct == 127)
					{
						...
						{
							errorMessage = "SCRYT_ALIAS_CONSENSUS_ERROR: ERRCODE: 5026 - " + _("Whitelist is already empty");
						}
						else
							theAlias.offerWhitelist.SetNull();
					}
					else
					{
						for (auto const &it : whiteList.entries)
						{
							...
							if (newEntry.nDiscountPct > 99) {
								errorMessage = "SCRYT_ALIAS_CONSENSUS_ERROR: ERRCODE: 5027 -" + _("Whitelist discount must be between 0 and 99");
								continue;
							}
							...
							else
							{
								...
								{
									errorMessage = "SCRYT_ALIAS_CONSENSUS_ERROR: ERRCODE: 5028 -" + _("Too many affiliates for this whitelist, maximum 20 entries allowed");
								}
							}
						}
					}
		...
		else if (op == OP_ALIAS_ACTIVATE)
		{
			if (!dbAlias.IsNull())
			{
				errorMessage = "SCRYT_ALIAS_CONSENSUS_ERROR: ERRCODE: 5029 - " + _("Trying to renew an alias that isn't expired");
				return true;
			}
			if (paliasdb->ExistsAddress(theAlias.vchAddress) && chainActive.Tip()->GetMedianTimePast() < GetAliasExpiration(theAlias))
			{
				errorMessage = "SCRYT_ALIAS_CONSENSUS_ERROR: ERRCODE: 5030 - " + _("Trying to create an alias with an address of an alias that isn't expired");
				return true;
			}
		}
		if (!theAliasNull)
		{
			...
			if (!bSanityCheck) {
				...
				{
					errorMessage = "SCRYT_ALIAS_CONSENSUS_ERROR: ERRCODE: 5031 - " + _("Failed to write to alias DB");
					return error(errorMessage.c_str());
				}
...
bool GetScrytData(const CTransaction &tx, vector<unsigned char> &vchData, vector<unsigned char> &vchHash, int& nOut)
{
	nOut = GetScrytDataOutput(tx);
    if(nOut == -1)
	   return false;

	const CScript &scriptPubKey = tx.vout[nOut].scriptPubKey;
	return GetScrytData(scriptPubKey, vchData, vchHash);
}
bool IsValidAliasName(const std::vector<unsigned char> &vchAlias)
{
	return (vchAlias.size() <= 71 && vchAlias.size() >= 3);
}
bool GetScrytData(const CScript &scriptPubKey, vector<unsigned char> &vchData, vector<unsigned char> &vchHash)
{
...
void GetAddress(const CAliasIndex& alias, CScrytAddress* address,CScript& script,const uint32_t nPaymentOption)
{
	if(!address)
		return;
	CChainParams::AddressType myAddressType = PaymentOptionToAddressType(nPaymentOption);
	CScrytAddress addrTmp = CScrytAddress(EncodeBase58(alias.vchAddress));
	address[0] = CScrytAddress(addrTmp.Get(), myAddressType);
...
void CleanupScrytServiceDatabases(int &numServicesCleaned)
{
	if(pofferdb != NULL)
...
bool GetAliasOfTx(const CTransaction& tx, vector<unsigned char>& name) {
	if (tx.nVersion != SCRYT_TX_VERSION)
		return false;
	...
	if (!good)
		return error("GetAliasOfTx() : could not decode a scryt tx");
...
bool DecodeAndParseScrytTx(const CTransaction& tx, int& op,
...
bool FindAssetOwnerInTx(const CCoinsViewCache &inputs, const CTransaction& tx, const string& ownerAddressToMatch) {
	...
	for (unsigned int i = 0; i < tx.vin.size(); i++) {
		...
		if (!ExtractDestination(prevCoins.out.scriptPubKey, dest))
			continue;
		if (CScrytAddress(dest).ToString() == ownerAddressToMatch) {
			return true;
		}
	}
	return false;
...
void CAliasDB::WriteAliasIndex(const CAliasIndex& alias, const int &op) {
	if (IsArgSet("-zmqpubaliasrecord")) {
		...
		CScrytAddress address(EncodeBase58(alias.vchAddress));
		...
		GetMainSignals().NotifyScrytUpdate(oName.write().c_str(), "aliasrecord");
	}
...
void CAliasDB::WriteAliasIndexHistory(const CAliasIndex& alias, const int &op) {
	if (IsArgSet("-zmqpubaliashistory")) {
		...
		GetMainSignals().NotifyScrytUpdate(oName.write().c_str(), "aliashistory");
	}
}
...
void CAliasDB::WriteAliasIndexTxHistory(const string &user1, const string &user2, const string &user3, const uint256 &txHash, const unsigned int& nHeight, const string &type, const string &guid) {
	if (IsArgSet("-zmqpubaliastxhistory")) {
		...
		GetMainSignals().NotifyScrytUpdate(oName.write().c_str(), "aliastxhistory");
	}
}
UniValue ScrytListReceived(bool includeempty=true)
{
	if (!pwalletMain)
		return NullUniValue;
	...
	BOOST_FOREACH(const PAIRTYPE(CScrytAddress, CAddressBookData)& item, pwalletMain->mapAddressBook)
	{
		const CScrytAddress& address = item.first;
	...
	BOOST_FOREACH(const COutput& out, vecOutputs) {
		...
		CScrytAddress sysAddress(address);
...
UniValue scryttxfund_helper(const vector<unsigned char> &vchAlias, const vector<unsigned char> &vchWitness, const CRecipient &aliasRecipient, vector<CRecipient> &vecSend) {
	CMutableTransaction txNew;
	txNew.nVersion = SCRYT_TX_VERSION;
	...
	if (!vchWitness.empty())
	{
		...
		{
			throw runtime_error("SCRYT_RPC_ERROR ERRCODE: 9000 - " + _("This transaction requires a witness but not enough outputs found for witness alias: ") + stringFromVch(vchWitness));
		}
		Coin pcoinW;
	...
	if(vchWitness == vchAlias || (!aliasOutPointWitness.IsNull() && aliasOutPointWitness == aliasOutPoint))
		throw runtime_error("SCRYT_RPC_ERROR ERRCODE: 9000 - " + _("Witness to this transaction must be different than the funding alias"));
	// set an address for scryttxfund so it uses that address to fund (alias passed in)
	string strAddress;
	if (!aliasRecipient.scriptPubKey.empty()) {
		CAliasIndex alias;
		if (!GetAlias(vchAlias, alias))
			throw runtime_error("SCRYT_RPC_ERROR ERRCODE: 9000 - " + _("Cannot find alias used to fund this transaction: ") + stringFromVch(vchAlias));
		strAddress = EncodeBase58(alias.vchAddress);
	}
...
UniValue scryttxfund(const JSONRPCRequest& request) {
	const UniValue &params = request.params;
	if (request.fHelp || 1 > params.size() || 3 < params.size())
		throw runtime_error(
			"scryttxfund\n"
			"\nFunds a new scryt transaction with inputs used from wallet or an array of addresses specified.\n"
			"\nArguments:\n"
			"  \"hexstring\" (string, required) The raw scryt transaction output given from rpc (ie: aliasnew, aliasupdate)\n"
			...
			"}\n"
			"\nExamples:\n"
			+ HelpExampleCli("scryttxfund", " <hexstring> '{\"addresses\": [\"175tWpb8K1S7NmH4Zx6rewF9WQrcZv245W\"]}' false")
			+ HelpExampleRpc("scryttxfund", " <hexstring> {\"addresses\": [\"175tWpb8K1S7NmH4Zx6rewF9WQrcZv245W\"]} true")
	...
	if (!DecodeHexTx(tx, hexstring))
		throw runtime_error("SCRYT_ALIAS_RPC_ERROR: ERRCODE: 5500 - " + _("Could not send raw transaction: Cannot decode transaction from hex string"));
	...
	if(params.size() > 1)
		...
		UniValue receivedList = ScrytListReceived(false);
	...
	if (resUTXOs.isArray())
		...
		throw runtime_error("SCRYT_ALIAS_RPC_ERROR: ERRCODE: 5501 - " + _("No funds found in addresses provided"));
	...
	if (fUseInstantSend && nDesiredAmount > sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE)*COIN) {
		throw runtime_error(_("InstantSend doesn't support sending values that high yet. Transactions are currently limited to 8000 SCY."));
	}
	...
	if (tx.nVersion == SCRYT_TX_VERSION && !DecodeAliasTx(tx, op, vvchAlias))
	{
	...
	for (auto& vin : tx.vin) {
		...
		if(tx.nVersion == SCRYT_TX_VERSION && params.size() > 1)
	...
	if (nCurrentAmount < (nDesiredAmount + nFees)) {
		...
		if (tx.nVersion == SCRYT_TX_VERSION && params.size() > 1 && !fUseInstantSend) {
			COutPoint aliasOutPoint;
	...
	if (nChange < 0)
		throw runtime_error("SCRYT_ALIAS_RPC_ERROR: ERRCODE: 5502 - " + _("Insufficient funds"));
	...
	if (params.size() > 1) {
		...
		CScrytAddress addressLast(values.back().get_str());
	...
	if (tx.nVersion == SCRYT_TX_VERSION) {
		...
		if (!CheckScrytInputs(tx, state, view, true, 0, CBlock(), true))
			throw runtime_error(FormatStateMessage(state));
		if (!CheckScrytInputs(tx, state, view, false, 0, CBlock(), true))
			throw runtime_error(FormatStateMessage(state));
	}
...
UniValue aliasnew(const JSONRPCRequest& request) {
	...
	The domain name should not start or end with hyphen (-) (e.g. -scryt.org or scryt-.org)
	...
	if (find_first(strName, "."))
	{
		if (!regex_search(strName, nameparts, domainwithtldregex) || string(nameparts[0]) != strName)
			throw runtime_error("SCRYT_ALIAS_RPC_ERROR: ERRCODE: 5503 - " + _("Invalid Scryt Identity. Must follow the domain name spec of 3 to 64 characters with no preceding or trailing dashes and a TLD of 2 to 6 characters"));
	}
	else
	{
		if (!regex_search(strName, nameparts, domainwithouttldregex) || string(nameparts[0]) != strName)
			throw runtime_error("SCRYT_ALIAS_RPC_ERROR: ERRCODE: 5504 - " + _("Invalid Scryt Identity. Must follow the domain name spec of 3 to 64 characters with no preceding or trailing dashes"));
	}
	...
	CMutableTransaction tx;
	tx.nVersion = SCRYT_TX_VERSION;
	tx.vin.clear();
	tx.vout.clear();
	CAliasIndex oldAlias;
	if (GetAlias(vchAlias, oldAlias))
		throw runtime_error("SCRYT_ALIAS_RPC_ERROR: ERRCODE: 5505 - " + _("This alias already exists"));


	const vector<unsigned char> &vchRandAlias = vchFromString(GenerateScrytGuid());
	...
	if (strAddress.empty() && !foundRegistration)
	{
		...
		CScrytAddress addressAlias(pubKey.GetID());
		strAddress = addressAlias.ToString();
		if (pwalletMain && !pwalletMain->AddKeyPubKey(privKey, pubKey))
			throw runtime_error("SCRYT_ALIAS_RPC_ERROR: ERRCODE: 5506 - " + _("Error adding key to wallet"));
	}
	...
	if (foundRegistration)
	{
		...
		if (!newAlias.UnserializeFromData(data, vchHashAlias))
			throw runtime_error("SCRYT_ALIAS_RPC_ERROR: ERRCODE: 5507 - " + _("Cannot unserialize alias registration transaction"));
		if (strAddress.empty())
	...
	if (bActivation)
		scriptPubKey << CScript::EncodeOP_N(OP_SCRYT_ALIAS) << CScript::EncodeOP_N(OP_ALIAS_ACTIVATE) << vchAlias << newAlias1.vchGUID << vchHashAlias1 << vchWitness << OP_2DROP << OP_2DROP << OP_2DROP;
	else
		scriptPubKey << CScript::EncodeOP_N(OP_SCRYT_ALIAS) << CScript::EncodeOP_N(OP_ALIAS_ACTIVATE) << vchHashAlias1 << OP_2DROP << OP_DROP;

	CScrytAddress newAddress;
	...
	if (bActivation && it != mapAliasRegistrations.end())
	{
		...
		if (pcoin.IsSpent()) {
			throw runtime_error("SCRYT_ALIAS_RPC_ERROR: ERRCODE: 5508 - " + _("Cannot find alias registration transaction, please ensure it has confirmed or re-submit the registration transaction again"));
		}
		...
		if (!vchWitness.empty())
		{
			...
			if (aliasOutPointWitness.IsNull())
			{
				throw runtime_error("SCRYT_RPC_ERROR ERRCODE: 5509 - " + _("This transaction requires a witness but not enough outputs found for witness alias: ") + stringFromVch(vchWitness));
			}
			const Coin &pcoinW = view.AccessCoin(aliasOutPointWitness);
			if (pcoinW.IsSpent()) {
				throw runtime_error("SCRYT_ALIAS_RPC_ERROR: ERRCODE: 5510 - " + _("Cannot find witness transaction"));
			}
...
UniValue aliasnewestimatedfee(const JSONRPCRequest& request) {
	...
	CAliasIndex oldAlias;
	if (GetAlias(vchAlias, oldAlias))
		throw runtime_error("SCRYT_ALIAS_RPC_ERROR: ERRCODE: 5506 - " + _("This alias already exists"));
	...
	scriptPubKey << CScript::EncodeOP_N(OP_SCRYT_ALIAS) << CScript::EncodeOP_N(OP_ALIAS_ACTIVATE) << vchAlias << newAlias1.vchGUID << vchHashAlias1 << vchWitness << OP_2DROP << OP_2DROP << OP_2DROP;
	scriptPubKey1<< CScript::EncodeOP_N(OP_SCRYT_ALIAS) << CScript::EncodeOP_N(OP_ALIAS_ACTIVATE) << vchHashAlias1 << OP_2DROP << OP_DROP;
	// setup dummy destination for fee calculation purposes
	CScrytAddress newAddress = CScrytAddress("1QFqqMUD55ZV3PJEJZtaKCsQmjLT6JkjvJ");
...
UniValue aliasupdate(const JSONRPCRequest& request) {
	...
	if (!GetAlias(vchAlias, theAlias))
		throw runtime_error("SCRYT_ALIAS_RPC_ERROR: ERRCODE: 5511 - " + _("Could not find an alias with this name"));
	...
	CScript scriptPubKey;
	scriptPubKey << CScript::EncodeOP_N(OP_SCRYT_ALIAS) << CScript::EncodeOP_N(OP_ALIAS_UPDATE) << copyAlias.vchAlias << copyAlias.vchGUID << vchHashAlias << vchWitness << OP_2DROP << OP_2DROP << OP_2DROP;
	scriptPubKey += scriptPubKeyOrig;
	...
	return scryttxfund_helper(vchAlias, vchWitness, recipient, vecSend);
}
UniValue aliasupdateestimatedfee(const JSONRPCRequest& request) {
	...
	const UniValue &txHexArray = scryttxfund_helper(vchAlias, vchWitness, recipient, vecSend);
	CMutableTransaction tx;
...
UniValue scrytdecoderawtransaction(const JSONRPCRequest& request) {
	const UniValue &params = request.params;
	if (request.fHelp || 1 != params.size())
		throw runtime_error("scrytdecoderawtransaction <hexstring>\n"
		"Decode raw scryt transaction (serialized, hex-encoded) and display information pertaining to the service that is included in the transactiion data output(OP_RETURN)\n"
	...
	if(rawTx.IsNull())
	{
		throw runtime_error("SCRYT_RPC_ERROR: ERRCODE: 5512 - " + _("Could not decode transaction"));
	}
	vector<unsigned char> vchData;
	int nOut;
	int op;
	vector<vector<unsigned char> > vvch;
	vector<unsigned char> vchHash;
	GetScrytData(rawTx, vchData, vchHash, nOut);	
	UniValue output(UniValue::VOBJ);
	char type;
	if(DecodeAndParseScrytTx(rawTx, op,  vvch, type))
...
UniValue scrytsendrawtransaction(const JSONRPCRequest& request) {
	const UniValue &params = request.params;
	if (request.fHelp || params.size() < 1 || params.size() > 3)
		throw runtime_error("scrytsendrawtransaction \"hexstring\" ( allowhighfees instantsend )\n"
	...
	if (!DecodeHexTx(txIn, hexstring))
		throw runtime_error("SCRYT_ALIAS_RPC_ERROR: ERRCODE: 5513 - " + _("Could not send raw transaction: Cannot decode transaction from hex string"));
	CTransaction tx(txIn);
	if (tx.vin.size() <= 0)
		throw runtime_error("SCRYT_ALIAS_RPC_ERROR: ERRCODE: 5514 - " + _("Could not send raw transaction: Inputs are empty"));
	if (tx.vout.size() <= 0)
		throw runtime_error("SCRYT_ALIAS_RPC_ERROR: ERRCODE: 5515 - " + _("Could not send raw transaction: Outputs are empty"));
	...
	if (!returnRes.isStr())
		throw runtime_error("SCRYT_ALIAS_RPC_ERROR: ERRCODE: 5516 - " + _("Could not send raw transaction: Invalid response from sendrawtransaction"));
...
string GenerateScrytGuid()
{
	int64_t rand = GetRand(std::numeric_limits<int64_t>::max());
	vector<unsigned char> vchGuidRand = CScriptNum(rand).getvch();
	return HexStr(vchGuidRand);
}
UniValue prunescrytservices(const JSONRPCRequest& request)
{
	const UniValue &params = request.params;
	if (request.fHelp || params.size() > 0)
		throw runtime_error(
			"prunescrytservices\n"
			"\nPrune expired Scryt service data from the internal database.\n"
			+ HelpExampleCli("prunescrytservices", "")
		);
	int servicesCleaned = 0;
	CleanupScrytServiceDatabases(servicesCleaned);
	UniValue res(UniValue::VOBJ);
	res.push_back(Pair("services_cleaned", servicesCleaned));
	if (fDebug)
		LogPrintf("prunescrytservices # cleaned: %d\n", servicesCleaned);
	return res;
}
UniValue aliasbalancemulti(const JSONRPCRequest& request)
{
	...
	if (request.fHelp || params.size() < 1 || params.size() > 2)
		throw runtime_error(
			...
			"\nArguments:\n"
			"1. \"aliases\"  (array, required) The scryt aliases to find balances for. Must be an array.\n"
...
UniValue aliasbalance(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"alias\"  (string, required) The scryt alias for transactions.\n"
			"2. \"instantsend\"  (boolean, optional) Check for balance available to instant send. Default is false.\n"
       );
...
UniValue aliasinfo(const JSONRPCRequest& request) {
	...
	CAliasIndex txPos;
	if (!paliasdb)
		throw runtime_error("SCRYT_ALIAS_RPC_ERROR: ERRCODE: 5517 - " + _("Failed to read from alias DB"));

	UniValue oName(UniValue::VOBJ);
	if(!paliasdb->ReadAlias(vchAlias, txPos) || !BuildAliasJson(txPos, oName))
		throw runtime_error("SCRYT_ALIAS_RPC_ERROR: ERRCODE: 5518 - " + _("Could not find this alias"));
...
UniValue aliaspay(const JSONRPCRequest& request) {
    ...
    if (request.fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\":amount   (numeric or string) The scryt address is the key, the numeric amount (can be string) in SCY is the value\n"
            "      ,...\n"
            "    }\n"
			...
			"                           Those recipients will receive less scryts than you enter in their corresponding amount field.\n"
	...
	if (!GetAlias(vchFromString(strFrom), theAlias))
		throw runtime_error("SCRYT_ALIAS_RPC_ERROR: ERRCODE: 5519 - " + _("Invalid fromalias"));
    ...
    {
        CScrytAddress address(name_);
		vector<unsigned char> vchPubKey;
		string strAddress = name_;
		if (GetAddressFromAlias(name_, strAddress, vchPubKey))
		{
			address = CScrytAddress(strAddress);
		}
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid Scryt address: ")+name_);
...
UniValue aliasupdatewhitelist(const JSONRPCRequest& request) {
	...
	if (!GetAlias(vchOwnerAlias, theAlias))
		throw runtime_error("SCRYT_ALIAS_RPC_ERROR ERRCODE: 5520 - " + _("Could not find an alias with this guid"));

	CScrytAddress aliasAddress;
	...
	for (unsigned int idx = 0; idx < whitelistEntries.size(); idx++) {
		...
		if (!theAlias.offerWhitelist.GetLinkEntryByHash(vchFromString(aliasEntryName), entry))
			throw runtime_error("SCRYT_ALIAS_RPC_ERROR ERRCODE: 5521 - " + _("This alias entry was not added to affiliate list: ") + aliasEntryName);
	}
	vector<unsigned char> data;
	theAlias.Serialize(data);
	uint256 hash = Hash(data.begin(), data.end());
	...
	return scryttxfund_helper(copyAlias.vchAlias, vchWitness, recipient, vecSend);
}
UniValue aliasclearwhitelist(const JSONRPCRequest& request) {
	...
	ToLowerCase(vchAlias);
	if (!GetAlias(vchAlias, theAlias))
		throw runtime_error("SCRYT_ALIAS_RPC_ERROR ERRCODE: 5522 - " + _("Could not find an alias with this name"));
...
UniValue scrytlistreceivedbyaddress(const JSONRPCRequest& request)
{
	const UniValue &params = request.params;
	if (request.fHelp || params.size() != 0)
		throw runtime_error(
			"scrytlistreceivedbyaddress\n"
			...
			"\nExamples:\n"
			+ HelpExampleCli("scrytlistreceivedbyaddress", "")
		);

	return ScrytListReceived();
}
...
string GetScrytTransactionDescription(const CTransaction& tx, const int op, string& responseEnglish, const char &type, string& responseGUID)
{
	if (tx.IsNull()) {
		return "Null Tx";
	}
...
UniValue listaliases(const JSONRPCRequest& request) {
	...
	if (params.size() > 0) {
		...
		if (count < 0) {
			throw runtime_error("SCRYT_ALIAS_RPC_ERROR: ERRCODE: 5522 - " + _("'count' must be 0 or greater"));
		}
	}
	if (params.size() > 1) {
		from = params[1].get_int();
		if (from < 0) {
			throw runtime_error("SCRYT_ALIAS_RPC_ERROR: ERRCODE: 5522 - " + _("'from' must be 0 or greater"));
		}
	}
	if (params.size() > 2) {
		options = params[2];
	...
	if (!paliasdb->ScanAliases(count, from, options, oRes))
		throw runtime_error("SCRYT_ALIAS_RPC_ERROR: ERRCODE: 5522 - " + _("Scan failed"));
	return oRes;
}
