...
UniValue getinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total scryt balance of the wallet\n"
            "  \"privatesend_balance\": xxxxxx, (numeric) the anonymized scryt balance of the wallet\n"
	...
	obj.push_back(Pair("version", SCRYT_CLIENT_VERSION));
	// SCRYT
...
UniValue debug(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "scryt|privatesend|instantsend|masternode|spork|keepass|mnpayments|gobject )\n"
            "Change debug category on the fly. Specify single category or use '+' to specify many.\n"
            "\nExamples:\n"
            + HelpExampleCli("debug", "scryt")
            + HelpExampleRpc("debug", "scryt+net")
        );
    ...
    UniValue operator()(const CScriptID &scriptID) const {
        ...
        if (pwalletMain && pwalletMain->GetCScript(scriptID, subscript)) {
            ...
            BOOST_FOREACH(const CTxDestination& addr, addresses)
                a.push_back(CScrytAddress(addr).ToString());
...
UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nReturn information about the given scryt address.\n"
            "\nArguments:\n"
            "1. \"address\"     (string, required) The scryt address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
			"  \"address\" : \"scrytaddress\", (string) The scryt address validated\n"
			// SCRYT
			"  \"zaddress\" : \"scrytaddress\", (string) The zcash t-addr associated with this scryt address validated\n"
			"  \"saddress\" : \"scrytaddress\", (string) The Scryt 3 S address scheme associated with this scryt address validated\n"
			"  \"btcaddress\" : \"scrytaddress\", (string) The bitcoin address associated with this scryt address validated\n"
			"  \"alias\" : \"scrytalias\", (string) The scryt alias associated with this scryt address validated\n"
    ...
    if (isValid)
    {
		...
		address = CScrytAddress(address.ToString());
...
#ifdef ENABLE_WALLET
		// Case 1: Scryt address and we have full public key:
		CScrytAddress address(ks);
		// SCRYT
...
UniValue createmultisig(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 2)
    {
        std::string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are scryt addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) scryt address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
	...
	zaddr.Set(dest, CChainParams::ADDRESS_ZEC);
	CScrytAddress addr;
	addr.Set(dest, CChainParams::ADDRESS_SCY);
...
UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The scryt address to use for the signature.\n"
...
{
    if (type == 2) {
        address = CScrytAddress(CScriptID(hash)).ToString();
    } else if (type == 1) {
        address = CScrytAddress(CKeyID(hash)).ToString();
    } else {
        return false;
    }
    return true;
...
{
    if (params[0].isStr()) {
        CScrytAddress address(params[0].get_str());
        uint160 hashBytes;
        int type = 0;
        ...
        for (std::vector<UniValue>::iterator it = values.begin(); it != values.end(); ++it) {
            ...
            CScrytAddress address(it->get_str());
            uint160 hashBytes;
            int type = 0;
            if (!address.GetIndexKey(hashBytes, type)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid address");
...
UniValue echo(const JSONRPCRequest& request)
{
    if (request.fHelp)
        throw std::runtime_error(
            ...
            "scryt-cli and the GUI. There is no server-side difference."
        );
...
{ //  category              name                      actor (function)         okSafeMode
  //  --------------------- ------------------------  -----------------------  ----------
    ...
    /* Scryt features */
    { "scryt",               "mnsync",                 &mnsync,                 true,  {} },
    { "scryt",               "spork",                  &spork,                  true,  {"value"} },
