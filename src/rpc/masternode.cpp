...
UniValue masternode(const JSONRPCRequest& request)
{
    ...
    if (strCommand == "current" || strCommand == "winner")
    {
        ...
        obj.push_back(Pair("outpoint",      mnInfo.outpoint.ToStringShort()));
        obj.push_back(Pair("payee",         CScrytAddress(mnInfo.pubKeyCollateralAddress.GetID()).ToString()));
    ...
    if (strCommand == "genkey")
    {
        ...
        return CScrytSecret(secret).ToString();
    }
    ...
    if (strCommand == "status")
    {
        ...
        if(mnodeman.Get(activeMasternode.outpoint, mn)) {
            mnObj.push_back(Pair("payee", CScrytAddress(mn.pubKeyCollateralAddress.GetID()).ToString()));
        }
...
UniValue masternodelist(const JSONRPCRequest& request)
{
                ...
                strMode != "rank" && strMode != "sentinel" && strMode != "status"))
    {
        throw std::runtime_error(
                ...
                "  payee          - Print Scryt address associated with a masternode (can be additionally filtered,\n"
                "                   partial match)\n"
        ...
        for (const auto& mnpair : mapMasternodes) {
            ...
            if (strMode == "activeseconds") {
                ...
                streamFull << std::setw(18) <<
                               mn.GetStatus() << " " <<
                               mn.nProtocolVersion << " " <<
                               CScrytAddress(mn.pubKeyCollateralAddress.GetID()).ToString() << " " <<
                ...
                objMN.push_back(Pair("payee", CScrytAddress(mn.pubKeyCollateralAddress.GetID()).ToString()));
                ...
                CScrytAddress address(mn.pubKeyCollateralAddress.GetID());
    ...
    if (strCommand == "decode")
    {
        ...
        for (const auto& mnb : vecMnb) {
            ...
            if(mnb.CheckSignature(nDos)) {
                ...
                resultObj.push_back(Pair("pubKeyCollateralAddress", CScrytAddress(mnb.pubKeyCollateralAddress.GetID()).ToString()));
                resultObj.push_back(Pair("pubKeyMasternode", CScrytAddress(mnb.pubKeyMasternode.GetID()).ToString()));
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    { "scryt",               "masternode",             &masternode,             true,  {} },
    { "scryt",               "masternodelist",         &masternodelist,         true,  {} },
    { "scryt",               "masternodebroadcast",    &masternodebroadcast,    true,  {} },
    { "scryt",               "getpoolinfo",            &getpoolinfo,            true,  {} },
    { "scryt",               "sentinelping",           &sentinelping,           true,  {} },
#ifdef ENABLE_WALLET
    { "scryt",               "privatesend",            &privatesend,            false, {} },
#endif // ENABLE_WALLET
};
