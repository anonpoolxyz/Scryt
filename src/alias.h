...
const int SCRYT_TX_VERSION = 0x7400;
...
bool DecodeAndParseScrytTx(const CTransaction& tx, int& op, std::vector<std::vector<unsigned char> >& vvch, char &type);
...
bool GetScrytData(const CTransaction &tx, std::vector<unsigned char> &vchData, std::vector<unsigned char> &vchHash, int& nOut);
bool GetScrytData(const CScript &scriptPubKey, std::vector<unsigned char> &vchData, std::vector<unsigned char> &vchHash);
bool IsSysServiceExpired(const uint64_t &nTime);
bool GetTimeToPrune(const CScript& scriptPubKey, uint64_t &nTime);
bool IsScrytScript(const CScript& scriptPubKey, int &op, std::vector<std::vector<unsigned char> > &vvchArgs);
...
void CleanupScrytServiceDatabases(int &servicesCleaned);
void GetAddress(const CAliasIndex &alias, CScrytAddress* address, CScript& script, const uint32_t nPaymentOption=1);
std::string GetScrytTransactionDescription(const CTransaction& tx, const int op, std::string& responseEnglish, const char &type, std::string& responseGUID);
...
UniValue scryttxfund_helper(const std::vector<unsigned char> &vchAlias, const std::vector<unsigned char> &vchWitness, const CRecipient &aliasRecipient, std::vector<CRecipient> &vecSend);
bool FlushScrytDBs();
