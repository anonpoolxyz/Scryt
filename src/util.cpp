...
const char * const SCRYT_CONF_FILENAME = "scryt.conf";
const char * const SCRYT_PID_FILENAME = "scrytd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "scryt" is a composite category enabling all Scryt-related debug output
                if(ptrCategory->count(std::string("scryt"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Scryt
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Scryt
    // Mac: ~/Library/Application Support/Scryt
    // Unix: ~/.scrytcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Scryt";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Scryt";
#else
    // Unix
    return pathRet / ".scrytcore";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty scryt.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override scryt.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", SCRYT_PID_FILENAME));
...
std::string CopyrightHolders(const std::string& strPrefix, unsigned int nStartYear, unsigned int nEndYear)
{
    ...
    // Check for untranslated substitution to make sure Scryt Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Scryt Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2009, nEndYear) + "The ScrytExchange developers";
    }
