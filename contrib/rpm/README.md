...
reference the release tarball available from https://scryt.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Bitcoin git repository](https://github.com/scryt/scryt) but are not part of
...
Currently only `Source10` is used, for the example `scryt.conf` file.
...
in the scryt git repository. At present only an SVG version of the Bitcoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the scryt
developers so that an official fix to the issue can make it into the next
release.

### Patch0 scryt-0.12.0-libressl.patch
...
The `scryt.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `scryt-server` package. At this point, most RPM based
...
The `scryt-server` package should have SELinux support. How to properly do
...
[Ringing Liberty](https://www.ringingliberty.com/scryt/). He has been
...
4. I renamed the `scryt` package that contains the Qt GUI to `scryt-core` as
that appears to be how the general population refers to it, in contrast to
`scryt-xt` or `scryt-classic`. I wanted to make sure the general population
