...
###   scrytcore.org and scryt.org and compares them.
...
WORKINGDIR="/tmp/scryt_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://scrytcore.org"
HOST2="https://scryt.org"
BASEDIR="/bin/"
VERSIONPREFIX="scryt-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'scryt-',
...
if [ $? -ne 0 ]; then
   echo "scryt.org failed to provide signature file, but scrytcore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "scryt.org and scrytcore.org signature files were not equal?"
