UNIX BUILD NOTES
====================
Some notes on how to build Scryt Core in Unix.
...
Note
---------------------
Always use absolute paths to configure and compile scryt and the dependencies,
...
This will build scryt-qt as well if the dependencies are met.
...
memory available when compiling Scryt Core. On systems with less, gcc can be
...
`--with-incompatible-bdb` to configure. See the section "Disable-wallet mode" to build Scryt 
Core without wallet.
...
>**Ubuntu:** db4.8 packages are available [here](https://launchpad.net/~scryt/+archive/scryt).
...
If you want to build Scryt-Qt, make sure that the required packages for Qt development are 
...
Once these are installed, they will be found by configure and a scryt-qt executable will be
built by default.
...
The version of Boost that ships with CentOS is too old to work with Scryt - version 1.55+ 
...
to your `.bashrc` as well to configure this variable on when starting `scrytd`.
...
Notes
-----
The release is built with GCC and then "strip scrytd" to strip the debug symbols, 
...
so from the root of the Scryt repository.

```shell
./contrib/install_db4.sh `pwd`
```

When compiling Scryt you will need to run `./configure` with the results of this script.
...
Security
--------
To help make your scryt installation more secure by making certain attacks impossible to
...
* Non-executable Stack
  ...
  vulnerable buffers are found. By default, scryt should be built with a non-executable stack
...
Disable-wallet mode
--------------------
When the intention is to run only a P2P node without a wallet, scryt may be compiled in 
...
[PKGBUILD](https://projects.archlinux.org/svntogit/community.git/tree/scryt/trunk/PKGBUILD). 
As mentioned above, when maintaining portability of the wallet between the standard Scryt 
