...
*Setup instructions for a Gitian build of Scryt Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Scryt
...
- [Building Scryt Core](#building-scryt-core)
...
Clone the git repositories for Scryt Core and Gitian.
...
in the Scryt Core repository under 'Fetch and build inputs' to install sources which require
...
To build Scryt Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Scryt Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/scryt/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/scryt/scryt
...
For example:
```bash
URL=https://github.com/scryt/scryt.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit scryt=${COMMIT} --url scryt=${URL} ../scryt/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit scryt=${COMMIT} --url scryt=${URL} ../scryt/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit scryt=${COMMIT} --url scryt=${URL} ../scryt/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the scryt git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../scryt/contrib/gitian-descriptors/*|sort|uniq )
...
SCYPATH=/some/root/path/scryt.git
SIGPATH=/some/root/path/scryt-detached-sigs.git

./bin/gbuild --url scryt=${SCYPATH},signature=${SIGPATH} ../scryt/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/scryt-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/scryt-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/scryt-osx-build.assert
```
...
[scryt/gitian.sigs](https://github.com/scryt/gitian.sigs/) repository, or if that's not possible to create a pull
request.
